...
class qtype_prog extends question_type {
    ...
    public function save_question_options($question) {
        ...
        if ($question->problem_mode === 'new') {
            ...
            $existing = $DB->get_record('local_prog_problem', ['code' => $code]);
            if ($existing) {
            ...
            foreach (array_unique($types) as $typeid) {
                ...
                $type->type_id = $typeid;
                $DB->insert_record('local_prog_problem_type', $type);
            }
            ...
            foreach (array_unique($languages) as $langid) {
                ...
                $DB->insert_record('local_prog_problem_language', $lang);
            }
            ...
            if ($problemid && is_numeric($problemid)) {
                $record = $DB->get_record('local_prog_problem', ['id' => $problemid], '*', IGNORE_MISSING);
        ...
        // === Enregistrement dans qtype_prog_options ===
        if ($existing = $DB->get_record('qtype_prog_options', ['questionid' => $question->id])) {
            $options = $existing;
        } else {
        ...
        if (isset($options->id)) {
            $DB->update_record('qtype_prog_options', $options);
        } else {
            $DB->insert_record('qtype_prog_options', $options);
        }
    ...
    public function get_question_options($question) {
        ...
        // ✅ Récupère les options personnalisées de qtype_prog
        $options = $DB->get_record('qtype_prog_options', ['questionid' => $question->id]);

        // ✅ Affecte le code du problème à la question
        if ($question instanceof qtype_prog_question) {
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_prog_options', ['questionid' => $questionid]);
    ...
    public function make_question($questiondata) {
        ...
        $question = new qtype_prog_question();
