...
/**
 * Define all the backup steps that will be used by the backup_progcontest_activity_task
 *
 * @copyright  2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_progcontest_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $progcontest = new backup_nested_element('progcontest', array('id'), array(
        ...
        $this->add_subplugin_structure('progcontestaccess', $progcontest, true);
        ...
        $this->add_subplugin_structure('progcontestaccess', $attempt, true);

        // Build the tree.
        $progcontest->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $qinstance->add_child($qinstancetags);
        $qinstancetags->add_child($qinstancetag);

        $progcontest->add_child($sections);
        $sections->add_child($section);

        $progcontest->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $progcontest->add_child($overrides);
        $overrides->add_child($override);

        $progcontest->add_child($grades);
        $grades->add_child($grade);

        $progcontest->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $progcontest->set_source_table('progcontest', array('id' => backup::VAR_ACTIVITYID));

        $qinstance->set_source_table('progcontest_slots',
                array('progcontestid' => backup::VAR_PARENTID));

        $qinstancetag->set_source_table('progcontest_slot_tags',
                array('slotid' => backup::VAR_PARENTID));

        $section->set_source_table('progcontest_sections',
                array('progcontestid' => backup::VAR_PARENTID));

        $feedback->set_source_table('progcontest_feedback',
                array('progcontestid' => backup::VAR_PARENTID));

        // Programmingcontest overrides to backup are different depending of user info.
        $overrideparams = array('progcontest' => backup::VAR_PARENTID);
        ...
        $override->set_source_table('progcontest_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('progcontest_grades', array('progcontest' => backup::VAR_PARENTID));
            $attempt->set_source_sql('
                    SELECT *
                    FROM {progcontest_attempts}
                    WHERE progcontest = :progcontest AND preview = 0',
                    array('progcontest' => backup::VAR_PARENTID));
        }

        // Define source alias.
        $progcontest->set_source_alias('attempts', 'attempts_number');
        ...
        $progcontest->annotate_files('mod_progcontest', 'intro', null); // This file area hasn't itemid.
        $feedback->annotate_files('mod_progcontest', 'feedback', 'id');

        // Return the root element (progcontest), wrapped into standard activity structure.
        return $this->prepare_activity_structure($progcontest);
    }
}
