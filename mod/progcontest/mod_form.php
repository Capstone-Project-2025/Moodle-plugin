...
/**
 * Defines the progcontest module ettings form.
 *
...
/**
 * Settings form for the progcontest module.
 *
 * @copyright  2006 Jamie Pratt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_progcontest_mod_form extends moodleform_mod {
    /** @var array options to be used with date_time_selector fields in the progcontest. */
    public static $datefieldoptions = array('optional' => true);

    protected $_feedbacks;
    protected static $reviewfields = array(); // Initialised in the constructor.

    /** @var int the max number of attempts allowed in any user or group override on this progcontest. */
    protected $maxattemptsanyoverride = null;

    public function __construct($current, $section, $cm, $course) {
        self::$reviewfields = array(
            'attempt'          => array('theattempt', 'progcontest'),
            'correctness'      => array('whethercorrect', 'question'),
            'marks'            => array('marks', 'progcontest'),
            ...
            'overallfeedback'  => array('reviewoverallfeedback', 'progcontest'),
        );
    ...
    protected function definition() {
        ...
        $progcontestconfig = get_config('progcontest');
        $mform = $this->_form;
        ...
        // Introduction.
        $this->standard_intro_elements(get_string('introduction', 'progcontest'));

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'timing', get_string('timing', 'progcontest'));

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen', get_string('progcontestopen', 'progcontest'),
                self::$datefieldoptions);
        $mform->addHelpButton('timeopen', 'progcontestopenclose', 'progcontest');

        $mform->addElement('date_time_selector', 'timeclose', get_string('progcontestclose', 'progcontest'),
                self::$datefieldoptions);

        // Time limit.
        $mform->addElement('duration', 'timelimit', get_string('timelimit', 'progcontest'),
                array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'progcontest');

        // What to do with overdue attempts.
        $mform->addElement('select', 'overduehandling', get_string('overduehandling', 'progcontest'),
                progcontest_get_overdue_handling_options());
        $mform->addHelpButton('overduehandling', 'overduehandling', 'progcontest');
        ...
        // Grace period time.
        $mform->addElement('duration', 'graceperiod', get_string('graceperiod', 'progcontest'),
                array('optional' => true));
        $mform->addHelpButton('graceperiod', 'graceperiod', 'progcontest');
        ...
        if (property_exists($this->current, 'grade')) {
            ...
            $currentgrade = $progcontestconfig->maximumgrade;
        }
        ...
        $mform->addElement('select', 'attempts', get_string('attemptsallowed', 'progcontest'),
                $attemptoptions);

        // Grading method.
        $mform->addElement('select', 'grademethod', get_string('grademethod', 'progcontest'),
                progcontest_get_grading_options());
        $mform->addHelpButton('grademethod', 'grademethod', 'progcontest');
        if ($this->get_max_attempts_for_any_override() < 2) {
            $mform->hideIf('grademethod', 'attempts', 'eq', 1);
        }

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'layouthdr', get_string('layout', 'progcontest'));

        $pagegroup = array();
        $pagegroup[] = $mform->createElement('select', 'questionsperpage',
                get_string('newpage', 'progcontest'), progcontest_questions_per_page_options(), array('id' => 'id_questionsperpage'));
        $mform->setDefault('questionsperpage', $progcontestconfig->questionsperpage);

        if (!empty($this->_cm) && !progcontest_has_attempts($this->_cm->instance)) {
            $pagegroup[] = $mform->createElement('checkbox', 'repaginatenow', '',
                    get_string('repaginatenow', 'progcontest'), array('id' => 'id_repaginatenow'));
        }

        $mform->addGroup($pagegroup, 'questionsperpagegrp',
                get_string('newpage', 'progcontest'), null, false);
        $mform->addHelpButton('questionsperpagegrp', 'newpage', 'progcontest');
        $mform->setAdvanced('questionsperpagegrp', $progcontestconfig->questionsperpage_adv);

        // Navigation method.
        $mform->addElement('select', 'navmethod', get_string('navmethod', 'progcontest'),
                progcontest_get_navigation_options());
        $mform->addHelpButton('navmethod', 'navmethod', 'progcontest');

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'interactionhdr', get_string('questionbehaviour', 'progcontest'));

        // Shuffle within questions.
        $mform->addElement('selectyesno', 'shuffleanswers', get_string('shufflewithin', 'progcontest'));
        $mform->addHelpButton('shuffleanswers', 'shufflewithin', 'progcontest');
        ...
        $redochoices = array(0 => get_string('no'), 1 => get_string('canredoquestionsyes', 'progcontest'));
        $mform->addElement('select', 'canredoquestions', get_string('canredoquestions', 'progcontest'), $redochoices);
        $mform->addHelpButton('canredoquestions', 'canredoquestions', 'progcontest');
        ...
        $mform->addHelpButton('attemptonlast', 'eachattemptbuildsonthelast', 'progcontest');
        if ($this->get_max_attempts_for_any_override() < 2) {
        ...
        $mform->addHelpButton('reviewoptionshdr', 'reviewoptionsheading', 'progcontest');

        // Review options.
        $this->add_review_options_group($mform, $progcontestconfig, 'during',
                mod_progcontest_display_options::DURING, true);
        $this->add_review_options_group($mform, $progcontestconfig, 'immediately',
                mod_progcontest_display_options::IMMEDIATELY_AFTER);
        $this->add_review_options_group($mform, $progcontestconfig, 'open',
                mod_progcontest_display_options::LATER_WHILE_OPEN);
        $this->add_review_options_group($mform, $progcontestconfig, 'closed',
        ...
        // Show user picture.
        $mform->addElement('select', 'showuserpicture', get_string('showuserpicture', 'progcontest'),
                progcontest_get_user_image_options());
        $mform->addHelpButton('showuserpicture', 'showuserpicture', 'progcontest');
        ...
        $mform->addElement('select', 'decimalpoints', get_string('decimalplaces', 'progcontest'),
                $options);
        $mform->addHelpButton('decimalpoints', 'decimalplaces', 'progcontest');
        ...
        $mform->addElement('select', 'questiondecimalpoints',
                get_string('decimalplacesquestion', 'progcontest'), $options);
        $mform->addHelpButton('questiondecimalpoints', 'decimalplacesquestion', 'progcontest');

        // Show blocks during progcontest attempt.
        $mform->addElement('selectyesno', 'showblocks', get_string('showblocks', 'progcontest'));
        $mform->addHelpButton('showblocks', 'showblocks', 'progcontest');

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'security', get_string('extraattemptrestrictions', 'progcontest'));

        // Require password to begin progcontest attempt.
        $mform->addElement('passwordunmask', 'progcontestpassword', get_string('requirepassword', 'progcontest'));
        $mform->setType('progcontestpassword', PARAM_TEXT);
        $mform->addHelpButton('progcontestpassword', 'requirepassword', 'progcontest');

        // IP address.
        $mform->addElement('text', 'subnet', get_string('requiresubnet', 'progcontest'));
        $mform->setType('subnet', PARAM_TEXT);
        $mform->addHelpButton('subnet', 'requiresubnet', 'progcontest');

        // Enforced time delay between progcontest attempts.
        $mform->addElement('duration', 'delay1', get_string('delay1st2nd', 'progcontest'),
                array('optional' => true));
        $mform->addHelpButton('delay1', 'delay1st2nd', 'progcontest');
        if ($this->get_max_attempts_for_any_override() < 2) {
            $mform->hideIf('delay1', 'attempts', 'eq', 1);
        }

        $mform->addElement('duration', 'delay2', get_string('delaylater', 'progcontest'),
                array('optional' => true));
        $mform->addHelpButton('delay2', 'delaylater', 'progcontest');
        ...
        $mform->addElement('select', 'browsersecurity', get_string('browsersecurity', 'progcontest'),
                progcontest_access_manager::get_browser_security_choices());
        $mform->addHelpButton('browsersecurity', 'browsersecurity', 'progcontest');

        // Any other rule plugins.
        progcontest_access_manager::add_settings_form_fields($this, $mform);

        // -------------------------------------------------------------------------------
        $mform->addElement('header', 'overallfeedbackhdr', get_string('overallfeedback', 'progcontest'));
        $mform->addHelpButton('overallfeedbackhdr', 'overallfeedback', 'progcontest');

        if (isset($this->current->grade)) {
            $needwarning = $this->current->grade === 0;
        } else {
            $needwarning = $progcontestconfig->maximumgrade == 0;
        }
        if ($needwarning) {
            $mform->addElement('static', 'nogradewarning', '',
                    get_string('nogradewarning', 'progcontest'));
        }

        $mform->addElement('static', 'gradeboundarystatic1',
                get_string('gradeboundary', 'progcontest'), '100%');

        $repeatarray = array();
        $repeatedoptions = array();
        $repeatarray[] = $mform->createElement('editor', 'feedbacktext',
                get_string('feedback', 'progcontest'), array('rows' => 3), array('maxfiles' => EDITOR_UNLIMITED_FILES,
                        'noclean' => true, 'context' => $this->context));
        $repeatarray[] = $mform->createElement('text', 'feedbackboundaries',
                get_string('gradeboundary', 'progcontest'), array('size' => 10));
        $repeatedoptions['feedbacktext']['type'] = PARAM_RAW;
        $repeatedoptions['feedbackboundaries']['type'] = PARAM_RAW;

        if (!empty($this->_instance)) {
            $this->_feedbacks = $DB->get_records('progcontest_feedback',
                    array('progcontestid' => $this->_instance), 'mingrade DESC');
            $numfeedbacks = count($this->_feedbacks);
        } else {
            $this->_feedbacks = array();
            $numfeedbacks = $progcontestconfig->initialnumfeedbacks;
        }
        $numfeedbacks = max($numfeedbacks, 1);

        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1,
                $repeatedoptions, 'boundary_repeats', 'boundary_add_fields', 3,
                get_string('addmoreoverallfeedbacks', 'progcontest'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore($mform->createElement('editor',
                "feedbacktext[$nextel]", get_string('feedback', 'progcontest'), array('rows' => 3),
                array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true,
                      'context' => $this->context)),
                'boundary_add_fields');
        $mform->insertElementBefore($mform->createElement('static',
                'gradeboundarystatic2', get_string('gradeboundary', 'progcontest'), '0%'),
                'boundary_add_fields');
        ...
        $mform->setAdvanced('modoutcomes', !empty($progcontestconfig->outcomes_adv));

        // The standard_coursemodule_elements method sets this to 100, but the
        // progcontest has its own setting, so use that.
        $mform->setDefault('grade', $progcontestconfig->maximumgrade);
    ...
    protected function add_review_options_group($mform, $progcontestconfig, $whenname,
        ...
        $mform->addGroup($group, $whenname . 'optionsgrp',
                get_string('review' . $whenname, 'progcontest'), null, false);

        foreach (self::$reviewfields as $field => $notused) {
            $cfgfield = 'review' . $field;
            if ($progcontestconfig->$cfgfield & $when) {
    ...
    public function data_preprocessing(&$toform) {
        ...
        if (count($this->_feedbacks)) {
            ...
            foreach ($this->_feedbacks as $feedback) {
                ...
                if ($toform['grade'] == 0) {
                    // When a progcontest is un-graded, there can only be one lot of
                    // feedback. If the progcontest previously had a maximum grade and
        ...
        if (isset($toform['password'])) {
            $toform['progcontestpassword'] = $toform['password'];
            unset($toform['password']);
        }

        // Load any settings belonging to the access rules.
        if (!empty($toform['instance'])) {
            $accesssettings = progcontest_access_manager::load_settings($toform['instance']);
    ...
    public function validation($data, $files) {
        ...
        if ($data['timeopen'] != 0 && $data['timeclose'] != 0 &&
            ...
            $errors['timeclose'] = get_string('closebeforeopen', 'progcontest');
        }
        ...
        if ($data['overduehandling'] == 'graceperiod') {
            ...
            if ($data['graceperiod'] <= $graceperiodmin) {
                $errors['graceperiod'] = get_string('graceperiodtoosmall', 'progcontest', format_time($graceperiodmin));
            }
        }
        ...
        if (array_key_exists('completion', $data) && $data['completion'] == COMPLETION_TRACKING_AUTOMATIC) {
            ...
            if ($completionpass && (empty($data['gradepass']) || grade_floatval($data['gradepass']) == 0)) {
                if (isset($data['completionpass'])) {
                    $errors['completionpassgroup'] = get_string('gradetopassnotset', 'progcontest');
                } else {
                    $errors['gradepass'] = get_string('gradetopassmustbeset', 'progcontest');
                }
            }
        }
        ...
        if (!empty($data['completionminattempts'])) {
            if ($data['attempts'] > 0 && $data['completionminattempts'] > $data['attempts']) {
                $errors['completionminattemptsgroup'] = get_string('completionminattemptserror', 'progcontest');
            }
        }
        ...
        while (!empty($data['feedbackboundaries'][$i] )) {
                ...
                if ($boundary[strlen($boundary) - 1] == '%') {
                    ...
                    if (is_numeric($boundary)) {
                        ...
                        $errors["feedbackboundaries[$i]"] =
                                get_string('feedbackerrorboundaryformat', 'progcontest', $i + 1);
                    }
            ...
            if (is_numeric($boundary) && $boundary <= 0 || $boundary >= $data['grade'] ) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrorboundaryoutofrange', 'progcontest', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] =
                        get_string('feedbackerrororder', 'progcontest', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] =
                            get_string('feedbackerrorjunkinboundary', 'progcontest', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] =
                        get_string('feedbackerrorjunkinfeedback', 'progcontest', $i + 1);
            }
        }
        ...
        // Any other rule plugins.
        $errors = progcontest_access_manager::validate_settings_form_fields($errors, $data, $files, $this);
    ...
    public function add_completion_rules() {
        ...
        $group = array();
        $group[] = $mform->createElement('advcheckbox', 'completionpass', null, get_string('completionpass', 'progcontest'),
                array('group' => 'cpass'));
        $mform->disabledIf('completionpass', 'completionusegrade', 'notchecked');
        $group[] = $mform->createElement('advcheckbox', 'completionattemptsexhausted', null,
                get_string('completionattemptsexhausted', 'progcontest'),
                array('group' => 'cattempts'));
        $mform->disabledIf('completionattemptsexhausted', 'completionpass', 'notchecked');
        $mform->addGroup($group, 'completionpassgroup', get_string('completionpass', 'progcontest'), ' &nbsp; ', false);
        $mform->addHelpButton('completionpassgroup', 'completionpass', 'progcontest');
        $items[] = 'completionpassgroup';

        $group = array();
        $group[] = $mform->createElement('checkbox', 'completionminattemptsenabled', '',
            get_string('completionminattempts', 'progcontest'));
        $group[] = $mform->createElement('text', 'completionminattempts', '', array('size' => 3));
        $mform->setType('completionminattempts', PARAM_INT);
        $mform->addGroup($group, 'completionminattemptsgroup', get_string('completionminattemptsgroup', 'progcontest'), array(' '), false);
    ...
    public function get_max_attempts_for_any_override() {
        ...
        if (empty($this->_instance)) {
            // Programmingcontest not created yet, so no overrides.
            return 1;
        }
