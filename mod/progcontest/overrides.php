...
/**
 * This page handles listing of progcontest overrides
 *
...
require_once($CFG->dirroot.'/mod/progcontest/override_form.php');
...
$progcontest = $DB->get_record('progcontest', ['id' => $cm->instance], '*', MUST_EXIST);
...
$canedit = has_capability('mod/progcontest:manageoverrides', $context);
if (!$canedit) {
    require_capability('mod/progcontest:viewoverrides', $context);
}

$progcontestgroupmode = groups_get_activity_groupmode($cm);
$showallgroups = ($progcontestgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $context);
...
$url = new moodle_url('/mod/progcontest/overrides.php', ['cmid' => $cm->id, 'mode' => $mode]);

$title = get_string('overridesforprogcontest', 'progcontest',
        format_string($progcontest->name, true, ['context' => $context]));
$PAGE->set_url($url);
...
if (!empty($orphaned)) {
    $DB->delete_records_list('progcontest_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    if ($groups) {
        ...
        $sql = "SELECT o.*, g.name
                 ...
                 WHERE o.progcontest = :progcontestid AND g.id $insql
              ORDER BY g.name";
    ...
    $overrides = $DB->get_records_sql("
            SELECT o.*, $userfields
              ...
              $groupsjoin
             WHERE o.progcontest = :progcontestid
               $groupswhere
             ORDER BY $sort
            ", $params);
}
...
$table->head[] = get_string('overrides', 'progcontest');
...
$overridedeleteurl = new moodle_url('/mod/progcontest/overridedelete.php');
$overrideediturl = new moodle_url('/mod/progcontest/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    $active = true;
    if (!$groupmode) {
        if (!has_capability('mod/progcontest:attempt', $context, $override->userid)) {
            // User not allowed to take the progcontest.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('progcontestopens', 'progcontest');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'progcontest');
    }
    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('progcontestcloses', 'progcontest');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'progcontest');
    }
    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'progcontest');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'progcontest');
    }
    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'progcontest');
        $values[] = $override->attempts > 0 ?
                $override->attempts : get_string('unlimited');
    }
    // Format password.
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'progcontest');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'progcontest') : get_string('none', 'progcontest');
    }
...
echo html_writer::start_tag('div', ['id' => 'progcontestoverrides']);
if (count($table->data)) {
    echo html_writer::table($table);
} else {
    if ($groupmode) {
        echo $OUTPUT->notification(get_string('overridesnoneforgroups', 'progcontest'), 'info', false);
    } else {
        echo $OUTPUT->notification(get_string('overridesnoneforusers', 'progcontest'), 'info', false);
    }
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'progcontest'), 'info', false);
}
...
if ($canedit) {
    ...
    if ($groupmode) {
        if (empty($groups)) {
            // There are no groups.
            echo $OUTPUT->notification(get_string('groupsnone', 'progcontest'), 'error');
            $options['disabled'] = true;
        }
        echo $OUTPUT->single_button($overrideediturl->out(true,
                ['action' => 'addgroup', 'cmid' => $cm->id]),
                get_string('addnewgroupoverride', 'progcontest'), 'post', $options);
    } else {
        $users = [];
        // See if there are any students in the progcontest.
        if ($showallgroups) {
            $users = get_users_by_capability($context, 'mod/progcontest:attempt', 'u.id');
            $nousermessage = get_string('usersnone', 'progcontest');
        } else if ($groups) {
            $users = get_users_by_capability($context, 'mod/progcontest:attempt', 'u.id', '', '', '', array_keys($groups));
            $nousermessage = get_string('usersnone', 'progcontest');
        } else {
            $nousermessage = get_string('groupsnone', 'progcontest');
        }
        ...
        echo $OUTPUT->single_button($overrideediturl->out(true,
                ...
                get_string('addnewuseroverride', 'progcontest'), 'get', $options);
    }
