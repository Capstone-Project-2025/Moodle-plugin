...
/**
 * Class to store the options for a {@link progcontest_responses_report}.
 *
...
require_once($CFG->dirroot . '/mod/progcontest/report/attemptsreport_options.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_responses_options extends mod_progcontest_attempts_report_options {
    ...
    protected function get_url_params() {
        ...
        if (progcontest_allows_multiple_tries($this->progcontest)) {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->showqtext     = get_user_preferences('progcontest_report_responses_qtext', $this->showqtext);
        $this->showresponses = get_user_preferences('progcontest_report_responses_resp',  $this->showresponses);
        $this->showright     = get_user_preferences('progcontest_report_responses_right', $this->showright);
        if (progcontest_allows_multiple_tries($this->progcontest)) {
            $this->whichtries    = get_user_preferences('progcontest_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function update_user_preferences() {
        ...
        set_user_preference('progcontest_report_responses_qtext', $this->showqtext);
        set_user_preference('progcontest_report_responses_resp',  $this->showresponses);
        set_user_preference('progcontest_report_responses_right', $this->showright);
        if (progcontest_allows_multiple_tries($this->progcontest)) {
            set_user_preference('progcontest_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function resolve_dependencies() {
        ...
        $this->checkboxcolumn = has_capability('mod/progcontest:deleteattempts', context_module::instance($this->cm->id))
                && ($this->attempts != progcontest_attempts_report::ENROLLED_WITHOUT);
    }
}
