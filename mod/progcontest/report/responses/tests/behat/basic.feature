@mod @mod_progcontest @progcontest @progcontest_reponses
Feature: Basic use of the Responses report
  In order to see how my students are progressing
  As a teacher
  I need to see all their progcontest responses
  ...
  Background: Using the Responses report
    ...
    And the following "activities" exist:
      ...
      | progcontest       | Programmingcontest 1 | Programmingcontest 1 description | C1     | progcontest1    | interactive        |
  ...
  Scenario: Report works when there are no attempts
    Given I am on the "Programmingcontest 1" "progcontest activity" page logged in as teacher
    When I navigate to "Results > Responses" in current page administration
    Then I should see "Attempts: 0"
    And I should see "Nothing to display"
    And I set the field "Attempts from" to "enrolled users who have not attempted the progcontest"

  @javascript
  Scenario: Report works when there are attempts
    Given user "student1" has started an attempt at progcontest "Programmingcontest 1"
    And user "student1" has checked answers in their attempt at progcontest "Programmingcontest 1":
      | slot | response |
      |   1  | 1.0      |
    And user "student1" has checked answers in their attempt at progcontest "Programmingcontest 1":
      | slot | response |
    ...
    And user "student1" has checked answers in their attempt at progcontest "Programmingcontest 1":
      | slot | response |
      |   1  | 3.14     |
    And user "student1" has finished an attempt at progcontest "Programmingcontest 1"

    And I am on the "Programmingcontest 1" "progcontest activity" page logged in as teacher
    And I navigate to "Results > Responses" in current page administration
    Then I should see "Attempts: 1"
    And I should see "Student One"
    And I should not see "Student Two"
    And I set the field "Attempts from" to "enrolled users who have, or have not, attempted the progcontest"
  ...
  Scenario: Report does not allow strange combinations of options
    ...
    When I set the field "Attempts from" to "enrolled users who have not attempted the progcontest"
