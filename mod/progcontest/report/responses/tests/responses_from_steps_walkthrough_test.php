...
namespace progcontest_responses;
 ...
 */
class responses_from_steps_walkthrough_test extends \mod_progcontest\attempt_walkthrough_from_csv_test {
     ...
     *
     * @param array $progcontestsettings settings to override default settings for progcontest created by generator. Taken from progcontestzes.csv.
    ...
    public function test_walkthrough_from_csv($progcontestsettings, $csvdata) {
        ...
        $progcontestattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        foreach ($csvdata['responses'] as $responsesfromcsv) {
            ...
            if (!isset($progcontestattemptids[$responses['progcontestattempt']])) {
                throw new \coding_exception("There is no progcontestattempt {$responses['progcontestattempt']}!");
            }
            $this->assert_response_test($progcontestattemptids[$responses['progcontestattempt']], $responses);
        }
    }

    protected function assert_response_test($progcontestattemptid, $responses) {
        $progcontestattempt = progcontest_attempt::create($progcontestattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $progcontestattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new \coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in progcontestattempt {$responses['progcontestattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  progcontestattempt {$responses['progcontestattempt']} in $column, slot $slot, ".
