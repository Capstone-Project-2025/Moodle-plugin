...
use question_bank;
use question_finder;
use progcontest_statistics_report;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/progcontest/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/progcontest/report/default.php');
require_once($CFG->dirroot . '/mod/progcontest/report/statistics/report.php');
 ...
 *
 * The progcontest stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-progcontest-tools/tree/master/statsspreadsheet
 *
 ...
 */
class stats_from_steps_walkthrough_test extends \mod_progcontest\attempt_walkthrough_from_csv_test {

    /**
     * @var progcontest_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a progcontest add questions to it, walk through progcontest attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($progcontestsettings, $csvdata) {
        ...
        $whichattempts = progcontest_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudentsjoins = new \core\dml\sql_join();
        list($questions, $progconteststats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($progcontestsettings['testnumber'] === '00') {
            $this->check_variants_count_for_progcontest_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_progcontest_stats_for_progcontest_00($progconteststats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_progcontest_00($questions, $questionstats, $whichtries, $qubaids) {
    ...
    /**
     * @param $progconteststats
     */
    protected function check_progcontest_stats_for_progcontest_00($progconteststats) {
        $progconteststatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($progconteststatsexpected as $statname => $statvalue) {
            $this->assertEqualsWithDelta($statvalue, $progconteststats->$statname, abs($statvalue) * 1.5e-5, $progconteststats->$statname);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $progconteststats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new progcontest_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->progcontest);
        list($progconteststats, $questionstats) = $this->report->get_all_stats_and_analysis($this->progcontest,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = progcontest_statistics_qubaids_condition($this->progcontest->id, $groupstudentsjoins, $whichattempts);

        // We will create some progcontest and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $progcontestcalc = new calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($progcontestcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $progconteststats, $questionstats, $qubaids);
        }
        return array($questions, $progconteststats, $questionstats, $qubaids);
    }
