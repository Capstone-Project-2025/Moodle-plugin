@mod @mod_progcontest @progcontest @progcontest_statistics
Feature: Basic use of the Statistics report
  ...
  Scenario: Report works when there are no attempts
    Given I am on the "Programmingcontest 1" "progcontest activity" page logged in as teacher1
    And I navigate to "Results > Statistics" in current page administration
    Then I should see "No attempts have been made at this progcontest, or all attempts have questions that need manual grading."
    And I should not see "Statistics for question positions"
    And "Show chart data" "link" should not exist
    When user "student1" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      |   1  | True     |
      |   2  | False    |
      |   3  | False    |
    And user "student2" has attempted "Programmingcontest 1" with responses:
      ...
      |   2  | True     |
      |   3  | True     |
    And user "student3" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      |   1  | False    |
      |   2  | False    |
      |   3  | False    |
    And I press "Show report"
    Then I should not see "No attempts have been made at this progcontest, or all attempts have questions that need manual grading."
