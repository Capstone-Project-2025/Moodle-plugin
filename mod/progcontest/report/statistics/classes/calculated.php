...
class calculated {
    ...
    /* Following stats all described here : http://docs.moodle.org/dev/progcontest_statistics_calculations#Test_statistics  */
     ...
     */
    public function get_formatted_progcontest_info_data($course, $cm, $progcontest) {
        ...
        // General information about the progcontest.
        $progcontestinfo = array();
        $progcontestinfo[get_string('progcontestname', 'progcontest_statistics')] = format_string($progcontest->name);
        $progcontestinfo[get_string('coursename', 'progcontest_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $progcontestinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($progcontest->timeopen) {
            $progcontestinfo[get_string('progcontestopen', 'progcontest')] = userdate($progcontest->timeopen);
        }
        if ($progcontest->timeclose) {
            $progcontestinfo[get_string('progcontestclose', 'progcontest')] = userdate($progcontest->timeclose);
        }
        if ($progcontest->timeopen && $progcontest->timeclose) {
            $progcontestinfo[get_string('duration', 'progcontest_statistics')] =
                format_time($progcontest->timeclose - $progcontest->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = progcontest_report_scale_summarks_as_percentage($value, $progcontest);
                    break;
                case 'number_format_percent':
                    $formattedvalue = progcontest_format_grade($progcontest, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $progcontest->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $progcontestinfo[get_string($property, 'progcontest_statistics',
                                 calculator::using_attempts_lang_string($this->whichattempts))] = $formattedvalue;
        }

        return $progcontestinfo;
    }
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('progcontest_statistics', $toinsert);

    }

    /**
     * Given a record from 'progcontest_statistics' table load the data into the properties of this class.
     *
