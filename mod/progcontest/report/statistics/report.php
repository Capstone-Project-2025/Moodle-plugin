...
/**
 * Programmingcontest statistics report class.
 *
...
require_once($CFG->dirroot . '/mod/progcontest/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/progcontest/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/progcontest/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/progcontest/report/statistics/statisticslib.php');
/**
 * The progcontest statistics report provides summary information about each question in
 * a progcontest, compared to the whole progcontest. It also provides a drill-down to more
 * detailed information about each question.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_statistics_report extends progcontest_default_report {

    /** @var context_module context of this progcontest.*/
    protected $context;

    /** @var progcontest_statistics_table instance of table class used for main questions stats table. */
    protected $table;
     ...
     * Display the report.
     */
    public function display($progcontest, $cm, $course) {
        global $OUTPUT, $DB;
        ...
        if (!progcontest_has_questions($progcontest->id)) {
            $this->print_header_and_tabs($cm, $course, $progcontest, 'statistics');
            echo progcontest_no_questions_message($progcontest, $cm, $this->context);
            return true;
        }
        ...
        $whichattempts = optional_param('whichattempts', $progcontest->grademethod, PARAM_INT);
        ...
        $reporturl = new moodle_url('/mod/progcontest/report.php', $pageoptions);

        $mform = new progcontest_statistics_settings_form($reporturl, compact('progcontest'));

        $mform->set_data(array('whichattempts' => $whichattempts, 'whichtries' => $whichtries));

        if ($whichattempts != $progcontest->grademethod) {
        ...
        if (empty($currentgroup)) {
            ...
            // All users who can attempt progcontestzes and who are in the currently selected group.
            $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                    array('mod/progcontest:reviewmyattempts', 'mod/progcontest:attempt'), $currentgroup);
        ...
        $qubaids = progcontest_statistics_qubaids_condition($progcontest->id, $groupstudentsjoins, $whichattempts);
        ...
        $this->table = new progcontest_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'progcontest_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'progcontest_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = progcontest_report_download_filename($report, $courseshortname, $progcontest->name);
        $this->table->is_downloading($download, $filename,
                get_string('progconteststructureanalysis', 'progcontest_statistics'));
        $questions = $this->load_and_initialise_questions_for_calculations($progcontest);
        ...
        if (!$nostudentsingroup) {
            ...
            list($progconteststats, $questionstats) =
                $this->get_all_stats_and_analysis($progcontest, $whichattempts, $whichtries, $groupstudentsjoins, $questions, $progress);
        } else {
            // Or create empty stats containers.
            $progconteststats = new \progcontest_statistics\calculated($whichattempts);
            $questionstats = new \core_question\statistics\questions\all_calculated_for_qubaid_condition();
        }

        // Set up the table.
        $this->table->statistics_setup($progcontest, $cm->id, $reporturl, $progconteststats->s());
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && $nostudentsingroup) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'progcontest_statistics'));
                }
            }

            if (!$this->table->is_downloading() && $progconteststats->s() == 0) {
                echo $OUTPUT->notification(get_string('nogradedattempts', 'progcontest_statistics'));
            }
        ...
        if ($everything) { // Implies is downloading.
            ...
            $progcontestinfo = $progconteststats->get_formatted_progcontest_info_data($course, $cm, $progcontest);
            $this->download_progcontest_info_table($progcontestinfo);

            if ($progconteststats->s()) {
                $this->output_progcontest_structure_analysis_table($questionstats);

                if ($this->table->is_downloading() == 'html' && $progconteststats->s() != 0) {
                    $this->output_statistics_graph($progcontest->id, $qubaids);
                }
            ...
            $this->output_individual_question_data($progcontest, $questionstats->for_subq($qid, $variantno));
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                              get_string('backtoprogcontestreport', 'progcontest_statistics') . '</a>',
            ...
            if ($variantno === null &&
                ...
                if (!$this->table->is_downloading()) {
                    ...
                    echo $OUTPUT->heading(get_string('slotstructureanalysis', 'progcontest_statistics', $number), 3);
                }
                ...
                $this->output_individual_question_data($progcontest, $questionstats->for_slot($slot, $variantno));
            ...
            echo $OUTPUT->heading(get_string('progcontestinformation', 'progcontest_statistics'), 3);
            echo $this->output_caching_info($progconteststats->timemodified, $progcontest->id, $groupstudentsjoins, $whichattempts, $reporturl);
            echo $this->everything_download_options($reporturl);
            $progcontestinfo = $progconteststats->get_formatted_progcontest_info_data($course, $cm, $progcontest);
            echo $this->output_progcontest_info_table($progcontestinfo);
            if ($progconteststats->s()) {
                echo $OUTPUT->heading(get_string('progconteststructureanalysis', 'progcontest_statistics'), 3);
                $this->output_progcontest_structure_analysis_table($questionstats);
                $this->output_statistics_graph($progcontest, $qubaids);
            }
        }
     ...
     */
    protected function output_individual_question_data($progcontest, $questionstat) {
        global $OUTPUT;

        // On-screen display. Show a summary of the question's place in the progcontest,
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'progcontest'), $progcontest->name);
        $questioninfotable->data[] = array(get_string('questionname', 'progcontest_statistics'),
                $questionstat->question->name.'&nbsp;'.$datumfromtable['actions']);

        if ($questionstat->variant !== null) {
            $questioninfotable->data[] = array(get_string('variant', 'progcontest_statistics'), $questionstat->variant);

        }
        $questioninfotable->data[] = array(get_string('questiontype', 'progcontest_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($questionstat->question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'progcontest_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'progcontest_statistics'),
            'facility' => get_string('facility', 'progcontest_statistics'),
            'sd' => get_string('standarddeviationq', 'progcontest_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'progcontest_statistics'),
            'intended_weight' => get_string('intended_weight', 'progcontest_statistics'),
            'effective_weight' => get_string('effective_weight', 'progcontest_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'progcontest_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'progcontest_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'progcontest_statistics'), 3);
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($questionstat->question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'progcontest_statistics'), 3);
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_question_preview_urls($question->questiontext, $question->id,
                ...
                $this->context->id, 'progcontest_statistics');
    ...
    protected function output_individual_question_response_analysis($question, $variantno, $s, $reporturl, $qubaids,
        ...
        $qtable = new progcontest_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'progcontest_statistics'), 3);
            ...
            if (!empty($question->number) && !is_null($variantno)) {
                $questiontabletitle = get_string('analysisnovariant', 'progcontest_statistics', $a);
            } else if (!empty($question->number)) {
                $questiontabletitle = get_string('analysisno', 'progcontest_statistics', $a);
            } else if (!is_null($variantno)) {
                $questiontabletitle = get_string('analysisvariant', 'progcontest_statistics', $a);
            } else {
                $questiontabletitle = get_string('analysisnameonly', 'progcontest_statistics', $a);
            }

            if ($this->table->is_downloading() == 'html') {
                $questiontabletitle = get_string('analysisofresponsesfor', 'progcontest_statistics', $questiontabletitle);
            }
    ...
    /**
     * Output the table that lists all the questions in the progcontest with their statistics.
     *
     ...
     *                                                                                               the progcontest including subqs and
     *                                                                                               variants.
     */
    protected function output_progcontest_structure_analysis_table($questionstats) {
            ...
            if (is_null($structureanalysis)) {
                ...
                foreach ($structureanalysis as $row) {
                    ...
                    if ($row instanceof \core_question\statistics\questions\calculated_question_summary) {
                        ...
                        $bgcssclass = 'progcontest_statistics-summaryrow';
    ...
    /**
     * Return HTML for table of overall progcontest statistics.
     *
     * @param array $progcontestinfo as returned by {@link get_formatted_progcontest_info_data()}.
     * @return string the HTML.
     */
    protected function output_progcontest_info_table($progcontestinfo) {

        $progcontestinfotable = new html_table();
        $progcontestinfotable->align = array('center', 'center');
        $progcontestinfotable->width = '60%';
        $progcontestinfotable->attributes['class'] = 'generaltable titlesleft';
        $progcontestinfotable->data = array();

        foreach ($progcontestinfo as $heading => $value) {
             $progcontestinfotable->data[] = array($heading, $value);
        }

        return html_writer::table($progcontestinfotable);
    }

    /**
     * Download the table of overall progcontest statistics.
     *
     * @param array $progcontestinfo as returned by {@link get_formatted_progcontest_info_data()}.
     */
    protected function download_progcontest_info_table($progcontestinfo) {
        global $OUTPUT;
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('progcontestinformation', 'progcontest_statistics'));
     ...
     *
     * @param int|object $progcontestorid The progcontest, or its ID.
     ...
     */
    protected function output_statistics_graph($progcontestorid, $qubaids) {
        global $DB, $PAGE;

        $progcontest = $progcontestorid;
        if (!is_object($progcontest)) {
            $progcontest = $DB->get_record('progcontest', array('id' => $progcontestorid), '*', MUST_EXIST);
        }

        // Load the rest of the required data.
        $questions = progcontest_report_get_significant_questions($progcontest);
        ...
        $fieldstoplot = [
            'facility' => get_string('facility', 'progcontest_statistics'),
            'discriminativeefficiency' => get_string('discriminative_efficiency', 'progcontest_statistics')
        ];
        ...
        $chart->get_xaxis(0, true)->set_label(get_string('position', 'progcontest_statistics'));
        ...
        $graphname = get_string('statisticsreportgraph', 'progcontest_statistics');
        echo $output->chart($chart, $graphname);
    }

    /**
     * Get the progcontest and question statistics, either by loading the cached results,
     ...
     * @return array with 2 elements:    - $progconteststats The statistics for overall attempt scores.
    ...
    public function get_all_stats_and_analysis(
            $progcontest, $whichattempts, $whichtries, \core\dml\sql_join $groupstudentsjoins, $questions, $progress = null) {
        ...
        $progcontestcalc = new \progcontest_statistics\calculator($progress);

        $progress->start_progress('', 3);
        if ($progcontestcalc->get_last_calculated_time($qubaids) === false) {

            // Recalculate now.
            $questionstats = $qcalc->calculate($qubaids);
            $progress->progress(1);

            $progconteststats = $progcontestcalc->calculate($progcontest->id, $whichattempts, $groupstudentsjoins, count($questions),
                                              $qcalc->get_sum_of_mark_variance());
            $progress->progress(2);
        } else {
            $progconteststats = $progcontestcalc->get_cached($qubaids);
            $progress->progress(1);
        ...
        return array($progconteststats, $questionstats);
    }
    ...
    protected function get_progress_trace_instance() {
        if ($this->progress === null) {
            if (!$this->table->is_downloading()) {
                $this->progress = new \core\progress\display_if_slow(get_string('calculatingallstats', 'progcontest_statistics'));
    ...
    /**
     * Analyse responses for all questions and sub questions in this progcontest.
     *
    ...
    /**
     * Return a little form for the user to request to download the full report, including progcontest stats and response analysis for
    ...
    protected function everything_download_options(moodle_url $reporturl) {
        global $OUTPUT;
        return $OUTPUT->download_dataformat_selector(get_string('downloadeverything', 'progcontest_statistics'),
     ...
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $progcontestid, $groupstudentsjoins, $whichattempts, $reporturl) {
        global $DB, $OUTPUT;
        ...
        list($fromqa, $whereqa, $qaparams) = progcontest_statistics_attempts_sql($progcontestid, $groupstudentsjoins, $whichattempts, true);
        $count = $DB->count_records_sql("
                SELECT COUNT(1)
                FROM $fromqa
                WHERE $whereqa
                AND progcontesta.timefinish > {$lastcachetime}", $qaparams);
        ...
        $output .= get_string('lastcalculated', 'progcontest_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'progcontest_statistics'));
    ...
    protected function clear_cached_data($qubaids) {
        global $DB;
        $DB->delete_records('progcontest_statistics', array('hashcode' => $qubaids->get_hash_code()));
    ...
    /**
     * Load the questions in this progcontest and add some properties to the objects needed in the reports.
     *
     * @param object $progcontest the progcontest.
     * @return array of questions for this progcontest.
     */
    public function load_and_initialise_questions_for_calculations($progcontest) {
        // Load the questions.
        $questions = progcontest_report_get_significant_questions($progcontest);
        $questionids = array();
        foreach ($questions as $question) {
