...
/**
 * Base class for the options that control what is visible in an {@link progcontest_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_progcontest_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the progcontest being reported on. */
    public $progcontest;

    /** @var object the course module objects for the progcontest being reported on. */
    public $cm;

    /** @var object the course settings for the course the progcontest is in. */
    public $course;

    /**
     * @var array form field name => corresponding progcontest_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => progcontest_attempt::IN_PROGRESS,
        'stateoverdue'    => progcontest_attempt::OVERDUE,
        'statefinished'   => progcontest_attempt::FINISHED,
        'stateabandoned'  => progcontest_attempt::ABANDONED,
    );

    /**
     * @var string progcontest_attempts_report::ALL_WITH or progcontest_attempts_report::ENROLLED_WITH
     *      progcontest_attempts_report::ENROLLED_WITHOUT or progcontest_attempts_report::ENROLLED_ALL
     */
    public $attempts = progcontest_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of progcontest_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(progcontest_attempt::IN_PROGRESS, progcontest_attempt::OVERDUE,
            progcontest_attempt::FINISHED, progcontest_attempt::ABANDONED);
    ...
    public $pagesize = progcontest_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $progcontest the settings for the progcontest being reported on.
     * @param object $cm the course module objects for the progcontest being reported on.
     * @param object $coures the course settings for the coures this progcontest is in.
     */
    public function __construct($mode, $progcontest, $cm, $course) {
        $this->mode   = $mode;
        $this->progcontest   = $progcontest;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = progcontest_report_should_show_grades($progcontest, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/progcontest/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('progcontest_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('progcontest_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == progcontest_attempts_report::ALL_WITH) {
                $this->attempts = progcontest_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = progcontest_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(progcontest_attempts_report::ALL_WITH, progcontest_attempts_report::ENROLLED_WITH,
                progcontest_attempts_report::ENROLLED_WITHOUT, progcontest_attempts_report::ENROLLED_ALL))) {
            $this->attempts = progcontest_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!progcontest_report_can_filter_only_graded($this->progcontest)) {
        ...
        if ($this->attempts == progcontest_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = progcontest_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(progcontest_attempt::FINISHED, $this->states);
    }
}
