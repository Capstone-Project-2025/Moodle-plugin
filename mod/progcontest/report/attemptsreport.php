...
/**
 * Base class for progcontest reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class progcontest_attempts_report extends progcontest_default_report {
    ...
    /** @var object the progcontest context. */
    protected $context;

    /** @var mod_progcontest_attempts_report_form The settings form to use. */
    protected $form;
     ...
     */
    protected function init($mode, $formclass, $progcontest, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = progcontest_report_qm_filter_select($progcontest);

        $this->form = new $formclass($this->get_base_url(),
                array('progcontest' => $progcontest, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                array('mod/progcontest:attempt', 'mod/progcontest:reviewmyattempts'), $currentgroup);

        return array($currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins);
    }

    /**
     * Outputs the things you commonly want at the top of a progcontest report.
     *
     ...
     * @param mod_progcontest_attempts_report_options $options the current report settings.
     * @param int $currentgroup the current group.
     * @param bool $hasquestions whether there are any questions in the progcontest.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $progcontest,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $progcontest, $this->mode);
        ...
        if ($strattemptnum = progcontest_num_attempt_summary($progcontest, $cm, true, $currentgroup)) {
            echo '<div class="progcontestattemptcounts">' . $strattemptnum . '</div>';
        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'progcontest');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'progcontest');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'progcontest');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'progcontest');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this progcontest.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($progcontest, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'progcontest') . '/' .
                    progcontest_format_grade($progcontest, $progcontest->grade);
        }

        if ($includefeedback && progcontest_has_feedback($progcontest)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'progcontest');
        }
    }
     ...
     * @param mod_progcontest_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_progcontest_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/progcontest:deleteattempts', $this->context);
                    $this->delete_selected_attempts($progcontest, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the progcontest attempts
     * @param object $progcontest the progcontest settings. Attempts that don't belong to
     * this progcontest are not deleted.
     ...
     */
    protected function delete_selected_attempts($progcontest, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT progcontesta.*
                          FROM {progcontest_attempts} progcontesta
                          JOIN {user} u ON u.id = progcontesta.userid
                         WHERE progcontesta.id = :attemptid";
            } else {
                $sql = "SELECT progcontesta.*
                          FROM {progcontest_attempts} progcontesta
                          JOIN {user} u ON u.id = progcontesta.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND progcontesta.id = :attemptid";
            }
            $params = $allowedjoins->params + array('attemptid' => $attemptid);
            $attempt = $DB->get_record_sql($sql, $params, IGNORE_MULTIPLE);
            if (!$attempt || $attempt->progcontest != $progcontest->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this progcontest and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling progcontest_delete_attempt().
