...
/**
 * Base class for the table used by a {@link progcontest_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class progcontest_attempts_report_table extends table_sql {
    ...
    /** @var object the progcontest settings for the progcontest we are reporting on. */
    ...
    /** @var object mod_progcontest_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this progcontest.. */
    protected $questions;
    ...
    protected $togglegroup = 'progcontest-attempts';
     ...
     * @param mod_progcontest_attempts_report_options $options
     ...
     */
    public function __construct($uniqueid, $progcontest, $context, $qmsubselect,
            mod_progcontest_attempts_report_options $options, \core\dml\sql_join $groupstudentsjoins, \core\dml\sql_join $studentsjoins,
            $questions, $reporturl) {
    ...
    public function col_checkbox($attempt) {
        ...
        if ($attempt->attempt) {
            $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false, [
                ...
                'label' => get_string('selectattempt', 'progcontest'),
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/progcontest/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'progcontest'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return progcontest_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = progcontest_report_feedback_for_grade(
                progcontest_rescale_grade($attempt->sumgrades, $this->progcontest, false),
                $this->progcontest->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/progcontest/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'progcontest')));
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as progcontesta.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(progcontesta.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                progcontesta.uniqueid AS usageid,
                progcontesta.id AS attempt,
                u.id AS userid,
                u.idnumber,
                u.picture,
                u.imagealt,
                ...
                u.email' . $extrafields . ',
                progcontesta.state,
                progcontesta.sumgrades,
                progcontesta.timefinish,
                progcontesta.timestart,
                CASE WHEN progcontesta.timefinish = 0 THEN null
                     WHEN progcontesta.timefinish > progcontesta.timestart THEN progcontesta.timefinish - progcontesta.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and progcontest_attempts tables.
        $from = " {user} u";
        $from .= "\nLEFT JOIN {progcontest_attempts} progcontesta ON
                                    progcontesta.userid = u.id AND progcontesta.progcontest = :progcontestid";
        $params = array('progcontestid' => $this->progcontest->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (progcontesta.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = progcontest_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case progcontest_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'progcontesta.id IS NOT NULL AND progcontesta.preview = 0';
                break;
            case progcontest_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "progcontesta.preview = 0 AND progcontesta.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case progcontest_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "progcontesta.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case progcontest_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(progcontesta.preview = 0 OR progcontesta.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (progcontesta.state $statesql OR progcontesta.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{progcontest_attempts} {$alias}progcontesta", "{$alias}progcontesta.uniqueid",
                "{$alias}progcontesta.progcontest = :{$alias}progcontestid", array("{$alias}progcontestid" => $this->sql->params['progcontestid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = progcontesta.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT progcontesta.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) progcontestasubquery", 'progcontestasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['progcontesta.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/progcontest:deleteattempts', $this->context)) {
            $deletebuttonparams = [
                'type'  => 'submit',
                'class' => 'btn btn-secondary mr-1',
                'id'    => 'deleteattemptsbutton',
                'name'  => 'delete',
                'value' => get_string('deleteselected', 'progcontest_overview'),
                'data-action' => 'toggle',
            ...
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'progcontest')));
        }
    }
     ...
     *
     * It returns the HTML for a master \core\output\checkbox_toggleall component that selects/deselects all progcontest attempts.
     *
    ...
    public function checkbox_col_header(string $columnname) {
        ...
        $selectalltext = get_string('selectall', 'progcontest');
        $deselectalltext = get_string('selectnone', 'progcontest');
