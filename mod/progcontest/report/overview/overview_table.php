...
/**
 * This file defines the progcontest grades table.
 *
...
require_once($CFG->dirroot . '/mod/progcontest/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the progcontest grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_overview_table extends progcontest_attempts_report_table {
     ...
     * @param progcontest_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($progcontest, $context, $qmsubselect,
            progcontest_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-progcontest-report-overview-report', $progcontest , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the progcontest.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(progcontestaouter.sumgrades) AS grade, COUNT(progcontestaouter.sumgrades) AS numaveraged
                  FROM {progcontest_attempts} progcontestaouter
                  JOIN (
                       SELECT DISTINCT progcontesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON progcontestaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = progcontest_rescale_grade($record->grade, $this->progcontest, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext' => strip_tags(progcontest_report_feedback_for_grade(
                                         $record->grade, $this->progcontest->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{progcontest_attempts} progcontestaouter
                  JOIN (
                       SELECT DISTINCT progcontesta.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON progcontestaouter.id = relevant_attempt_ids.id",
                    'progcontestaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = progcontest_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->progcontest, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = progcontest_format_question_grade($this->progcontest, $record->grade);
        } else {
            $average = progcontest_format_grade($this->progcontest, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/progcontest:regrade', $this->context)) {
            $regradebuttonparams = [
                ...
                'name'  => 'regrade',
                'value' => get_string('regradeselected', 'progcontest_overview'),
                'data-action' => 'toggle',
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = progcontest_rescale_grade($attempt->sumgrades, $this->progcontest);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = progcontest_rescale_grade($newsumgrade, $this->progcontest);
            $oldsumgrade = progcontest_rescale_grade($oldsumgrade, $this->progcontest);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/progcontest/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'progcontest')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/progcontest/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = progcontest_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->progcontest, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = progcontest_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->progcontest, 'question');
            $oldgrade = progcontest_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'progcontest_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'progcontest_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {progcontest_regrades} qqr
                                 WHERE qqr.questionusageid = progcontesta.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/progcontest:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('progcontest_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return progcontest_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
