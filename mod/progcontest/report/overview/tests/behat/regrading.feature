@mod @mod_progcontest @progcontest @progcontest_overview @javascript
Feature: Regrading progcontest attempts using the Grades report
  In order to be able to correct mistakes I made setting up my progcontest
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | progcontest       | Programmingcontest for testing regrading | C1     | progcontest1    |
    And the following "questions" exist:
      | questioncategory | qtype       | name  |
      | Test questions   | truefalse   | TF    |
      | Test questions   | shortanswer | SA    |
    And progcontest "Programmingcontest for testing regrading" contains the following questions:
      | question | page | maxmark |
      | TF       | 1    | 5.0     |
      | SA       | 1    | 5.0     |
    And user "student1" has attempted "Programmingcontest for testing regrading" with responses:
      | slot | response |
      |   1  | True     |
      |   2  | frog     |
    And user "student2" has attempted "Programmingcontest for testing regrading" with responses:
      | slot | response |
      |   1  | True     |
      |   2  | toad     |

  Scenario: Regrade all attempts
    Given I am on the "Programmingcontest for testing regrading" "progcontest activity" page logged in as teacher
    ...
    # Also, nothing has changed in the progcontest, so the regrade won't alter any scores,
    # but this is still a useful test that the regrade process completes without errors.
    Then I should see "Programmingcontest for testing regrading"
    ...
    And I should see "Programmingcontest for testing regrading"
  ...
  Scenario: Dry-run a full regrade, then regrade the attempts that will need it.
    Given I am on the "Programmingcontest for testing regrading" "progcontest activity" page logged in as teacher
    When I navigate to "Edit progcontest" in current page administration
