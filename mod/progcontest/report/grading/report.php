...
/**
 * This file defines the progcontest manual grading report class.
 *
...
require_once($CFG->dirroot . '/mod/progcontest/report/grading/gradingsettings_form.php');


/**
 * Programmingcontest report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_grading_report extends progcontest_default_report {
    ...
    /** @var array from progcontest_report_get_significant_questions. */
    protected $questions;
    ...
    /** @var stdClass the progcontest settings. */
    protected $progcontest;

    /** @var context the progcontest context. */
    protected $context;

    /** @var progcontest_grading_renderer Renderer of Programmingcontest Grading. */
    protected $renderer;
    ...
    public function display($progcontest, $cm, $course) {
        ...
        $pagesize = optional_param('pagesize',
                get_user_preferences('progcontest_grading_pagesize', self::DEFAULT_PAGE_SIZE),
                PARAM_INT);
        $page = optional_param('page', 0, PARAM_INT);
        $order = optional_param('order',
                get_user_preferences('progcontest_grading_order', self::DEFAULT_ORDER),
                PARAM_ALPHA);
        ...
        require_capability('mod/progcontest:grade', $this->context);
        $shownames = has_capability('progcontest/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('progcontest/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this progcontest.
        $this->questions = progcontest_report_get_significant_questions($progcontest);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'progcontest_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    array('mod/progcontest:reviewmyattempts', 'mod/progcontest:attempt'), $this->currentgroup);
        }

        $hasquestions = progcontest_has_questions($this->progcontest->id);
        if (!$hasquestions) {
            $this->print_header_and_tabs($cm, $course, $progcontest, 'grading');
            echo $this->renderer->render_progcontest_no_question_notification($progcontest, $cm, $this->context);
            return true;
        }
        ...
        if (!$counts || $counts->$grade == 0) {
            redirect($this->list_questions_url(), get_string('alldoneredirecting', 'progcontest_grading'));
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "progcontesta.progcontest = :mangrprogcontestid AND
                progcontesta.preview = 0 AND
                progcontesta.state = :statefinished";
        $params = array('mangrprogcontestid' => $this->cm->instance, 'statefinished' => progcontest_attempt::FINISHED);
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND progcontesta.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON progcontesta.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{progcontest_attempts} progcontesta $usersjoin ", 'progcontesta.uniqueid', $where, $params);
    }

    /**
     * Load the progcontest_attempts rows corresponding to a list of question_usage ids.
     *
     * @param int[] $qubaids the question_usage ids of the progcontest_attempts to load.
     * @return array progcontest attempts, with added user name fields.
     */
    protected function load_attempts_by_usage_ids($qubaids) {
        global $DB;

        list($asql, $params) = $DB->get_in_or_equal($qubaids);
        $params[] = progcontest_attempt::FINISHED;
        $params[] = $this->progcontest->id;

        $fields = 'progcontesta.*, u.idnumber, ';
        ...
        $attemptsbyid = $DB->get_records_sql("
                ...
                WHERE progcontesta.uniqueid $asql AND progcontesta.state = ? AND progcontesta.progcontest = ?",
                $params);
    ...
    protected function display_index($includeauto) {
        ...
        $this->print_header_and_tabs($this->cm, $this->course, $this->progcontest, 'grading');
        ...
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'progcontest_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'progcontest_grading');
        }
        echo $this->renderer->render_display_index_heading($linktext, $this->list_questions_url(!$includeauto));
        $data = [];
        $header = [];

        $header[] = get_string('qno', 'progcontest_grading');
        $header[] = get_string('qtypeveryshort', 'question');
        $header[] = get_string('questionname', 'progcontest_grading');
        $header[] = get_string('tograde', 'progcontest_grading');
        $header[] = get_string('alreadygraded', 'progcontest_grading');
        if ($includeauto) {
            $header[] = get_string('automaticallygraded', 'progcontest_grading');
        }
        $header[] = get_string('total', 'progcontest_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new progcontest_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        if ($mform->is_submitted()) {
            if ($mform->is_validated()) {
                ...
                set_user_preference('progcontest_grading_pagesize', $pagesize);
                set_user_preference('progcontest_grading_order', $order);
                ...
                $pagesize = get_user_preferences('progcontest_grading_pagesize', self::DEFAULT_PAGE_SIZE);
            }
        }
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = progcontest_get_review_options($this->progcontest, $attempt, $this->context);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new progcontest_attempt($attempt, $this->progcontest, $this->cm, $this->course);
            ...
            $params = [
                ...
                'other' => [
                    'progcontestid' => $attemptobj->get_progcontestid(),
                    'attemptid' => $attemptobj->get_attemptid(),
                    'slot' => $assumedslotforevents,
                ],
            ];
            $events[] = \mod_progcontest\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON progcontesta.userid = u.id ";
     ...
     */
    public function print_header_and_tabs($cm, $course, $progcontest, $reportmode = 'overview') {
        global $PAGE;
        $this->renderer = $PAGE->get_renderer('progcontest_grading');
        parent::print_header_and_tabs($cm, $course, $progcontest, $reportmode);
    }

    /**
     * Get question heading.
     *
     * @param object $attempt an instance of progcontest_attempt.
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'progcontest_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'progcontest_grading', $a);
