...
/**
 * Defines the renderer for the progcontest_grading module.
 *
...
/**
 * The renderer for the progcontest_grading module.
 *
 * @copyright  2018 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_grading_renderer extends plugin_renderer_base {

    /**
     * Render no question notification.
     *
     * @param object $progcontest The progcontest settings.
     * @param object $cm The course-module for this progcontest.
     * @param object $context The progcontest context.
     * @return string The HTML for the no questions message.
     */
    public function render_progcontest_no_question_notification($progcontest, $cm, $context) {
        return progcontest_no_questions_message($progcontest, $cm, $context);
    }
     ...
     */
    public function render_progcontest_no_grade_question_notification() {
        return $this->notification(get_string('nothingfound', 'progcontest_grading'));
    }
    ...
    public function render_display_index_heading($linktext, $listquestionurl) {
        ...
        $output .= $this->heading(get_string('questionsthatneedgrading', 'progcontest_grading'), 3);
    ...
    public function render_questions_table($includeauto, $data, $header) {
        if (empty($data)) {
            return $this->render_progcontest_no_grade_question_notification();
        }
        $output = '';
    ...
    public function render_grade_link($counts, $type, $gradestring, $gradequestionurl) {
        ...
        if ($counts->$type > 0) {
            $output .= ' ' . html_writer::link(
                            $gradequestionurl,
                            get_string($gradestring, 'progcontest_grading'),
     ...
     * @param progcontest_grading_settings_form $filterform Question filter form.
    ...
    public function render_grading_interface($questioninfo, $listquestionsurl, $filterform, $paginginfo, $pagingbar, $formaction,
        ...
        $output .= $this->heading(get_string('gradingquestionx', 'progcontest_grading', $questioninfo), 3);

        $output .= html_writer::tag('p', html_writer::link($listquestionsurl,
                get_string('backtothelistofquestions', 'progcontest_grading')),
                ['class' => 'mdl-align']);

        $output .= $filterform->render();

        $output .= $this->heading(get_string('gradingattemptsxtoyofz', 'progcontest_grading', $paginginfo), 3);
        ...
        $output .= html_writer::tag('div', html_writer::empty_tag('input', [
                ...
                'value' => get_string('saveandnext', 'progcontest_grading')
        ]), ['class' => 'mdl-align']);
