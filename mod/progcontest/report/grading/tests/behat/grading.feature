@mod @mod_progcontest @progcontest @progcontest_grading
Feature: Basic use of the Manual grading report
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | progcontest       | Programmingcontest 1 | C1     | progcontest1    | 1         | tging    |
    And progcontest "Programmingcontest 1" contains the following questions:
      | question         | page |
      | Short answer 001 | 1    |

  Scenario: Manual grading report without attempts
    When I am on the "Programmingcontest 1" "mod_progcontest > View" page logged in as "teacher1"
    ...
    And I should see "Programmingcontest 1"
  ...
  Scenario: Manual grading report with attempts
    Given user "student1" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      | 1    | Paris    |
  ...
  Scenario: Manual grading settings are remembered as user preferences
    Given user "student1" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      | 1    | Paris    |
    When I am on the "Programmingcontest 1" "mod_progcontest > Manual grading report" page logged in as "teacher1"
    And I follow "Also show questions that have been graded automatically"
    And I click on "update grades" "link" in the "Short answer 001" "table_row"
    ...
    And I press "Change options"
    And I log out
    And I am on the "Programmingcontest 1" "mod_progcontest > Manual grading report" page logged in as "teacher1"
    And I follow "Also show questions that have been graded automatically"
    And I click on "update grades" "link" in the "Short answer 001" "table_row"
  ...
  Scenario: A marker cannot access the report in separate group
    Given I am on the "Programmingcontest 1" "mod_progcontest > View" page logged in as "marker"
    And user "student1" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      | 1    | frog     |
    And user "student2" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      | 1    | Duck     |
    When I navigate to "Results > Manual grading" in current page administration
    Then I should see "Programmingcontest 1"
