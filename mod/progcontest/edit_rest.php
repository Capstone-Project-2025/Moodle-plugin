...
/**
 * Rest endpoint for ajax editing of progcontest structure.
 *
...
$PAGE->set_url('/mod/progcontest/edit-rest.php',
        array('progcontestid' => $progcontestid, 'class' => $class));
...
switch($requestmethod) {
    ...
    case 'GET': // For debugging.
        switch ($class) {
            case 'section':
                $table = 'progcontest_sections';
                $section = $structure->get_section_by_id($id);
                switch ($field) {
                    case 'getsectiontitle':
                        require_capability('mod/progcontest:manage', $modcontext);
            ...
            case 'resource':
                switch ($field) {
                    ...
                    case 'getmaxmark':
                        require_capability('mod/progcontest:manage', $modcontext);
                        $slot = $DB->get_record('progcontest_slots', array('id' => $id), '*', MUST_EXIST);
                        $result = array('instancemaxmark' => progcontest_format_question_grade($progcontest, $slot->maxmark));
                        break;
                    ...
                    case 'updatemaxmark':
                        ...
                        $result = array('instancemaxmark' => progcontest_format_question_grade($progcontest, $maxmark),
                                'newsummarks' => progcontest_format_grade($progcontest, $progcontest->sumgrades));
                        break;
                    ...
                    case 'deletemultiple':
                        ...
                        $ids = explode(',', $ids);
                        foreach ($ids as $id) {
                            $slot = $DB->get_record('progcontest_slots', array('progcontestid' => $progcontest->id, 'id' => $id),
                                    '*', MUST_EXIST);
                            if (progcontest_has_question_use($progcontest, $slot->slot)) {
                        ...
                        $result = array('newsummarks' => progcontest_format_grade($progcontest, $progcontest->sumgrades),
    ...
    case 'DELETE':
        switch ($class) {
            ...
            case 'resource':
                require_capability('mod/progcontest:manage', $modcontext);
                if (!$slot = $DB->get_record('progcontest_slots', array('progcontestid' => $progcontest->id, 'id' => $id))) {
