...
/**
 * Settings form for overrides in the progcontest module.
 *
...
require_once($CFG->dirroot . '/mod/progcontest/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the progcontest settings object. */
     ...
     * @param object the progcontest settings object.
     * @param context the progcontest context.
     ...
     */
    public function __construct($submiturl, $cm, $progcontest, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'progcontest'));

        $progcontestgroupmode = groups_get_activity_groupmode($cm);
        $accessallgroups = ($progcontestgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'progcontest'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/progcontest/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'progcontest', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'progcontest'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if ($accessallgroups) {
                    $users = get_users_by_capability($this->context, 'mod/progcontest:attempt',
                            $userfields, $sort);
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/progcontest/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'progcontest', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'progcontest'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'progcontest');
        $mform->setDefault('password', $this->progcontest->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('progcontestopen', 'progcontest'), mod_progcontest_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->progcontest->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('progcontestclose', 'progcontest'), mod_progcontest_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->progcontest->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'progcontest'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'progcontest');
        $mform->setDefault('timelimit', $this->progcontest->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'progcontest'), $attemptoptions);
        $mform->addHelpButton('attempts', 'attempts', 'progcontest');
        $mform->setDefault('attempts', $this->progcontest->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'progcontest'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'progcontest'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'progcontest'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one progcontest setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $progcontest->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'progcontest');
        }
