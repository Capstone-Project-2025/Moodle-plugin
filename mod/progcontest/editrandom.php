...
// Get the progcontest slot.
$slot = $DB->get_record('progcontest_slots', array('id' => $slotid));
if (!$slot || empty($slot->questioncategoryid)) {
    print_error('invalidrandomslot', 'mod_progcontest');
}

if (!$progcontest = $DB->get_record('progcontest', array('id' => $slot->progcontestid))) {
    print_error('invalidprogcontestid', 'progcontest');
}

$cm = get_coursemodule_from_instance('progcontest', $slot->progcontestid, $progcontest->course);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/progcontest/edit.php', array('cmid' => $cm->id));
}

$url = new moodle_url('/mod/progcontest/editrandom.php', array('slotid' => $slotid));
$PAGE->set_url($url);
...
$mform = new mod_progcontest\form\randomquestion_form(new moodle_url('/mod/progcontest/editrandom.php'),
...
$currentslottags = progcontest_retrieve_slot_tags($slot->id);
...
if ($mform->is_cancelled()) {
    ...
    // We need to save some data into the progcontest_slots table.
    ...
    foreach ($tags as $tag) {
        if ($key = array_search(['tagid' => $tag->id, 'tagname' => $tag->name], $searchableslottags)) {
            // If found, $key would be the id field in the progcontest_slot_tags table.
    ...
    if (!empty($recordstokeep)) {
        ...
        $DB->delete_records_select('progcontest_slot_tags', "slotid = ? AND id $select", $params);
    } else {
...
echo $OUTPUT->header();
$heading = get_string('randomediting', 'mod_progcontest');
echo $OUTPUT->heading_with_help($heading, 'randomquestion', 'mod_progcontest');
