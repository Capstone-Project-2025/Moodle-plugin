...
/**
 * Fallback page of /mod/progcontest/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $progcontest, $pagevars) =
        question_edit_setup('editq', '/mod/progcontest/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/progcontest/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new progcontest_add_random_form(new moodle_url('/mod/progcontest/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    progcontest_add_random_questions($progcontest, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids);
...
// Setup $PAGE.
$streditingprogcontest = get_string('editinga', 'moodle', get_string('modulename', 'progcontest'));
$PAGE->navbar->add($streditingprogcontest);
$PAGE->set_title($streditingprogcontest);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$progcontestname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoprogcontest', 'progcontest', $progcontestname), 2);
$mform->display();
echo $OUTPUT->footer();
