...
/**
 * This script controls the display of the progcontest reports.
 *
...
$url = new moodle_url('/mod/progcontest/report.php', array('id' => $cm->id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
$reportlist = progcontest_report_list($context);
if (empty($reportlist)) {
    print_error('erroraccessingreport', 'progcontest');
}
...
if (!is_readable("report/$mode/report.php")) {
    print_error('reportnotfound', 'progcontest', '', $mode);
}

// Open the selected progcontest report and display it.
$file = $CFG->dirroot . '/mod/progcontest/report/' . $mode . '/report.php';
if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'progcontest_' . $mode . '_report';
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'progcontest');
}

$report = new $reportclassname();
$report->display($progcontest, $cm, $course);
