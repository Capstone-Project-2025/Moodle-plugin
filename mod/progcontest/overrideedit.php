...
/**
 * This page handles editing and creation of progcontest overrides
 *
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('progcontest_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'progcontest');
    }
    if (! $progcontest = $DB->get_record('progcontest', array('id' => $override->progcontest))) {
...
$url = new moodle_url('/mod/progcontest/overrideedit.php');
if ($action) {
...
require_capability('mod/progcontest:manageoverrides', $context);
...
// Merge progcontest defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $progcontest->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/progcontest/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new progcontest_override_form($url, $cm, $progcontest, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->progcontest = $progcontest->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $progcontest->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('progcontest_overrides', $conditions)) {
            ...
            // Set the course module id before calling progcontest_delete_override().
            $progcontest->cmid = $cm->id;
            progcontest_delete_override($progcontest, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('progcontest_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->progcontest}_g_{$fromform->groupid}" : "{$fromform->progcontest}_u_{$fromform->userid}";
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('progcontest_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->progcontest}_g_{$fromform->groupid}" : "{$fromform->progcontest}_u_{$fromform->userid}";
        cache::make('mod_progcontest', 'overrides')->delete($cachekey);
    ...
    if ($groupmode) {
        ...
        progcontest_update_events($progcontest, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'progcontest');
$PAGE->navbar->add($pagetitle);
$PAGE->set_pagelayout('admin');
$PAGE->set_title($pagetitle);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($progcontest->name, true, array('context' => $context)));
