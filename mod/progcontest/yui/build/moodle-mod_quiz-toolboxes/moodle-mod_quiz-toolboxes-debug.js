YUI.add('moodle-mod_progcontest-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW: 'a.' + CSS.SHOW,
        SLOTLI: 'li.slot',
        SUMMARKS: '.mod_progcontest_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.progcontestid = this.get('progcontestid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'progcontest', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a progcontest in editing mode.
 *
 * @module mod_progcontest-resource-toolbox
 * @namespace M.mod_progcontest.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_progcontest.progcontestbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'progcontest', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_progcontest.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_progcontest.util.slot.remove(element);
    ...
    delete_multiple_action: function(ev) {
        ...
        if (typeof problemsection !== 'undefined') {
            var alert = new M.core.alert({
                title: M.util.get_string('cannotremoveslots', 'progcontest'),
                message: M.util.get_string('cannotremoveallsectionslots', 'progcontest', problemsection)
            });
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Moodle.mod_progcontest.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Moodle.mod_progcontest.util.slot.getId(slot);
            slots.push(slot);
        });
        var element = Y.one('div.mod-progcontest-edit-content');
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('areyousureremoveselected', 'progcontest'),
            modal: true
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Moodle.mod_progcontest.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Moodle.mod_progcontest.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_progcontest.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_progcontest.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_progcontest.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_progcontest.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_progcontest.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_progcontest.util.slot.reorderSlots();
        Y.Moodle.mod_progcontest.util.slot.reorderPageBreaks();
        Y.Moodle.mod_progcontest.util.page.reorderPages();
        Y.Moodle.mod_progcontest.util.slot.updateOneSlotSections();
        Y.Moodle.mod_progcontest.util.slot.updateAllDependencyIcons();
    },

    NAME: 'mod_progcontest-resource-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_progcontest.resource_toolbox = null;
M.mod_progcontest.init_resource_toolbox = function(config) {
    M.mod_progcontest.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_progcontest.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_progcontest-toolboxes
 * @namespace M.mod_progcontest.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a progcontest.
 *
 * @class section
 * @constructor
 * @extends M.mod_progcontest.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'progcontest', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'progcontest', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'progcontest', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'progcontest', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'progcontest', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_progcontest-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_progcontest.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_progcontest-progcontestbase",
        "moodle-mod_progcontest-util-slot",
