...
/**
 * Defines the \mod_progcontest\structure class.
 *
...
/**
 * Programmingcontest structure class.
 *
 * The structure of the progcontest. That is, which questions it is built up
 * from. This is used on the Edit progcontest page (edit.php) and also when
 * starting an attempt at the progcontest (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \progcontest the progcontest this is the structure of. */
    protected $progcontestobj = null;

    /**
     * @var \stdClass[] the questions in this progcontest. Contains the row from the questions
     * table, with the data from the progcontest_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] progcontest_slots.slot => the progcontest_slots rows for this progcontest, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * progcontest_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty progcontest.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given progcontest.
     * @param \progcontest $progcontestobj the progcontest.
     * @return structure
     */
    public static function create_for_progcontest($progcontestobj) {
        $structure = self::create();
        $structure->progcontestobj = $progcontestobj;
        $structure->populate_structure($progcontestobj->get_progcontest());
        return $structure;
    }

    /**
     * Whether there are any questions in the progcontest.
     * @return bool true if there is at least one question in the progcontest.
     */
    ...
    /**
     * Get the number of questions in the progcontest.
     * @return int the number of questions in the progcontest.
     */
     ...
     * question_category.contextid, and the progcontest_slots data for the question in this progcontest.
     */
     ...
     * questions cannot be depended upon because of progcontest options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->progcontestobj->get_navigation_method() == progcontest_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->progcontestobj->get_progcontest()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_progcontest', $this->progcontestobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->progcontestobj->get_progcontest()->preferredbehaviour);
    ...
    /**
     * Get the course id that the progcontest belongs to.
     * @return int the course.id for the progcontest.
     */
    ...
    /**
     * Get the course module id of the progcontest.
     * @return int the course_modules.id for the progcontest.
     */
    ...
    /**
     * Get id of the progcontest.
     * @return int the progcontest.id for the progcontest.
     */
     ...
     * @return \stdClass the progcontest settings row from the database.
     */
    ...
    /**
     * Programmingcontestzes can only be repaginated if they have not been attempted, the
     * questions are not shuffled, and there are two or more questions.
     * @return bool whether this progcontest can be repaginated.
     */
    ...
    /**
     * Programmingcontestzes can only be edited if they have not been attempted.
     * @return bool whether the progcontest can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !progcontest_has_attempts($this->progcontestobj->get_progcontestid());
        }
        return $this->canbeedited;
    }

    /**
     * This progcontest can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = progcontest_attempt_summary_link_to_reports($this->get_progcontest(),
                    $this->progcontestobj->get_cm(), $this->progcontestobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'progcontest',
                    new \moodle_url('/mod/progcontest/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the progcontest.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the progcontest.
     * @return int the number of questions that should be on each page of the
     * progcontest by default.
     */
    public function get_questions_per_page() {
        return $this->progcontestobj->get_progcontest()->questionsperpage;
    }

    /**
     * Get progcontest slots.
     * @return \stdClass[] the slots in this progcontest.
     */
    ...
    /**
     * Is this slot the last one in the progcontest?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the progcontest.
     */
    public function is_last_slot_in_progcontest($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the progcontest?
     * @param \stdClass $section the progcontest_sections row.
     * @return bool whether this is first section in the progcontest.
     */
    ...
    /**
     * Is this the last section in the progcontest?
     * @param \stdClass $section the progcontest_sections row.
     * @return bool whether this is first section in the progcontest.
     */
    ...
    /**
     * Get the final slot in the progcontest.
     * @return \stdClass the progcontest_slots for for the final slot in the progcontest.
     */
     ...
     * @return \stdClass the requested progcontest_slots row.
    ...
    /**
     * Get all the slots in a section of the progcontest.
    ...
    /**
     * Get all the sections of the progcontest.
     * @return \stdClass[] the sections in this progcontest.
     */
    ...
    /**
     * Get the overall progcontest grade formatted for display.
     * @return string the maximum grade for this progcontest.
     */
    public function formatted_progcontest_grade() {
        return progcontest_format_grade($this->get_progcontest(), $this->get_progcontest()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return progcontest_format_question_grade($this->get_progcontest(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall progcontest grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_progcontest()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return progcontest_get_grade_format($this->get_progcontest());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (progcontest_has_attempts($this->progcontestobj->get_progcontestid())) {
            $reviewlink = progcontest_attempt_summary_link_to_reports($this->progcontestobj->get_progcontest(),
                    $this->progcontestobj->get_cm(), $this->progcontestobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'progcontest', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the progcontest.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $progcontest = $this->progcontestobj->get_progcontest();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($progcontest->timeopen > 0) {
            if ($timenow > $progcontest->timeopen) {
                $dates[] = get_string('progcontestopenedon', 'progcontest', userdate($progcontest->timeopen));
            } else {
                $dates[] = get_string('progcontestwillopen', 'progcontest', userdate($progcontest->timeopen));
            }
        }
        if ($progcontest->timeclose > 0) {
            if ($timenow > $progcontest->timeclose) {
                $dates[] = get_string('progcontestclosed', 'progcontest', userdate($progcontest->timeclose));
            } else {
                $dates[] = get_string('progcontestcloseson', 'progcontest', userdate($progcontest->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'progcontest');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $progcontest->timeopen) {
            $currentstatus = get_string('progcontestisclosedwillopen', 'progcontest',
                    userdate($progcontest->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($progcontest->timeclose && $timenow <= $progcontest->timeclose) {
            $currentstatus = get_string('progcontestisopenwillclose', 'progcontest',
                    userdate($progcontest->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($progcontest->timeclose && $timenow > $progcontest->timeclose) {
            $currentstatus = get_string('progcontestisclosed', 'progcontest');
        } else {
            $currentstatus = get_string('progcontestisopen', 'progcontest');
        }
    ...
    /**
     * Set up this class with the structure for a given progcontest.
     * @param \stdClass $progcontest the progcontest settings.
     */
    public function populate_structure($progcontest) {
        global $DB;
        ...
        // Get progcontest sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('progcontest_sections', array('progcontestid' => $progcontest->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the progcontest.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'progcontest');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slotsinorder as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'progcontest');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_progcontest($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_progcontest($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_progcontest($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('progcontest_slots', 'slot', $slotreorder,
                    array('progcontestid' => $this->get_progcontestid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('progcontest_slots', 'page', $page,
                    array('id' => $movingslot->id));
        }

        // Update section fist slots.
        progcontest_update_section_firstslots($this->get_progcontestid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE progcontestid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {progcontest_slots} WHERE progcontestid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_progcontestid(), $this->get_progcontestid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {progcontest_slots}
                       SET page = page - 1
                     WHERE progcontestid = ?
                       AND page > ?
                    ", array($this->get_progcontestid(), $page));
        }
    ...
    /**
     * Refresh page numbering of progcontest slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('progcontest_slots', array('progcontestid' => $this->get_progcontestid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of progcontest slots and save to the database.
     * @param \stdClass $progcontest the progcontest object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('progcontest_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    /**
     * Remove a slot from a progcontest
     *
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('progcontest_slots', array('progcontestid' => $this->get_progcontestid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {progcontest_slots} WHERE progcontestid = ?', array($this->get_progcontestid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('progcontest_slot_tags', array('slotid' => $slot->id));
        $DB->delete_records('progcontest_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('progcontest_slots', 'slot', $i - 1,
                    array('progcontestid' => $this->get_progcontestid(), 'slot' => $i));
        ...
        progcontest_update_section_firstslots($this->get_progcontestid(), -1, $slotnumber);
     ...
     *
     * Saves changes to the question grades in the progcontest_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the progcontest table.
     *
     * @param \stdClass $slot row from the progcontest_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('progcontest_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_progcontest($slot->progcontestid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('progcontest_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the progcontest_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $progcontestslots = $DB->get_records('progcontest_slots', array('progcontestid' => $this->get_progcontestid()), 'slot');
        $repaginate = new \mod_progcontest\repaginate($this->get_progcontestid(), $progcontestslots);
        $repaginate->repaginate_slots($progcontestslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'progcontest');
        }
        $section->progcontestid = $this->get_progcontestid();
        $slotsonpage = $DB->get_records('progcontest_slots', array('progcontestid' => $this->get_progcontestid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('progcontest_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('progcontest_sections', array('id' => $id), '*', MUST_EXIST);
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('progcontest_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a progcontest.');
        }
        $DB->delete_records('progcontest_sections', array('id' => $sectionid));
    }
    ...
    protected function populate_slot_tags() {
        ...
        $this->slottags = progcontest_retrieve_tags_for_slot_ids($slotids);
    }
    ...
    /**
     * Whether the current user can add random questions to the progcontest or not.
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            $progcontestcontext = $this->progcontestobj->get_context();
            $relatedcontexts = new \question_edit_contexts($progcontestcontext);
