...
/**
 * Cache data source for the progcontest overrides.
 *
...
namespace mod_progcontest\cache;

use cache_definition;

/**
 * Class progcontest_overrides
 *
 * @package   mod_progcontest
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class overrides implements \cache_data_source {
    ...
    public function load_for_cache($key) {
        ...
        [$progcontestid, $ug, $ugid] = explode('_', $key);
        $progcontestid = (int) $progcontestid;

        switch ($ug) {
            case 'u':
                $userid = (int) $ugid;
                $override = $DB->get_record(
                    'progcontest_overrides',
                    ['progcontest' => $progcontestid, 'userid' => $userid],
            ...
            case 'g':
                ...
                $override = $DB->get_record(
                    'progcontest_overrides',
                    ['progcontest' => $progcontestid, 'groupid' => $groupid],
                    'timeopen, timeclose, timelimit, attempts, password'
                );
                break;
            default:
