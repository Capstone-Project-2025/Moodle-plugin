...
/**
 * Renderer outputting the progcontest editing UI.
 *
...
class edit_renderer extends \plugin_renderer_base {
    ...
    protected $togglegroup = 'progcontest-questions';

    /**
     * Render the edit page
     *
     * @param \progcontest $progcontestobj object containing all the progcontest settings information.
     * @param structure $structure object containing the structure of the progcontest.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\progcontest $progcontestobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingprogcontestx', 'progcontest',
                format_string($progcontestobj->get_progcontest_name())), 'editingprogcontest', 'progcontest', '',
                get_string('basicideasofprogcontest', 'progcontest'), 2);

        // Information at the top.
        $output .= $this->progcontest_state_warnings($structure);

        $output .= html_writer::start_div('mod_progcontest-edit-top-controls');
        ...
        $output .= $this->progcontest_information($structure);
        $output .= html_writer::end_tag('div');
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex justify-content-between flex-wrap mb-1');
        $output .= html_writer::start_div('mod_progcontest-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex flex-column justify-content-around');
        $output .= $this->total_marks($progcontestobj->get_progcontest());
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_progcontest/progcontestquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_progcontest/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the progcontest,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the progcontest structure.
     * @return string HTML to output.
     */
    public function progcontest_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function progcontest_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'progcontest', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'progcontestopeningstatus',
    ...
    /**
     * Render the form for setting a progcontest' overall grade
     *
     * @param structure $structure the progcontest structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'progcontestsavegradesform form-inline'));
        ...
        $output .= html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_progcontest_grade(),
                'class' => 'form-control'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'class' => 'btn btn-secondary ml-1',
                'name' => 'savechanges', 'value' => get_string('save', 'progcontest')));
     ...
     * @param structure $structure the structure of the progcontest being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        $header = html_writer::tag('span', get_string('repaginatecommand', 'progcontest'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'progcontest'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->js_call_amd('mod_progcontest/repaginate', 'init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = array(
            ...
            'value' => get_string('selectmultipleitems', 'progcontest'),
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'progcontest'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = array(
            ...
            'value' => get_string('deleteselected', 'mod_progcontest'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_progcontest'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = array(
            ...
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'progcontest'),
        );
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'progcontest');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'progcontest', $select) .
    ...
    /**
     * Render the total marks available for the progcontest.
     *
     * @param \stdClass $progcontest the progcontest settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($progcontest) {
        $totalmark = html_writer::span(progcontest_format_grade($progcontest, $progcontest->sumgrades), 'mod_progcontest_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'progcontest', $totalmark),
     ...
     *
     * @param structure $structure the structure of the progcontest being edited.
     * @param \stdClass $section The progcontest_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'progcontest', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the progcontest.
     * @param \stdClass $section data from the progcontest_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'progcontest');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'progcontest'),
    ...
    /**
     * Render an icon to remove a section from the progcontest.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'progcontest', $section->heading);
        $url = new \moodle_url('/mod/progcontest/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the progcontest.
     *
     * This function calls {@link core_course_renderer::progcontest_section_question()}
     *
     * @param structure $structure object containing the structure of the progcontest.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_progcontest($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-progcontest-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'progcontest'), array('class' => 'add-menu'));
        ...
        // Disable the link if progcontest has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'progcontest');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = array();

        // Add a new question to the progcontest.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'progcontest', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'progcontest');
        }
        ...
        if ($structure->can_add_random_questions()) {
            $returnurl = new \moodle_url('/mod/progcontest/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
            $params = ['returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion'];
            $url = new \moodle_url('/mod/progcontest/addrandom.php', $params);
            ...
            if ($page) {
                $title = get_string('addrandomquestiontopage', 'progcontest', $page);
            } else {
                $title = get_string('addrandomquestionatend', 'progcontest');
            }
    ...
    /**
     * Render the form that contains the data for adding a new question to the progcontest.
     *
     * @param structure $structure object containing the structure of the progcontest.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_progcontest(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $progcontest the progcontest settings from the database.
     * @param \stdClass $question data from the question and progcontest_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($progcontest, $question, $label = null, $variant = null) {
        $url = progcontest_question_preview_url($progcontest, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'progcontest');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'progcontest');
    ...
    /**
     * Render an icon to remove a question from the progcontest.
     *
     * @param structure $structure object containing the structure of the progcontest.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the progcontest.
     *
     * @param structure $structure object containing the structure of the progcontest.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('progcontestid' => $structure->get_progcontestid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'progcontest');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'progcontest');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'progcontest', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'progcontest'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'progcontest', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'progcontest'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a progcontest edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = progcontest_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'progcontest').' '.$title));
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/progcontest/editrandom.php',
                array('returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id));

        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = progcontest_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'progcontest');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'progcontest'), array('class' => 'mod_progcontest_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'progcontest')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'progcontest'),
                )
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_progcontest/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_progcontest\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/progcontest/edit_rest.php';
        $config->sectionurl = '/mod/progcontest/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_progcontest-toolboxes',
                'M.mod_progcontest.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'progcontestid' => $structure->get_progcontestid(),
        ...
        $this->page->requires->strings_for_js(array('areyousureremoveselected'), 'progcontest');
        $this->page->requires->yui_module('moodle-mod_progcontest-toolboxes',
                'M.mod_progcontest.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'progcontestid' => $structure->get_progcontestid(),
        ...
        $this->page->requires->yui_module('moodle-mod_progcontest-dragdrop', 'M.mod_progcontest.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'progcontestid' => $structure->get_progcontestid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_progcontest-dragdrop', 'M.mod_progcontest.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'progcontestid' => $structure->get_progcontestid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'progcontest', '');
     ...
     *
     * @param \mod_progcontest\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_progcontest\question\bank\custom_view $questionbank, array $pagevars) {
