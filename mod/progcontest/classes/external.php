...
/**
 * Programmingcontest external API
 *
...
/**
 * Programmingcontest external functions
 *
 * @package    mod_progcontest
 * @category   external
 * @copyright  2016 Juan Leyva <juan@moodle.com>
 ...
 * @since      Moodle 3.1
 */
class mod_progcontest_external extends external_api {

    /**
     * Describes the parameters for get_progcontestzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_progcontestzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of progcontestzes in a provided list of courses,
     * if no list is provided all progcontestzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of progcontestzes details
     * @since Moodle 3.1
     */
    public static function get_progcontestzes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedprogcontestzes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_progcontestzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the progcontestzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $progcontestzes = get_all_instances_in_courses("progcontest", $courses);
            foreach ($progcontestzes as $progcontest) {
                $context = context_module::instance($progcontest->coursemodule);

                // Update progcontest with override information.
                $progcontest = progcontest_update_effective_access($progcontest, $USER->id);

                // Entry to return.
                $progcontestdetails = array();
                // First, we return information that any user can see in the web interface.
                $progcontestdetails['id'] = $progcontest->id;
                $progcontestdetails['coursemodule']      = $progcontest->coursemodule;
                $progcontestdetails['course']            = $progcontest->course;
                $progcontestdetails['name']              = external_format_string($progcontest->name, $context->id);

                if (has_capability('mod/progcontest:view', $context)) {
                    // Format intro.
                    $options = array('noclean' => true);
                    list($progcontestdetails['intro'], $progcontestdetails['introformat']) =
                        external_format_text($progcontest->intro, $progcontest->introformat, $context->id, 'mod_progcontest', 'intro', null, $options);

                    $progcontestdetails['introfiles'] = external_util::get_area_files($context->id, 'mod_progcontest', 'intro', false, false);
                    ...
                    $hasfeedback = progcontest_has_feedback($progcontest);
                    $progcontestdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $progcontestobj = progcontest::create($progcontest->id, $USER->id);
                    $accessmanager = new progcontest_access_manager($progcontestobj, $timenow, has_capability('mod/progcontest:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the progcontest.
                    if (!$accessmanager->prevent_access()) {
                        $progcontestdetails['hasquestions'] = (int) $progcontestobj->has_questions();
                        $progcontestdetails['autosaveperiod'] = get_config('progcontest', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $progcontestdetails[$field] = $progcontest->{$field};
                    }
                }
                $returnedprogcontestzes[] = $progcontestdetails;
            }
        }
        $result = array();
        $result['progcontestzes'] = $returnedprogcontestzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_progcontestzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_progcontestzes_by_courses_returns() {
        return new external_single_structure(
            array(
                'progcontestzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this progcontest is part of.'),
                            'coursemodule' => new external_value(PARAM_INT, 'Course module id.'),
                            'name' => new external_value(PARAM_RAW, 'Programmingcontest name.'),
                            'intro' => new external_value(PARAM_RAW, 'Programmingcontest introduction text.', VALUE_OPTIONAL),
                            'introformat' => new external_format_value('intro', VALUE_OPTIONAL),
                            'introfiles' => new external_files('Files in the introduction text', VALUE_OPTIONAL),
                            'timeopen' => new external_value(PARAM_INT, 'The time when this progcontest opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this progcontest closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for progcontest attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a progcontest attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values progcontest_GRADEHIGHEST, progcontest_GRADEAVERAGE,
                                                                    progcontest_ATTEMPTFIRST or progcontest_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their progcontest
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_progcontest_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their progcontest
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their progcontest attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            progcontest attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            progcontest attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their progcontest
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their progcontest
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the progcontest, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the progcontest. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the progcontest overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the progcontest was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a progcontest attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this progcontest can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark progcontest complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the progcontest to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the progcontest has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the progcontest has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a progcontest.
     *
     * @param int $progcontestid progcontest instance id
     * @return array array containing the progcontest, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_progcontest($progcontestid) {
        global $DB;

        // Request and permission validation.
        $progcontest = $DB->get_record('progcontest', array('id' => $progcontestid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($progcontest, 'progcontest');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($progcontest, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_progcontest.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_progcontest_parameters() {
        return new external_function_parameters (
            array(
                'progcontestid' => new external_value(PARAM_INT, 'progcontest instance id'),
            )
        );
    }
     ...
     */
    public static function view_progcontest($progcontestid) {
        global $DB;

        $params = self::validate_parameters(self::view_progcontest_parameters(), array('progcontestid' => $progcontestid));
        $warnings = array();

        list($progcontest, $course, $cm, $context) = self::validate_progcontest($params['progcontestid']);
    ...
    /**
     * Describes the view_progcontest return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_progcontest_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'progcontest status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given progcontest and user.
     *
     * @param int $progcontestid progcontest instance id
     * @param int $userid user id
     * @param string $status progcontest status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($progcontestid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;

        $warnings = array();

        $params = array(
            'progcontestid' => $progcontestid,
            'userid' => $userid,
            'status' => $status,
        ...
        // Update progcontest with override information.
        $progcontest = progcontest_update_effective_access($progcontest, $params['userid']);
        $attempts = progcontest_get_user_attempts($progcontest->id, $user->id, $params['status'], $params['includepreviews']);
        $attemptresponse = [];
        foreach ($attempts as $attempt) {
            $reviewoptions = progcontest_get_review_options($progcontest, $attempt, $context);
            if (!has_capability('mod/progcontest:viewreports', $context) &&
                    ($reviewoptions->marks < question_display_options::MARK_AND_MAX || $attempt->state != progcontest_attempt::FINISHED)) {
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'progcontest' => new external_value(PARAM_INT, 'Foreign key reference to the progcontest that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this progcontest.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this progcontest
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time progcontest cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a progcontest.
     *
     * @param int $progcontestid progcontest instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($progcontestid, $userid = 0) {
        global $DB, $USER, $CFG;
        ...
        // This code was mostly copied from mod/progcontest/view.php. We need to make the web service logic consistent.
        // Get this user's attempts.
        $attempts = progcontest_get_user_attempts($progcontest->id, $user->id, 'all');
        $canviewgrade = false;
        if ($attempts) {
            if ($USER->id != $user->id) {
                // No need to check the permission here. We did it at by require_capability('mod/progcontest:viewreports', $context).
                $canviewgrade = true;
            } else {
                // Work out which columns we need, taking account what data is available in each attempt.
                [$notused, $alloptions] = progcontest_get_combined_reviewoptions($progcontest, $attempts);
                $canviewgrade = $alloptions->marks >= question_display_options::MARK_AND_MAX;
            }
        }

        $grade = $canviewgrade ? progcontest_get_best_grade($progcontest, $user->id) : null;
        ...
        $gradinginfo = grade_get_grades($course->id, 'mod', 'progcontest', $progcontest->id, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given progcontest.'),
                'grade' => new external_value(PARAM_FLOAT, 'The grade (only if the user has a grade).', VALUE_OPTIONAL),
                'gradetopass' => new external_value(PARAM_FLOAT, 'The grade to pass the progcontest (only if set).', VALUE_OPTIONAL),
                'warnings' => new external_warnings(),
            )
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($progcontestid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = progcontest_get_user_attempts($progcontest->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a progcontest.
     *
     * @param int $progcontestid progcontest instance id
     ...
     */
    public static function start_attempt($progcontestid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$progcontestobj->has_questions()) {
            throw new moodle_progcontest_exception($progcontestobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            progcontest_validate_new_attempt($progcontestobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$progcontestobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_progcontest_exception($progcontestobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == progcontest_attempt::OVERDUE) {
                    throw new moodle_progcontest_exception($progcontestobj, 'stateoverdue');
                } else {
                    throw new moodle_progcontest_exception($progcontestobj, 'attemptstillinprogress');
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = progcontest_prepare_and_start_new_attempt($progcontestobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = progcontest_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == progcontest_attempt::OVERDUE) {
            throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'stateoverdue');
        }

        // User submitted data (like the progcontest password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'noquestionsfound');
            }
        }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the progcontest display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the progcontest this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the progcontest', VALUE_OPTIONAL),
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the progcontest display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the progcontest display settings.'
        );
    }
     ...
     *
     * @param  progcontest_attempt  $attemptobj  the progcontest attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(progcontest_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_progcontestobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_progcontest');
        $contextid = $attemptobj->get_progcontestobj()->get_context()->id;
    ...
    /**
     * Returns information for the given attempt page for a progcontest attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_progcontest_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/progcontest:preview capability,
    ...
    /**
     * Returns a summary of a progcontest attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the progcontest. This function is intended for the progcontest auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a progcontest and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false, $preflightdata = array()) {
        ...
        $attemptobj = progcontest_attempt::create($params['attemptid']);
        $failifoverdue = !($attemptobj->get_progcontestobj()->get_progcontest()->overduehandling == 'graceperiod');
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'noreview', null, '',
                    $attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new moodle_progcontest_exception($attemptobj->get_progcontestobj(), 'noreviewattempt');
        }
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_progcontestobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = progcontest_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_progcontest(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'progcontest'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the progcontest (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_progcontest_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'progcontestid' => new external_value(PARAM_INT, 'progcontest instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given progcontest.
     *
     * @param int $progcontestid progcontest instance id
     ...
     * @throws moodle_exception
     */
    public static function get_progcontest_feedback_for_grade($progcontestid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_progcontest_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = progcontest_feedback_record_for_grade($params['grade'], $progcontest);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_progcontest', 'feedback', $feedback->id);
            $result['feedbacktext'] = $text;
            $result['feedbacktextformat'] = $format;
            $feedbackinlinefiles = external_util::get_area_files($context->id, 'mod_progcontest', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_progcontest_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_progcontest_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_progcontest_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_progcontest_access_information_parameters() {
        return new external_function_parameters (
            array(
                'progcontestid' => new external_value(PARAM_INT, 'progcontest instance id')
            )
    ...
    /**
     * Return access information for a given progcontest.
     *
     * @param int $progcontestid progcontest instance id
     ...
     * @throws  moodle_progcontest_exception
     */
    public static function get_progcontest_access_information($progcontestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_progcontest_access_information_parameters(), $params);

        list($progcontest, $course, $cm, $context) = self::validate_progcontest($params['progcontestid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/progcontest:attempt', $context);;
        $result['canmanage'] = has_capability('mod/progcontest:manage', $context);;
        $result['canpreview'] = has_capability('mod/progcontest:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/progcontest:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/progcontest:viewreports', $context);;

        // Access manager now.
        $progcontestobj = progcontest::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/progcontest:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new progcontest_access_manager($progcontestobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_progcontest_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_progcontest_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the progcontest or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the progcontest settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the progcontest or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the progcontest reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a progcontest.
     *
     * @param int $progcontestid progcontest instance id
     ...
     * @throws  moodle_progcontest_exception
     */
    public static function get_attempt_access_information($progcontestid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = progcontest_get_user_attempt_unfinished($progcontest->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != progcontest_attempt::IN_PROGRESS and $unfinishedattempt->state != progcontest_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_progcontest_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_progcontest_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'progcontestid' => new external_value(PARAM_INT, 'progcontest instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given progcontest.
     ...
     * @throws  moodle_progcontest_exception
     */
    public static function get_progcontest_required_qtypes($progcontestid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_progcontest_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $progcontestobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_progcontest_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_progcontest_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the progcontest'),
                'warnings' => new external_warnings(),
            )
        );
    }
