...
/**
 * Defines the progcontest repaginate class.
 *
 ...
 *
 * The progcontest setting allows users to write progcontestzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the progcontest being manipulated. */
    private $progcontestid;
    /** @var array the progcontest_slots for that progcontest. */
    private $slots;

    /**
     * Constructor.
     * @param int $progcontestid the id of the progcontest being manipulated.
     * @param stdClass[] $slots the progcontest_slots for that progcontest.
     */
    public function __construct($progcontestid = 0, $slots = null) {
        global $DB;
        $this->progcontestid = $progcontestid;
        if (!$this->progcontestid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('progcontest_slots', array('progcontestid' => $this->progcontestid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('progcontest_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $progcontestslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($progcontestslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$progcontestslots) {
            return null;
        }
        $newslots = array();
        foreach ($progcontestslots as $slot) {
            if ($type == self::LINK) {
