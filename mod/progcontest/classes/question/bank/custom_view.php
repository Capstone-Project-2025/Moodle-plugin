...
/**
 * Defines the custom question bank view used on the Edit progcontest page.
 *
...
namespace mod_progcontest\question\bank;

use coding_exception;

/**
 * Subclass to customise the view of the question bank for the progcontest editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the progcontest this is used by has been attemptd. */
    protected $progcontesthasattempts = false;
    /** @var \stdClass the progcontest settings. */
    protected $progcontest = false;
     ...
     * @param \stdClass $progcontest progcontest settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $progcontest) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->progcontestquestionbankcolumns)) {
            $progcontestquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $progcontestquestionbankcolumns = explode(',', $CFG->progcontestquestionbankcolumns);
        }

        foreach ($progcontestquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_progcontest\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_progcontest\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_progcontest\\question\\bank\\' .
                            $fullname, DEBUG_DEVELOPER);
                    $fullname = 'question_bank_' . $fullname;
                } else {
                    throw new coding_exception('Invalid progcontest question bank column', $fullname);
                }
            }
    ...
    protected function heading_column() {
        return 'mod_progcontest\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_progcontest\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the progcontest has been attempted,
     * hence whether some bits of UI, like the add this question to the progcontest icon,
     * should be displayed.
     * @param bool $progcontesthasattempts whether the progcontest has attempts.
     */
    public function set_progcontest_has_attempts($progcontesthasattempts) {
        $this->progcontesthasattempts = $progcontesthasattempts;
        if ($progcontesthasattempts && isset($this->visiblecolumns['addtoprogcontestaction'])) {
            unset($this->visiblecolumns['addtoprogcontestaction']);
        }
    }

    public function preview_question_url($question) {
        return progcontest_question_preview_url($this->progcontest, $question);
    }

    public function add_to_progcontest_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/progcontest/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->progcontesthasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the progcontest.
            $params = array(
                'type' => 'submit',
                'name' => 'add',
                'class' => 'btn btn-primary',
                'value' => get_string('addselectedquestionstoprogcontest', 'progcontest'),
                'data-action' => 'toggle',
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/progcontest/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'progcontest');
