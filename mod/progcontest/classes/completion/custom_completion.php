...
namespace mod_progcontest\completion;
...
use grade_item;
use progcontest;
use progcontest_access_manager;

/**
 * Activity custom completion subclass for the progcontest activity.
 *
 * Class for defining mod_progcontest's custom completion rules and fetching the completion statuses
 * of the custom completion rules for a given progcontest instance and a user.
 *
...
class custom_completion extends activity_custom_completion {
    ...
    protected function check_passing_grade_or_all_attempts(): bool {
        ...
        $attempts = progcontest_get_user_attempts($this->cm->instance, $this->userid, 'finished', true);
        if (!$attempts) {
            return false;
        }
        $lastfinishedattempt = end($attempts);
        $context = context_module::instance($this->cm->id);
        $progcontestobj = progcontest::create($this->cm->instance, $this->userid);
        $accessmanager = new progcontest_access_manager(
            $progcontestobj,
            time(),
            has_capability('mod/progcontest:ignoretimelimits', $context, $this->userid, false)
        );
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        $completionpassorattempts = $this->cm->customdata['customcompletionrules']['completionpassorattemptsexhausted'] ?? [];
        if (!empty($completionpassorattempts['completionattemptsexhausted'])) {
            $passorallattemptslabel = get_string('completiondetail:passorexhaust', 'mod_progcontest');
        } else {
            $passorallattemptslabel = get_string('completiondetail:passgrade', 'mod_progcontest');
        }

        return [
            'completionpassorattemptsexhausted' => $passorallattemptslabel,
            'completionminattempts' => get_string('completiondetail:minattempts', 'mod_progcontest', $minattempts),
        ];
    }
