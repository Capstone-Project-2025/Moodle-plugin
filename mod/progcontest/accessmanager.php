...
/**
 * Classes to enforce the various access rules that can apply to a progcontest.
 *
 ...
 * progcontest, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class progcontest_access_manager {
    /** @var progcontest the progcontest settings object. */
    protected $progcontestobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of progcontest_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular progcontest.
     * @param object $progcontestobj An instance of the class progcontest from attemptlib.php.
     *      The progcontest we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/progcontest:ignoretimelimits', ...)).
     */
    public function __construct($progcontestobj, $timenow, $canignoretimelimits) {
        $this->progcontestobj = $progcontestobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($progcontestobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular progcontest.
     ...
     * @return array of {@link progcontest_access_rule_base}s.
     */
    protected function make_rules($progcontestobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($progcontestobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['progcontestaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('progcontestaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the progcontest table.
     *
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($progcontestform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'progcontest'));
        foreach (self::get_rule_classes() as $rule) {
    ...
    public static function validate_settings_form_fields(array $errors,
        ...
        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $progcontestform);
        }
    ...
    /**
     * Save any submitted settings when the progcontest settings form is submitted.
     *
    ...
    public static function save_settings($progcontest) {
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($progcontest);
        }
    }

    /**
     * Delete any rule-specific settings when the progcontest is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the progcontest table.
     *
    ...
    public static function delete_settings($progcontest) {
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($progcontest);
        }
    }
     ...
     */
    protected static function get_load_sql($progcontestid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{progcontest} progcontest';
        $allparams = array('progcontestid' => $progcontestid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($progcontestid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE progcontest.id = :progcontestid", $allparams);
    }
     ...
     */
    public static function load_settings($progcontestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($progcontestid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($progcontestid);
        }
    ...
    /**
     * Load the progcontest settings and any settings required by the access rules.
     ...
     *
     * @param int $progcontestid the progcontest id.
     * @return object mdl_progcontest row with extra fields.
     */
    public static function load_progcontest_and_settings($progcontestid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($progcontestid, $rules, 'progcontest.*');
        $progcontest = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($progcontestid) as $name => $value) {
                $progcontest->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this progcontest, such
     * as is shown at the top of the progcontest view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this progcontest now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_progcontest_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_progcontest_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_progcontest_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'progcontestobj' => $this->progcontestobj,
     ...
     * progcontest. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant progcontest_attempts row.
    ...
    /**
     * @return bolean if this progcontest should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the progcontest view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_progcontest_renderer $output the progcontest renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->progcontestobj->view_url(), $message);
            die();
        } else {
            redirect($this->progcontestobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the progcontest, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(progcontest_attempt::IN_PROGRESS, progcontest_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = progcontest_attempt_state($this->progcontestobj->get_progcontest(), $attempt);
        $reviewoptions = mod_progcontest_display_options::make_from_progcontest(
                $this->progcontestobj->get_progcontest(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->progcontestobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->progcontestobj->review_url($attempt->id),
