...
/**
 * Back-end code for handling data about progcontestzes and the current user's attempt.
 *
 * There are classes for loading all the information about a progcontest and attempts,
...
/**
 * Class for progcontest exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_progcontest_exception extends moodle_exception {
    /**
     * Constructor.
     *
     * @param progcontest $progcontestobj the progcontest the error relates to.
     ...
     */
    public function __construct($progcontestobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $progcontestobj->view_url();
        }
        parent::__construct($errorcode, 'progcontest', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a progcontest and the questions it contains, and making the
...
class progcontest {
    ...
    protected $cm;
    /** @var stdClass the progcontest settings from the database. */
    protected $progcontest;
    /** @var context the progcontest context. */
    protected $context;

    /** @var stdClass[] of questions augmented with slot information. */
    protected $questions = null;
    /** @var stdClass[] of progcontest_section rows. */
    protected $sections = null;
    /** @var progcontest_access_manager the access manager for this progcontest. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/progcontest:preview. */
     ...
     *
     * @param object $progcontest the row from the progcontest table.
     * @param object $cm the course_module object for this progcontest.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($progcontest, $cm, $course, $getcontext = true) {
        $this->progcontest = $progcontest;
        $this->cm = $cm;
        $this->progcontest->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new progcontest object for a specific user.
     *
     * @param int $progcontestid the the progcontest id.
     * @param int|null $userid the the userid (optional). If passed, relevant overrides are applied.
     * @return progcontest the new progcontest object.
     */
    public static function create($progcontestid, $userid = null) {
        global $DB;

        $progcontest = progcontest_access_manager::load_progcontest_and_settings($progcontestid);
        $course = $DB->get_record('course', array('id' => $progcontest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('progcontest', $progcontest->id, $course->id, false, MUST_EXIST);

        // Update progcontest with override information.
        if ($userid) {
            $progcontest = progcontest_update_effective_access($progcontest, $userid);
        }

        return new progcontest($progcontest, $cm, $course);
    }

    /**
     * Create a {@link progcontest_attempt} for an attempt at this progcontest.
     *
     * @param object $attemptdata row from the progcontest_attempts table.
     * @return progcontest_attempt the new progcontest_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new progcontest_attempt($attemptdata, $this->progcontest, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this progcontest.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                ...
                '{progcontest_slots} slot ON slot.progcontestid = :progcontestid AND q.id = slot.questionid',
                array('progcontestid' => $this->progcontest->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this progcontest. You must call
    ...
    /**
     * Get an instance of the {@link \mod_progcontest\structure} class for this progcontest.
     * @return \mod_progcontest\structure describes the questions in the progcontest.
     */
    public function get_structure() {
        return \mod_progcontest\structure::create_for_progcontest($this);
    }

    // Simple getters ==========================================================
    /**
     * Get the id of the course this progcontest belongs to.
     *
    ...
    /**
     * Get the course settings object that this progcontest belongs to.
     *
    ...
    /**
     * Get this progcontest's id (in the progcontest table).
     *
     * @return int the progcontest id.
     */
    public function get_progcontestid() {
        return $this->progcontest->id;
    }

    /**
     * Get the progcontest settings object.
     *
     * @return stdClass the row of the progcontest table.
     */
    ...
    /**
     * Get the progcontest name.
     *
     * @return string the name of this progcontest.
     */
    public function get_progcontest_name() {
        return $this->progcontest->name;
    }

    /**
     * Get the navigation method in use.
     *
     * @return int progcontest_NAVMETHOD_FREE or progcontest_NAVMETHOD_SEQ.
     */
    public function get_navigation_method() {
        return $this->progcontest->navmethod;
    }

    /** @return int the number of attempts allowed at this progcontest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->progcontest->attempts;
    }

    /**
     * Get the course-module id for this progcontest.
     *
    ...
    /**
     * Get the course-module object for this progcontest.
     *
    ...
    /**
     * Get the progcontest context.
     *
     * @return context the module context for this progcontest.
     */
    ...
    /**
     * @return bool wether the current user is someone who previews the progcontest,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/progcontest:preview', $this->context);
        }
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/progcontest:attempt', $this->show_only_active_users());
    }
    ...
    /**
     * @return bool whether any questions have been added to this progcontest.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'progcontest', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this progcontest.
     *
     * @return array 0, 1, 2, ... => progcontest_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('progcontest_sections',
                    array('progcontestid' => $this->get_progcontestid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return progcontest_access_manager and instance of the progcontest_access_manager class
     * for this progcontest at this time.
     *
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new progcontest_access_manager($this, $timenow,
                    has_capability('mod/progcontest:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the progcontest context.
     *
     * @param string $capability the name of the capability to check. For example mod/progcontest:view.
    ...
    /**
     * Wrapper round the require_capability function that automatically passes in the progcontest context.
     *
     * @param string $capability the name of the capability to check. For example mod/progcontest:view.
     * @param int|null $userid A user id. By default (null) checks the permissions of the current user.
     * @param bool $doanything If false, ignore effect of admin role assignment.
    ...
    /**
     * @return string the URL of this progcontest's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/progcontest/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this progcontest's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/progcontest/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/progcontest/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
     ...
     * @return moodle_url the URL of this progcontest's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/progcontest/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/progcontest/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/progcontest/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by progcontest access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by progcontest access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_progcontest_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_progcontest_display_options::DURING ||
                $when == mod_progcontest_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_progcontest_display_options::LATER_WHILE_OPEN && $this->progcontest->timeclose &&
                $this->progcontest->reviewattempt & mod_progcontest_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'progcontest',
                    userdate($this->progcontest->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'progcontest');
        }
    }
     ...
     *
     * @param string $title the name of this particular progcontest page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_progcontest_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this progcontest.
     *
     * @param  boolean $includepotential if the progcontest include random questions,
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the progcontest may content
...
/**
 * This class extends the progcontest class to hold data about the state of a particular attempt,
 * in addition to the data about the progcontest.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class progcontest_attempt {
    ...
    /** @var int maximum number of slots in the progcontest for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var progcontest object containing the progcontest settings. */
    protected $progcontestobj;

    /** @var stdClass the progcontest_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this progcontest attempt. */
    protected $quba;
    ...
    /** @var array of progcontest_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_progcontest_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the progcontest_attempts table.
     * @param object $progcontest the progcontest object for this attempt and user.
     * @param object $cm the course_module object for this progcontest.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $progcontest, $cm, $course, $loadquestions = true) {
        $this->attempt = $attempt;
        $this->progcontestobj = new progcontest($progcontest, $cm, $course);
     ...
     *
     * @param array $conditions passed to $DB->get_record('progcontest_attempts', $conditions).
     * @return progcontest_attempt the desired instance of this class.
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('progcontest_attempts', $conditions, '*', MUST_EXIST);
        $progcontest = progcontest_access_manager::load_progcontest_and_settings($attempt->progcontest);
        $course = $DB->get_record('course', array('id' => $progcontest->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('progcontest', $progcontest->id, $course->id, false, MUST_EXIST);

        // Update progcontest with override information.
        $progcontest = progcontest_update_effective_access($progcontest, $attempt->userid);

        return new progcontest_attempt($attempt, $progcontest, $cm, $course);
    }

    /**
     * Static function to create a new progcontest_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new progcontest_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return progcontest_attempt_state_name($state);
    }
    ...
    public function load_questions() {
        ...
        if (isset($this->quba)) {
            throw new coding_exception('This progcontest attempt has already had the questions loaded.');
        }

        $this->quba = question_engine::load_questions_usage_by_activity($this->attempt->uniqueid);
        $this->slots = $DB->get_records('progcontest_slots',
                array('progcontestid' => $this->get_progcontestid()), 'slot',
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'progcontest');
                }
    ...
    // Simple getters ==========================================================
    public function get_progcontest() {
        return $this->progcontestobj->get_progcontest();
    }

    public function get_progcontestobj() {
        return $this->progcontestobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->progcontestobj->get_courseid();
    }
     ...
     */
    public function get_course() {
        return $this->progcontestobj->get_course();
    }

    /** @return int the progcontest id. */
    public function get_progcontestid() {
        return $this->progcontestobj->get_progcontestid();
    }

    /** @return string the name of this progcontest. */
    public function get_progcontest_name() {
        return $this->progcontestobj->get_progcontest_name();
    }

    /** @return int the progcontest navigation method. */
    public function get_navigation_method() {
        return $this->progcontestobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->progcontestobj->get_cm();
    }
     ...
     */
    public function get_cmid() {
        return $this->progcontestobj->get_cmid();
    }

    /**
     * @return bool whether the current user is someone who previews the progcontest,
     * rather than attempting it.
     */
    public function is_preview_user() {
        return $this->progcontestobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this progcontest (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->progcontestobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this progcontest. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->progcontestobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the progcontest_attempts table. */
    ...
    /** @return string one of the progcontest_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/progcontest:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the progcontest in a non-trivial way?
     *
     ...
     * To see how this works, try setting a progcontest to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the progcontest
     ...
     *
     * @param question_display_options $options the display options for this progcontest attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return progcontest_feedback_for_grade($grade, $this->get_progcontest(),
                $this->progcontestobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->progcontestobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        $this->progcontestobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_progcontest_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/progcontest:attempt';
        } else {
            $capability = 'mod/progcontest:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a progcontest
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the progcontest as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/progcontest:viewreports') ||
                $this->has_capability('mod/progcontest:preview')) {
    ...
    public function can_navigate_to($slot) {
        ...
        switch ($this->get_navigation_method()) {
            case progcontest_NAVMETHOD_FREE:
                return true;
                break;
            case progcontest_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return progcontest_attempt_state($this->get_progcontest(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_progcontest_display_options for this progcontest at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = progcontest_get_review_options($this->get_progcontest(),
                        $this->attempt, $this->progcontestobj->get_context());
            ...
            $options = mod_progcontest_display_options::make_from_progcontest($this->get_progcontest(),
                    mod_progcontest_display_options::DURING);
            $options->flags = progcontest_get_flag_option($this->attempt, $this->progcontestobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the progcontest.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the progcontest, or for the
     * whole progcontest.
     *
    ...
    public function get_question_usage() {
        if (!(PHPUNIT_TEST || defined('BEHAT_TEST'))) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the progcontest_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != progcontest_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_progcontest()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the progcontest where this question appears.
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the progcontest this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the progcontest.
     */
    public function get_question_mark($slot) {
        return progcontest_format_question_grade($this->get_progcontest(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->progcontestobj->get_progcontest()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->progcontestobj->get_progcontest()->timelimit;
        }
        if ($this->progcontestobj->get_progcontest()->timeclose) {
            $deadlines[] = $this->progcontestobj->get_progcontest()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->progcontestobj->get_progcontest()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string progcontest view url.
     */
    public function view_url() {
        return $this->progcontestobj->view_url();
    }
     ...
     * @return string the URL of this progcontest's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->progcontestobj->start_attempt_url($page);
    }
    ...
    public function attempt_page_title(int $page) : string {
        if ($this->get_num_pages() > 1) {
            $a = new stdClass();
            $a->name = $this->get_progcontest_name();
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attempttitlepaged', 'progcontest', $a);
        } else {
            $title = get_string('attempttitle', 'progcontest', $this->get_progcontest_name());
        }
    ...
    public function summary_page_title() : string {
        return get_string('attemptsummarytitle', 'progcontest', $this->get_progcontest_name());
    }

    /**
     * @return moodle_url the URL of this progcontest's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/progcontest/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/progcontest/processattempt.php');
    }
    ...
    public function review_page_title(int $page, bool $showall = false) : string {
        if (!$showall && $this->get_num_pages() > 1) {
            ...
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attemptreviewtitlepaged', 'progcontest', $a);
        } else {
            $title = get_string('attemptreviewtitle', 'progcontest', $this->get_progcontest_name());
        }
    ...
    public function cannot_review_message($short = false) {
        return $this->progcontestobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'progcontest'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_progcontest_renderer $renderer the progcontest renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_progcontest_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_progcontest-blocked_question_warning');
        }
    ...
    protected function render_question_helper($slot, $reviewing, $thispageurl,
            mod_progcontest_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'progcontest', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'progcontest');
        ...
        $placeholderqa->start($this->get_progcontest()->preferredbehaviour, 1);
     ...
     *
     * @param int $slot the slot number of a question in this progcontest attempt.
    ...
    public function render_question_at_step($slot, $seq, $reviewing,
            mod_progcontest_renderer $renderer, $thispageurl = null) {
     ...
     *
     * @param int $slot the slot of a question in this progcontest attempt.
     * @param bool $reviewing is the being printed on an attempt or a review page.
     ...
     *
     * @param mod_progcontest_renderer $output the progcontest renderer to use to output things.
     * @param string $panelclass The type of panel, progcontest_attempt_nav_panel or progcontest_review_nav_panel
     * @param int $page the current page number.
     * @param bool $showall whether we are showing the whole progcontest on one page. (Used by review.php.)
     * @return block_contents the requested object.
     */
    public function get_navigation_panel(mod_progcontest_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_progcontest_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->title = get_string('progcontestnavigation', 'progcontest');
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this progcontest.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_progcontest_links_to_other_attempts} object returned contains an
     ...
     * @return mod_progcontest_links_to_other_attempts|bool containing array int => null|moodle_url.
     *      False if none.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = progcontest_get_user_attempts($this->get_progcontest()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_progcontest_links_to_other_attempts();
     ...
     * @return mod_progcontest_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->progcontestobj->get_progcontest()->graceperiod;
        ...
        // Otherwise, we were in progcontest_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->progcontestobj->get_progcontest()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('progcontest_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            progcontest_save_best_grade($this->get_progcontest(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_progcontest\question\qubaids_for_users_attempts(
                $this->get_progcontestid(), $this->get_userid(), 'all', true);
        ...
        if ($questiondata->qtype != 'random') {
            ...
            $tagids = progcontest_retrieve_slot_tag_ids($this->slots[$slot]->id);
            ...
            if ($newqusetionid === null) {
                throw new moodle_exception('notenoughrandomquestions', 'progcontest',
                        $this->progcontestobj->view_url(), $questiondata);
            }
        }
     ...
     * Submit the attempt.
     *
     * The separate $timefinish argument should be used when the progcontest attempt
    ...
    public function process_finish($timestamp, $processsubmitted, $timefinish = null) {
        ...
        if (!$this->is_preview()) {
            progcontest_save_best_grade($this->get_progcontest(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_progcontest\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('progcontest_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_progcontest\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        progcontest_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_progcontest\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $progcontestrecord = $this->get_progcontest();
        $params = array(
            'context' => $this->get_progcontestobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'progcontestid' => $progcontestrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('progcontest', $this->get_progcontest());
        $event->add_record_snapshot('progcontest_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the progcontest.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/progcontest/$script.php.
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/progcontest/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a progcontest.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        $graceperiodmin = get_config('progcontest', 'graceperiodmin');
        ...
        $toolate = false;
        if ($timeclose !== false) {
            if ($timenow > $timeclose - progcontest_MIN_TIME_TO_CONTINUE) {
                // If there is only a very small amount of time left, there is no point trying
                // to show the student another page of the progcontest. Just finish now.
                $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_progcontest()->overduehandling === 'graceperiod') {
                if ($timenow > $timeclose + $this->get_progcontest()->graceperiod + $graceperiodmin) {
        ...
        // Update the progcontest attempt record.
        try {
            if ($becomingabandoned) {
                $this->process_abandon($timenow, true);
            } else {
                if (!$toolate || $this->get_progcontest()->overduehandling == 'graceperiod') {
    ...
    public function check_page_access($page, $allownext = true) {
        if ($this->get_navigation_method() != progcontest_NAVMETHOD_SEQ) {
            return true;
        }
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('progcontest_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'progcontestid' => $this->get_progcontestid()
            )
        );
 ...
 * @since      Moodle 2.9
 */
class progcontest_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class progcontest_nav_question_button implements renderable {
    ...
    public $url;
    /** @var int progcontest_NAVMETHOD_FREE or progcontest_NAVMETHOD_SEQ. */
    public $navmethod;
}
 ...
 * @since      Moodle 2.0
 */
abstract class progcontest_nav_panel_base {
    /** @var progcontest_attempt */
    ...
    public function __construct(progcontest_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the progcontest navigation block.
     *
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            if (!is_null($heading)) {
                $sections = $this->attemptobj->get_progcontestobj()->get_sections();
                if (!(empty($heading) && count($sections) == 1)) {
                    $buttons[] = new progcontest_nav_section_heading(format_string($heading));
                }
            }
            ...
            $button = new progcontest_nav_question_button();
            $button->id          = 'progcontestnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'progcontest');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'progcontest');
        } else {
            return get_string('viewed', 'progcontest');
        }
    }
     ...
     *
     * @param mod_progcontest_renderer $output the progcontest renderer to use.
     * @return string HTML to output.
     */
    public function render_before_button_bits(mod_progcontest_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_progcontest_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_progcontest()->showuserpicture == progcontest_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_progcontest()->showuserpicture == progcontest_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // Programmingcontest navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // Programmingcontest navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link progcontest_nav_panel_base} for the attempt progcontest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class progcontest_attempt_nav_panel extends progcontest_nav_panel_base {
    ...
    public function render_before_button_bits(mod_progcontest_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'progcontest'),
                array('id' => 'progcontestnojswarning'));
    }

    public function render_end_bits(mod_progcontest_renderer $output) {
        if ($this->page == -1) {
            // Don't link from the summary page to itself.
            return '';
        }
        return html_writer::link($this->attemptobj->summary_url(),
                get_string('endtest', 'progcontest'), array('class' => 'endtestlink aalink')) .
...
/**
 * Specialisation of {@link progcontest_nav_panel_base} for the review progcontest page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class progcontest_review_nav_panel extends progcontest_nav_panel_base {
    public function get_question_url($slot) {
        ...
        if ($this->attemptobj->get_num_pages() > 1) {
            if ($this->showall) {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, false),
                        get_string('showeachpage', 'progcontest'));
            } else {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, true),
                        get_string('showall', 'progcontest'));
            }
        }
