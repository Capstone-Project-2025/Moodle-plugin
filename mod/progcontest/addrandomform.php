...
/**
 * Defines the Moodle forum used to add random questions to the progcontest.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_add_random_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'existingcategoryheader',
                get_string('randomfromexistingcategory', 'progcontest'));
        ...
        $mform->addElement('checkbox', 'includesubcategories', '', get_string('recurse', 'progcontest'));
        ...
        if ($CFG->usetags) {
            ...
            $options = array(
                'multiple' => true,
                'noselectionstring' => get_string('anytags', 'progcontest'),
            );
            $mform->addElement('autocomplete', 'fromtags', get_string('randomquestiontags', 'mod_progcontest'), $tagstrings, $options);
            $mform->addHelpButton('fromtags', 'randomquestiontags', 'mod_progcontest');
        }
        ...
        // only matched 9 questions (not already in the progcontest), then the drop-down would
        // only offer choices 1..9. This nice UI hint got lost when the UI became Ajax-y.
        // We should add it back.
        $mform->addElement('select', 'numbertoadd', get_string('randomnumber', 'progcontest'),
                $this->get_number_of_questions_to_add_choices());

        $previewhtml = $OUTPUT->render_from_template('mod_progcontest/random_question_form_preview', []);
        $mform->addElement('html', $previewhtml);

        $mform->addElement('submit', 'existingcategory', get_string('addrandomquestion', 'progcontest'));

        // Random from a new category section.
        $mform->addElement('header', 'newcategoryheader',
                get_string('randomquestionusinganewcategory', 'progcontest'));
        ...
        $mform->addElement('submit', 'newcategory',
                get_string('createcategoryandaddrandomquestion', 'progcontest'));
        ...
        $PAGE->requires->js_call_amd('mod_progcontest/add_random_form', 'init', [
