...
namespace progcontestaccess_timelimit;

use progcontest;
use progcontestaccess_timelimit;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/progcontest/accessrule/timelimit/rule.php');


/**
 * Unit tests for the progcontestaccess_timelimit plugin.
 *
 * @package    progcontestaccess_timelimit
...
class rule_test extends \basic_testcase {
    public function test_time_limit_access_rule() {
        ...
        $progcontest->timelimit = 3600;
        $cm = new \stdClass();
        $cm->id = 0;
        $progcontestobj = new progcontest($progcontest, $cm, null);
        $rule = new progcontestaccess_timelimit($progcontestobj, 10000);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('progcontesttimelimit', 'progcontestaccess_timelimit', format_time(3600)));
    ...
    /**
     * Test the time_left_display method of the progcontestaccess_timelimit class.
     *
     * @param int $timetoclose  The number of seconds that is left to the progcontest' closing time
     * @param int $timelimit    Time limit of the progcontest
     * @param int $displaylimit The limit that is displayed on the progcontest page
    ...
    public function test_time_limit_access_rule_with_time_close($timetoclose, $timelimit, $displaylimit, $actuallimit) {
        ...
        $progcontest = new \stdClass();
        $progcontest->timeclose = $timenow + $timetoclose;
        $progcontest->timelimit = $timelimit;
        $cm = new \stdClass();
        $cm->id = 0;
        $progcontestobj = new progcontest($progcontest, $cm, null);
        $rule = new progcontestaccess_timelimit($progcontestobj, $timenow);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('progcontesttimelimit', 'progcontestaccess_timelimit', format_time($displaylimit)));
