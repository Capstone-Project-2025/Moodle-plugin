...
require_once($CFG->dirroot  . '/mod/progcontest/accessrule/seb/lib.php');

/**
 * Custom code to be run on installing the plugin.
 */
function xmldb_progcontestaccess_seb_install() {
    global $DB;

    // Reconfigure all existing progcontestzes to use a new progcontestaccess_seb.
    $params = ['browsersecurity' => 'safebrowser'];

    $total = $DB->count_records('progcontest', $params);
    if ($total > 0) {
        $rs = $DB->get_recordset('progcontest', $params);

        $i = 0;
        $pbar = new progress_bar('updateprogcontestrecords', 500, true);

        foreach ($rs as $progcontest) {
            if (!$DB->record_exists('seb_settings', ['progcontestid' => $progcontest->id])) {
                $cm = get_coursemodule_from_instance('progcontest', $progcontest->id, $progcontest->course);

                $sebsettings = new stdClass();

                $sebsettings->progcontestid = $progcontest->id;
                ...
                $progcontest->browsersecurity = '-';
                $DB->update_record('progcontest', $progcontest);
            }

            $i++;
            $pbar->update($i, $total, "Reconfiguring existing progcontestzes to use a new SEB plugin - $i/$total.");
        }
