...
/**
 * Manage the access to the progcontest.
 *
...
class access_manager {
    ...
    /** @var progcontest $progcontest A progcontest object containing all information pertaining to current progcontest. */
    private $progcontest;

    /** @var progcontest_settings $progcontestsettings A progcontest settings persistent object containing plugin settings */
    private $progcontestsettings;

    /** @var context_module $context Context of this progcontest activity. */
    private $context;
     ...
     *
     * @param progcontest $progcontest The details of the progcontest.
     */
    public function __construct(progcontest $progcontest) {
        $this->progcontest = $progcontest;
        $this->context = context_module::instance($progcontest->get_cmid());
        $this->progcontestsettings = progcontest_settings::get_by_progcontest_id($progcontest->get_progcontestid());
        $this->validconfigkey = progcontest_settings::get_config_key_by_progcontest_id($progcontest->get_progcontestid());
    }

    /**
     * Check if the browser exam key hash in header matches one of the listed browser exam keys from progcontest settings.
     *
    ...
    public function validate_browser_exam_keys() : bool {
        ...
        $browserexamkeys = $this->progcontestsettings->get('allowedbrowserexamkeys');
    ...
    /**
     * Check if the config key hash in header matches progcontest settings.
     *
    ...
    /**
     * Check if Safe Exam Browser is required to access progcontest.
     * If progcontestsettings do not exist, then there is no requirement for using SEB.
     *
     * @return bool If required.
     */
    public function seb_required() : bool {
        if (!$this->progcontestsettings) {
            return false;
        } else {
    ...
    /**
     * This is the basic check for the Safe Exam Browser previously used in the progcontestaccess_safebrowser plugin that
    ...
    public function can_bypass_seb() : bool {
        return has_capability('progcontestaccess/seb:bypassseb', $this->context);
    }
    ...
    /**
     * Getter for the progcontest object.
     *
     * @return progcontest
     */
    public function get_progcontest() : progcontest {
    ...
    /**
     * Get type of SEB usage for the progcontest.
     *
     * @return int
     */
    public function get_seb_use_type() : int {
        if (empty($this->progcontestsettings)) {
            return settings_provider::USE_SEB_NO;
        } else {
            return $this->progcontestsettings->get('requiresafeexambrowser');
        }
    }
