...
/**
 * Entity model representing progcontest settings for the seb plugin.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontest_settings extends persistent {

    /** Table name for the persistent. */
    const TABLE = 'seb_settings';
    ...
    /**
     * Return an instance by progcontest id.
     *
     * This method gets data from cache before doing any DB calls.
     *
     * @param int $progcontestid Programmingcontest id.
     * @return false|\progcontestaccess_seb\progcontest_settings
     */
    public static function get_by_progcontest_id(int $progcontestid) {
        if ($data = self::get_progcontest_settings_cache()->get($progcontestid)) {
            return new static(0, $data);
        }

        return self::get_record(['progcontestid' => $progcontestid]);
    }

    /**
     * Return cached SEB config represented as a string by progcontest ID.
     *
     * @param int $progcontestid Programmingcontest id.
     * @return string|null
     */
    public static function get_config_by_progcontest_id(int $progcontestid) : ?string {
        $config = self::get_config_cache()->get($progcontestid);
        ...
        $config = null;
        if ($settings = self::get_by_progcontest_id($progcontestid)) {
            $config = $settings->get_config();
            self::get_config_cache()->set($progcontestid, $config);
        }
    ...
    /**
     * Return cached SEB config key by progcontest ID.
     *
     * @param int $progcontestid Programmingcontest id.
     * @return string|null
     */
    public static function get_config_key_by_progcontest_id(int $progcontestid) : ?string {
        $configkey = self::get_config_key_cache()->get($progcontestid);
        ...
        if ($settings = self::get_by_progcontest_id($progcontestid)) {
            ...
            self::get_config_key_cache()->set($progcontestid, $configkey);
        }
    ...
    private static function get_config_key_cache() : \cache_application {
        return \cache::make('progcontestaccess_seb', 'configkey');
    }
    ...
    private static function get_config_cache() : \cache_application {
        return \cache::make('progcontestaccess_seb', 'config');
    }

    /**
     * Return progcontest settings cache object,
     *
     * @return \cache_application
     */
    private static function get_progcontest_settings_cache() : \cache_application {
        return \cache::make('progcontestaccess_seb', 'progcontestsettings');
    }
    ...
    private function after_save() {
        self::get_progcontest_settings_cache()->set($this->get('progcontestid'), $this->to_record());
        self::get_config_cache()->set($this->get('progcontestid'), $this->config);
        self::get_config_key_cache()->set($this->get('progcontestid'), $this->configkey);
    }
    ...
    protected function before_delete() {
        $key = $this->get('progcontestid');
        self::get_progcontest_settings_cache()->delete($key);
    ...
    protected function validate_allowedbrowserexamkeys($keys) {
        ...
        foreach ($keys as $i => $key) {
            if (!preg_match('~^[a-f0-9]{64}$~', $key)) {
                return new lang_string('allowedbrowserkeyssyntax', 'progcontestaccess_seb');
            }
        }
        if (count($keys) != count(array_unique($keys))) {
            return new lang_string('allowedbrowserkeysdistinct', 'progcontestaccess_seb');
        }
        return true;
    }
    ...
    /**
     * Create or update the config string based on the current progcontest settings.
     */
    ...
    /**
     * Case for USE_SEB_UPLOAD_CONFIG. This creates a plist from an uploaded configuration file, then applies the progcontest
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new moodle_exception('noconfigfilefound', 'progcontestaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $progcontesturl = new moodle_url($CFG->wwwroot . "/mod/progcontest/view.php", ['id' => $this->get('cmid')]);
        $this->plist->set_or_update_value('startURL', new CFString($progcontesturl->out(true)));
    ...
    /**
     * Sets the quitURL if found in the progcontest_settings.
     */
    ...
    /**
     * Sets the progcontest_setting's linkquitseb if a quitURL value was found in a template or uploaded config.
     */
