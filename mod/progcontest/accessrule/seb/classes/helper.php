...
class helper {
    ...
    /**
     * Get seb config content for a particular progcontest. This method checks caps.
     *
     * @param string $cmid The course module ID for a progcontest with config.
    ...
    public static function get_seb_config_content(string $cmid) : string {
        ...
        // Retrieve the config for progcontest.
        $config = progcontest_settings::get_config_by_progcontest_id($cm->instance);
        if (empty($config)) {
            throw new \moodle_exception('noconfigfound', 'progcontestaccess_seb', '', $cm->id);
        }
        return $config;
    }
