...
/**
 * Implementation of the progcontestaccess_seb plugin.
 *
...
use \progcontestaccess_seb\event\access_prevented;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class progcontestaccess_seb extends progcontest_access_rule_base {

    /** @var access_manager $accessmanager Instance to manage the access to the progcontest for this plugin. */
    private $accessmanager;

    /**
     * Create an instance of this rule for a particular progcontest.
     *
     * @param progcontest $progcontestobj information about the progcontest in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param access_manager $accessmanager the progcontest accessmanager.
     */
    public function __construct(progcontest $progcontestobj, int $timenow, access_manager $accessmanager) {
        parent::__construct($progcontestobj, $timenow);
        $this->accessmanager = $accessmanager;
    }

    /**
     * Return an appropriately configured instance of this rule, if it is applicable
     * to the given progcontest, otherwise return null.
     *
     * @param progcontest $progcontestobj information about the progcontest in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/progcontest:ignoretimelimits capability.
     * @return progcontest_access_rule_base|null the rule, if applicable, else null.
     */
    public static function make (progcontest $progcontestobj, $timenow, $canignoretimelimits) {
        $accessmanager = new access_manager($progcontestobj);
        // If Safe Exam Browser is not required, this access rule is not applicable.
        if (!$accessmanager->seb_required()) {
            return null;
        }

        return new self($progcontestobj, $timenow, $accessmanager);
    }

    /**
     * Add any fields that this rule requires to the progcontest settings form. This
     * method is called from {@link mod_progcontest_mod_form::definition()}, while the
     ...
     */
    public static function add_settings_form_fields(mod_progcontest_mod_form $progcontestform, MoodleQuickForm $mform) {
        settings_provider::add_seb_settings_fields($progcontestform, $mform);
    }
     ...
     * @param mod_progcontest_mod_form $progcontestform the progcontest form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
                                                         array $data, $files, mod_progcontest_mod_form $progcontestform) : array {

        $progcontestid = $data['instance'];
        $cmid = $data['coursemodule'];
        $context = $progcontestform->get_context();

        if (!settings_provider::can_configure_seb($context)) {
            return $errors;
        }

        if (settings_provider::is_seb_settings_locked($progcontestid)) {
            return $errors;
        }
        ...
        $progcontestsettings = (new progcontest_settings())->from_record($settings);
        // Set non-form fields.
        $progcontestsettings->set('progcontestid', $progcontestid);
        $progcontestsettings->set('cmid', $cmid);
        $progcontestsettings->validate();

        // Add any errors to list.
        foreach ($progcontestsettings->get_errors() as $name => $error) {
        ...
        if ($progcontestsettings->get('requiresafeexambrowser') == settings_provider::USE_SEB_UPLOAD_CONFIG) {
        ...
        if ($progcontestsettings->get('requiresafeexambrowser') == settings_provider::USE_SEB_TEMPLATE) {
            if (empty($data['seb_templateid'])) {
                $errors['seb_templateid'] = get_string('invalidtemplate', 'progcontestaccess_seb');
            }
        }

        if ($progcontestsettings->get('requiresafeexambrowser') != settings_provider::USE_SEB_NO) {
            // Global settings may be active which require a progcontest password to be set if using SEB.
            if (!empty(get_config('progcontestaccess_seb', 'progcontestpasswordrequired')) && empty($data['progcontestpassword'])) {
                $errors['progcontestpassword'] = get_string('passwordnotset', 'progcontestaccess_seb');
            }
        }
    ...
    /**
     * Save any submitted settings when the progcontest settings form is submitted. This
     * is called from {@link progcontest_after_add_or_update()} in lib.php.
     *
     * @param object $progcontest the data from the progcontest form, including $progcontest->id
     *      which is the id of the progcontest being saved.
     */
    public static function save_settings($progcontest) {
        ...
        if (settings_provider::is_seb_settings_locked($progcontest->id)) {
            return;
        }
        ...
        $cm = get_coursemodule_from_instance('progcontest', $progcontest->id, $progcontest->course, false, MUST_EXIST);

        $settings = settings_provider::filter_plugin_settings($progcontest);
        $settings->progcontestid = $progcontest->id;
        $settings->cmid = $cm->id;

        // Get existing settings or create new settings if none exist.
        $progcontestsettings = progcontest_settings::get_by_progcontest_id($progcontest->id);
        if (empty($progcontestsettings)) {
            $progcontestsettings = new progcontest_settings(0, $settings);
        } else {
            $settings->id = $progcontestsettings->get('id');
        ...
        } else if ($progcontestsettings->get('id')) {
            $progcontestsettings->delete();
        }
    }

    /**
     * Delete any rule-specific settings when the progcontest is deleted. This is called
     * from {@link progcontest_delete_instance()} in lib.php.
     *
     * @param object $progcontest the data from the database, including $progcontest->id
     *      which is the id of the progcontest being deleted.
     */
    public static function delete_settings($progcontest) {
        $progcontestsettings = progcontest_settings::get_by_progcontest_id($progcontest->id);
        // Check that there are existing settings.
        if ($progcontestsettings !== false) {
            $progcontestsettings->delete();
        }
    }
     ...
     * here is probalby to read the code of {@link progcontest_access_manager::load_settings()}.
     *
     * If you have some settings that cannot be loaded in this way, then you can
     * use the {@link get_extra_settings()} method instead, but that has
     * performance implications.
     *
     * @param int $progcontestid the id of the progcontest we are loading settings for. This
     *     can also be accessed as progcontest.id in the SQL. (progcontest is a table alisas for {progcontest}.)
     ...
     */
    public static function get_settings_sql($progcontestid) : array {
        return [
                ...
                , 'LEFT JOIN {seb_settings} seb ON seb.progcontestid = progcontest.id '
                . 'LEFT JOIN {seb_template} sebtemplate ON seb.templateid = sebtemplate.id '
                , []
        ];
    }
    ...
    public function prevent_access() {
        ...
        // If the rule is active, enforce a secure view whilst taking the progcontest.
    ...
    private function get_user_finished_attempts() : array {
        ...
        return progcontest_get_user_attempts(
            $this->progcontestobj->get_progcontestid(),
            $USER->id,
            progcontest_attempt::FINISHED,
            false
        );
    }
    ...
    private function prevent_display_blocks() {
        ...
        if ($PAGE->has_set_url() && $PAGE->url == $this->progcontestobj->view_url()) {
            $attempts = $this->get_user_finished_attempts();

            // Don't display blocks before starting an attempt.
            if (empty($attempts) && !get_config('progcontestaccess_seb', 'displayblocksbeforestart')) {
                $PAGE->blocks->show_only_fake_blocks();
            }

            // Don't display blocks after finishing an attempt.
            if (!empty($attempts) && !get_config('progcontestaccess_seb', 'displayblockswhenfinished')) {
                $PAGE->blocks->show_only_fake_blocks();
            }
        }
    }
    ...
    private function get_reason_text(string $identifier) : string {
        if (in_array($identifier, ['not_seb', 'invalid_config_key', 'invalid_browser_key'])) {
            return get_string($identifier, 'progcontestaccess_seb');
        }

        return get_string('unknown_reason', 'progcontestaccess_seb');
    }
    ...
    private function get_invalid_key_error_message() : string {
        ...
        return get_string('invalidkeys', 'progcontestaccess_seb')
    ...
    private function get_require_seb_error_message() : string {
        $message = get_string('clientrequiresseb', 'progcontestaccess_seb');
    ...
    private function get_quit_button() : string {
        ...
        if (!empty($this->progcontest->seb_linkquitseb)) {
            $quitbutton = html_writer::link(
                $this->progcontest->seb_linkquitseb,
                get_string('exitsebbutton', 'progcontestaccess_seb'),
    ...
    /**
     * Information, such as might be shown on the progcontest view page, relating to this restriction.
    ...
    public function description() : array {
        $messages = [get_string('sebrequired', 'progcontestaccess_seb')];
    ...
    public function setup_attempt_page($page) {
        $page->set_title($this->progcontestobj->get_course()->shortname . ': ' . $page->title);
    ...
    private function get_action_buttons() : string {
        ...
        $linkconfig = explode(',', get_config('progcontestaccess_seb', 'showseblinks'));
    ...
    private function get_download_seb_button() : string {
        ...
        if (!empty($this->get_seb_download_url())) {
            $button = $OUTPUT->single_button($this->get_seb_download_url(), get_string('sebdownloadbutton', 'progcontestaccess_seb'));
        }
    ...
    private function get_launch_seb_button() : string {
        ...
        $seblink = \progcontestaccess_seb\link_generator::get_link($this->progcontest->cmid, true, is_https());

        $buttonlink = html_writer::start_tag('div', array('class' => 'singlebutton'));
        $buttonlink .= html_writer::link($seblink, get_string('seblinkbutton', 'progcontestaccess_seb'),
            ['class' => 'btn btn-secondary', 'title' => get_string('seblinkbutton', 'progcontestaccess_seb')]);
    ...
    private function get_download_config_button() : string {
        // Rendering as a href and not as button in a form to circumvent browser warnings for sending to URL with unknown protocol.
        $httplink = \progcontestaccess_seb\link_generator::get_link($this->progcontest->cmid, false, is_https());

        $buttonlink = html_writer::start_tag('div', array('class' => 'singlebutton'));
        $buttonlink .= html_writer::link($httplink, get_string('httplinkbutton', 'progcontestaccess_seb'),
            ['class' => 'btn btn-secondary', 'title' => get_string('httplinkbutton', 'progcontestaccess_seb')]);
        $buttonlink .= html_writer::end_tag('div');
    ...
    private function get_seb_download_url() : string {
        return get_config('progcontestaccess_seb', 'downloadlink');
    }
    ...
    private function should_display_download_seb_link() : bool {
        return !empty($this->progcontest->seb_showsebdownloadlink);
    }
    ...
    private function should_redirect_to_seb_config_link() : bool {
        return $this->accessmanager->is_using_seb() && get_config('progcontestaccess_seb', 'autoreconfigureseb');
    }
