...
<plist version=\"1.0\"><dict><key>showTaskBar</key><true/><key>allowWlan</key><false/><key>showReloadButton</key><true/>"
        ...
        $this->assertTrue(\progcontestaccess_seb\helper::is_valid_seb_config($validseb));
        $this->assertFalse(\progcontestaccess_seb\helper::is_valid_seb_config($invalidseb));
        $this->assertFalse(\progcontestaccess_seb\helper::is_valid_seb_config($emptyseb));
    }
    ...
    public function test_get_seb_file_headers() {
        ...
        $headers = \progcontestaccess_seb\helper::get_seb_file_headers($expiretime);
    ...
    public function test_can_not_get_config_content_with_invalid_cmid() {
        ...
        $this->expectExceptionMessage("Can't find data record in database. (SELECT cm.*, m.name, md.name AS modname \n"
            ...
            . "                   JOIN {progcontest} m ON m.id = cm.instance\n"
            . "                   \n"
            ...
            .')])');
        \progcontestaccess_seb\helper::get_seb_config_content('999');
    }
    ...
    public function test_can_not_get_config_content_when_user_not_enrolled_in_course() {
        ...
        $this->setAdminUser();
        $course = $this->getDataGenerator()->create_course();
        $progcontest = $this->create_test_progcontest($course, \progcontestaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        \progcontestaccess_seb\helper::get_seb_config_content($progcontest->cmid);
    }

    /**
     * Test that if SEB progcontest settings can't be found, a seb config content won't be provided.
     */
    public function test_can_not_get_config_content_if_config_not_found_for_cmid() {
        ...
        $this->expectException(\moodle_exception::class);
        $this->expectExceptionMessage("No SEB config could be found for progcontest with cmid: $progcontest->cmid");
        \progcontestaccess_seb\helper::get_seb_config_content($progcontest->cmid);
    }

    /**
     * That that if config is empty for a progcontest, a seb config content won't be provided.
     */
    public function test_can_not_get_config_content_if_config_empty() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $progcontest = $this->create_test_progcontest($course, \progcontestaccess_seb\settings_provider::USE_SEB_NO);
    ...
    public function test_config_provided() {
        ...
        $config = \progcontestaccess_seb\helper::get_seb_config_content($progcontest->cmid);

        $url = new \moodle_url("/mod/progcontest/view.php", ['id' => $progcontest->cmid]);
