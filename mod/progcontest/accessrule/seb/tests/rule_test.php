...
class rule_test extends \advanced_testcase {
    ...
    private function get_seb_launch_link() {
        return 'sebs://www.example.com/moodle/mod/progcontest/accessrule/seb/config.php';
    }
    ...
    private function get_seb_config_download_link() {
        return 'https://www.example.com/moodle/mod/progcontest/accessrule/seb/config.php';
    }
    ...
    public function test_validate_settings_with_valid_data(string $setting, string $data) {
        ...
        $form = $this->createMock('mod_progcontest_mod_form');
        $form->method('get_context')->willReturn(\context_module::instance($this->progcontest->cmid));

        // Validate settings with a dummy form.
        $errors = progcontestaccess_seb::validate_settings_form_fields([], [
            'instance' => $this->progcontest->id,
            'coursemodule' => $this->progcontest->cmid,
            $setting => $data
        ], [], $form);
    ...
    public function test_validate_settings_with_invalid_data(string $setting, string $data) {
        ...
        // Validate settings with a dummy form and progcontest instance.
        $errors = progcontestaccess_seb::validate_settings_form_fields([], [
            'instance' => $this->progcontest->id,
            'coursemodule' => $this->progcontest->cmid,
            $setting => $data
    ...
    public function test_settings_validation_is_not_run_if_settings_are_locked() {
        ...
        $this->progcontest = $this->create_test_progcontest($this->course);
        $this->attempt_progcontest($this->progcontest, $user);
        ...
        $errors = progcontestaccess_seb::validate_settings_form_fields([], [
            'instance' => $this->progcontest->id,
            'coursemodule' => $this->progcontest->cmid, 'seb_requiresafeexambrowser' => 'Uh oh!'
        ], [], $form);
        $this->assertEmpty($errors);
    }
    ...
    public function test_settings_validation_is_not_run_if_conflicting_permissions() {
        ...
        $context = \context_module::instance($this->progcontest->cmid);
        assign_capability('progcontestaccess/seb:manage_seb_requiresafeexambrowser', CAP_ALLOW, $roleid, $context->id);
     ...
     */
    public function test_create_settings_with_existing_progcontest() {
        global $DB;

        $this->setAdminUser();

        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_NO);
        $this->assertFalse($DB->record_exists('seb_settings', ['progcontestid' => $this->progcontest->id]));

        $this->progcontest->seb_requiresafeexambrowser = settings_provider::USE_SEB_CONFIG_MANUALLY;
        progcontestaccess_seb::save_settings($this->progcontest);
        $this->assertNotFalse($DB->record_exists('seb_settings', ['progcontestid' => $this->progcontest->id]));
    }
    ...
    public function test_settings_are_not_saved_if_conflicting_permissions() {
        ...
        $this->progcontest->seb_requiresafeexambrowser = settings_provider::USE_SEB_NO;
        progcontestaccess_seb::save_settings($this->progcontest);

        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
        $this->assertEquals(settings_provider::USE_SEB_CONFIG_MANUALLY, $progcontestsettings->get('requiresafeexambrowser'));
    }
    ...
    public function test_save_settings_throw_an_exception_if_cm_not_found() {
        ...
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_CONFIG_MANUALLY);
        $DB->delete_records('progcontest', ['id' => $this->progcontest->id]);
        $this->progcontest->seb_requiresafeexambrowser = settings_provider::USE_SEB_NO;
        progcontestaccess_seb::save_settings($this->progcontest);
    }
    ...
    public function test_delete_settings_without_existing_settings() {
        ...
        $progcontest->id = 1;
        progcontestaccess_seb::delete_settings($progcontest);
        $this->assertFalse($DB->record_exists('seb_settings', ['progcontestid' => $progcontest->id]));
    }
    ...
    public function test_delete_settings_with_existing_settings() {
        ...
        // Using a generator will create the progcontest_settings record.
        $this->assertNotFalse($DB->record_exists('seb_settings', ['progcontestid' => $this->progcontest->id]));
        progcontestaccess_seb::delete_settings($this->progcontest);
        $this->assertFalse($DB->record_exists('seb_settings', ['progcontestid' => $this->progcontest->id]));
    }
    ...
    protected function check_invalid_config_key() {
        ...
        $this->assertInstanceOf('\progcontestaccess_seb\event\access_prevented', $event);
    ...
    public function test_access_prevented_if_config_key_invalid() {
        ...
        $FULLME = 'https://example.com/moodle/mod/progcontest/attempt.php?attemptid=123&page=4';
    ...
    public function test_access_prevented_if_config_key_invalid_uploaded_config() {
        ...
        // Set progcontest setting to require seb and save BEK.
        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
    ...
    public function test_access_prevented_if_config_key_invalid_uploaded_template() {
        ...
        $progcontestsettings->set('templateid', $this->create_template()->get('id'));
        $progcontestsettings->save();
    ...
    public function test_access_allowed_if_config_key_valid() {
        ...
        // Set progcontest setting to require seb.
        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);

        // Set up dummy request.
        $FULLME = 'https://example.com/moodle/mod/progcontest/attempt.php?attemptid=123&page=4';
        $expectedhash = hash('sha256', $FULLME . $progcontestsettings->get_config_key());
    ...
    public function test_access_allowed_if_browser_exam_keys_valid() {
        ...
        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
        $progcontestsettings->set('requiresafeexambrowser', settings_provider::USE_SEB_CLIENT_CONFIG); // Doesn't check config key.
        $progcontestsettings->set('allowedbrowserexamkeys', $browserexamkey);
        $progcontestsettings->save();
    ...
    public function test_access_allowed_if_browser_exam_keys_valid_use_uploaded_file() {
        ...
        $expectedconfigkey = hash('sha256', $FULLME . $progcontestsettings->get_config_key());
    ...
    public function test_access_prevented_if_using_client_configuration_and_basic_head_is_invalid() {
        ...
        $this->assertStringContainsString(
            'This progcontest has been configured to use the Safe Exam Browser with client configuration.',
    ...
    public function test_access_allowed_if_using_client_configuration_and_basic_head_is_invalid_use_uploaded_config() {
        ...
        // Set progcontest setting to require seb.
        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
        $progcontestsettings->set('requiresafeexambrowser', settings_provider::USE_SEB_UPLOAD_CONFIG); // Doesn't check basic header.
        $xml = file_get_contents(__DIR__ . '/fixtures/unencrypted.seb');
        $this->create_module_test_file($xml, $this->progcontest->cmid);
        $progcontestsettings->save();
    ...
    public function test_access_allowed_if_seb_not_required() {
        ...
        // Set progcontest setting to not require seb.
        $progcontestsettings = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
    ...
    public function test_access_allowed_if_user_has_bypass_capability() {
        ...
        $this->assign_user_capability('progcontestaccess/seb:bypassseb', \context_module::instance($this->progcontest->cmid)->id);
    ...
    /**
     * Test that progcontest form cannot be saved if using template, but not actually pick one.
     */
    public function test_mod_progcontest_form_cannot_be_saved_using_template_and_template_is_not_set() {
        $this->setAdminUser();
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $this->assertContains(get_string('invalidtemplate', 'progcontestaccess_seb'), $errors);
    }

    /**
     * Test that progcontest form cannot be saved if uploaded invalid file.
     */
    public function test_mod_progcontest_form_cannot_be_saved_using_uploaded_file_and_file_is_not_valid() {
        $this->setAdminUser();
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $this->assertContainsEquals(get_string('filenotpresent', 'progcontestaccess_seb'), $errors);
    }

    /**
     * Test that progcontest form cannot be saved if the global settings are set to require a password and no password is set.
     */
    public function test_mod_progcontest_form_cannot_be_saved_if_global_settings_force_progcontest_password_and_none_is_set() {
        $this->setAdminUser();
        // Set global settings to require progcontest password but set password to be empty.
        set_config('progcontestpasswordrequired', '1', 'progcontestaccess_seb');
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $this->assertContains(get_string('passwordnotset', 'progcontestaccess_seb'), $errors);
    }

    /**
     * Test that access to progcontest is allowed if global setting is set to restrict progcontest if no progcontest password is set, and global progcontest
     * password is set.
     */
    public function test_mod_progcontest_form_can_be_saved_if_global_settings_force_progcontest_password_and_is_set() {
        $this->setAdminUser();
        // Set global settings to require progcontest password but set password to be empty.
        set_config('progcontestpasswordrequired', '1', 'progcontestaccess_seb');
        ...
        $errors = progcontestaccess_seb::validate_settings_form_fields([], [
            'instance' => $this->progcontest->id,
            'coursemodule' => $this->progcontest->cmid,
            'progcontestpassword' => 'set',
            'seb_requiresafeexambrowser' => settings_provider::USE_SEB_CONFIG_MANUALLY,
        ], [], $form);
        $this->assertNotContains(get_string('passwordnotset', 'progcontestaccess_seb'), $errors);
    }

    /**
     * Test that progcontest form can be saved if the global settings are set to require a password and no seb usage selected.
     */
    public function test_mod_progcontest_form_can_be_saved_if_global_settings_force_progcontest_password_and_none_no_seb() {
        $this->setAdminUser();
        // Set global settings to require progcontest password but set password to be empty.
        set_config('progcontestpasswordrequired', '1', 'progcontestaccess_seb');
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_NO);
    ...
    /**
     * Test get_download_seb_button, checks for empty config setting progcontestaccess_seb/downloadlink.
     */
    public function test_get_download_seb_button() {
        ...
        $reflection = new \ReflectionClass('progcontestaccess_seb');
        ...
        set_config('downloadlink', '', 'progcontestaccess_seb');
    ...
    public function test_get_get_action_buttons_shows_download_seb_link() {
        ...
        $this->progcontest->seb_showsebdownloadlink = 0;
    ...
    public function test_get_get_action_buttons_shows_launch_and_download_config_links_as_configured() {
        ...
        set_config('showseblinks', 'seb,http', 'progcontestaccess_seb');
        $this->assertStringContainsString($this->get_seb_launch_link(), $method->invoke($this->make_rule()));
        $this->assertStringContainsString($this->get_seb_config_download_link(), $method->invoke($this->make_rule()));

        set_config('showseblinks', 'http', 'progcontestaccess_seb');
        $this->assertStringNotContainsString($this->get_seb_launch_link(), $method->invoke($this->make_rule()));
        $this->assertStringContainsString($this->get_seb_config_download_link(), $method->invoke($this->make_rule()));

        set_config('showseblinks', 'seb', 'progcontestaccess_seb');
        $this->assertStringContainsString($this->get_seb_launch_link(), $method->invoke($this->make_rule()));
        $this->assertStringNotContainsString($this->get_seb_config_download_link(), $method->invoke($this->make_rule()));

        set_config('showseblinks', '', 'progcontestaccess_seb');
        $this->assertStringNotContainsString($this->get_seb_launch_link(), $method->invoke($this->make_rule()));
        $this->assertStringNotContainsString($this->get_seb_config_download_link(), $method->invoke($this->make_rule()));
    }
    ...
    public function test_get_quit_button() {
        $this->setAdminUser();
        $this->progcontest = $this->create_test_progcontest($this->course, settings_provider::USE_SEB_CLIENT_CONFIG);
        $this->progcontest->seb_linkquitseb = "http://test.quit.link";
    ...
    public function test_description() {
        ...
        $this->assertEquals($description[0], get_string('sebrequired', 'progcontestaccess_seb'));
        ...
        // The button is contained in the description when a progcontest attempt is finished.
    ...
    public function test_description_shows_download_config_link_when_required() {
        ...
        $roleid = $this->getDataGenerator()->create_role();
        $context = \context_module::instance($this->progcontest->cmid);
        assign_capability('progcontestaccess/seb:bypassseb', CAP_ALLOW, $roleid, $context->id);
    ...
    /**
     * Test block display before a progcontest started.
     */
    public function test_blocks_display_before_attempt_started() {
        ...
        set_config('displayblocksbeforestart', 0, 'progcontestaccess_seb');
        $this->set_up_progcontest_view_page();
        $this->make_rule()->prevent_access();
        $this->assertEquals('secure', $PAGE->pagelayout);
        $this->assertTrue($property->getValue($PAGE->blocks));

        // Display blocks before start.
        set_config('displayblocksbeforestart', 1, 'progcontestaccess_seb');
        $this->set_up_progcontest_view_page();
        $this->make_rule()->prevent_access();
        $this->assertEquals('secure', $PAGE->pagelayout);
        $this->assertFalse($property->getValue($PAGE->blocks));
    ...
    /**
     * Test block display after a progcontest completed.
     */
    public function test_blocks_display_after_attempt_finished() {
        ...
        set_config('displayblockswhenfinished', 0, 'progcontestaccess_seb');
        $this->set_up_progcontest_view_page();
        $this->make_rule()->prevent_access();
        $this->assertEquals('secure', $PAGE->pagelayout);
        $this->assertTrue($property->getValue($PAGE->blocks));
        ...
        set_config('displayblockswhenfinished', 1, 'progcontestaccess_seb');
        $this->set_up_progcontest_view_page();
        $this->make_rule()->prevent_access();
        $this->assertEquals('secure', $PAGE->pagelayout);
        $this->assertFalse($property->getValue($PAGE->blocks));
    ...
    public function test_should_redirect_to_seb_config_link() {
        ...
        set_config('autoreconfigureseb', '0', 'progcontestaccess_seb');
        ...
        set_config('autoreconfigureseb', '1', 'progcontestaccess_seb');
        $_SERVER['HTTP_USER_AGENT'] = 'TEST';
        $this->assertFalse($method->invoke($this->make_rule()));
