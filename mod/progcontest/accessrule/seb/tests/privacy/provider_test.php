...
use progcontestaccess_seb\privacy\provider;
...
class provider_test extends provider_testcase {
    ...
    /**
     * Setup the user, the progcontest and ensure that the user is the last user to modify the SEB progcontest settings.
     */
    public function setup_test_data() {
        ...
        $this->progcontest = $this->create_test_progcontest($this->course, \progcontestaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        ...
        $progcontestsettings->set('requiresafeexambrowser', \progcontestaccess_seb\settings_provider::USE_SEB_TEMPLATE);
    ...
    public function test_get_contexts_for_userid() {
        ...
        $this->assertEquals(\context_module::instance($this->progcontest->cmid)->id, reset($contextids));
    }

    /**
     * That that no module context is found for a user who has not modified any progcontest settings.
     */
    ...
    public function test_export_user_data() {
        ...
        // the progcontest and not anything with the same instance id.
        ...
        $index = '1'; // Get first data returned from the progcontestsettings table metadata.
        ...
        $data = $writer->get_data([
            get_string('pluginname', 'progcontestaccess_seb'),
            \progcontestaccess_seb\template::TABLE,
            $index,
        ]);
        $this->assertNotEmpty($data);
    ...
    public function test_get_users_in_context() {
        ...
        // Create empty userlist with progcontest module context.
        $userlist = new userlist(\context_module::instance($this->progcontest->cmid), 'progcontestaccess_seb');
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new approved_userlist(\context_module::instance($this->progcontest->cmid),
                'progcontestaccess_seb', [$this->user->id]);

        // Test data exists.
        $this->assertNotEmpty(progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]));

        // Test data is deleted.
        provider::delete_data_for_users($approveduserlist);
        $record = progcontest_settings::get_record(['progcontestid' => $this->progcontest->id]);
        $this->assertEmpty($record->get('usermodified'));

        $template = \progcontestaccess_seb\template::get_record(['id' => $record->get('templateid')]);
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedcontextlist = new approved_contextlist($this->user,
                'progcontestaccess_seb', [$context->id]);
