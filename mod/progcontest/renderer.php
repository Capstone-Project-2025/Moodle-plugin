...
/**
 * Defines the renderer for the progcontest module.
 *
...
/**
 * The renderer for the progcontest module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_progcontest_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param progcontest_attempt $attemptobj an instance of progcontest_attempt.
     ...
     * @param mod_progcontest_display_options $displayoptions instance of mod_progcontest_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(progcontest_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_progcontest_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(progcontest_attempt $attemptobj, $slot, $seq,
            mod_progcontest_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(progcontest_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_progcontest_name(), true,
                                  array("context" => $attemptobj->get_progcontestobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox progcontestreviewsummary'));
     ...
     *
     * @param progcontest_attempt $attemptobj instance of progcontest_attempt
     ...
     * @param mod_progcontest_display_options $displayoptions instance of mod_progcontest_display_options
     */
    public function questions(progcontest_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_progcontest_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_progcontest.init_review_form', null, false,
                progcontest_get_js_module());
     ...
     * @param progcontest_attempt $attemptobj instance of progcontest_attempt
     */
    public function finish_review_link(progcontest_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_progcontest.secure_window.init_close_button',
                    array($url), false, progcontest_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'progcontest'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_progcontest-next-nav btn btn-primary'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'progcontest'),
                    array('class' => 'mod_progcontest-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(progcontest_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'progcontest'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_progcontest-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'progcontest'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_progcontest-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the progcontest timer.
     * @return string HTML content.
     */
    public function countdown_timer(progcontest_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the progcontest to be submitted immediately.
        ...
        return $this->output->render_from_template('mod_progcontest/timer', (object)[]);
    }
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'progcontest'));
    }

    /**
     * Outputs the navigation block panel
     *
     * @param progcontest_nav_panel_base $panel instance of progcontest_nav_panel_base
     */
    public function navigation_panel(progcontest_nav_panel_base $panel) {
        ...
        $this->page->requires->js_init_call('M.mod_progcontest.nav.init', null, false,
                progcontest_get_js_module());
    ...
    /**
     * Display a progcontest navigation button.
     *
     * @param progcontest_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_progcontest_nav_question_button(progcontest_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod, 'btn');
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'progcontest');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'progcontest', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-progcontest-page' => $button->page);
    ...
    /**
     * Display a progcontest navigation heading.
     *
     * @param progcontest_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
    protected function render_progcontest_nav_section_heading(progcontest_nav_section_heading $heading) {
        if (empty($heading->heading)) {
            $headingtext = get_string('sectionnoname', 'progcontest');
            $class = ' dimmed_text';
        } else {
            $headingtext = $heading->heading;
            $class = '';
        }
        return $this->heading($headingtext, 3, 'mod_progcontest-section-heading' . $class);
    }

    /**
     * outputs the link the other attempts.
     *
     * @param mod_progcontest_links_to_other_attempts $links
     */
    protected function render_mod_progcontest_links_to_other_attempts(
            mod_progcontest_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(progcontest $progcontestobj, mod_progcontest_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($progcontestobj->get_progcontest_name(), true,
                                  array("context" => $progcontestobj->get_context())));
        $output .= $this->progcontest_intro($progcontestobj->get_progcontest(), $progcontestobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param progcontest_attempt $attemptobj Instance of progcontest_attempt
     * @param int $page Current page number
     * @param progcontest_access_manager $accessmanager Instance of progcontest_access_manager
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output = '';
        $output .= $this->header();
        $output .= $this->progcontest_notices($messages);
     ...
     */
    public function progcontest_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'progcontest'), 3) .
                $this->access_messages($messages), 'progcontestaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param progcontest_attempt $attemptobj
     * @param int $page Current page number
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $navmethod = $attemptobj->get_progcontest()->navmethod;
     ...
     * @param bool $lastpage is this the last page in the progcontest?
     * @param string $navmethod Optional progcontest attribute, 'free' (default) or 'sequential'
     * @return string HTML fragment.
     */
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'progcontest'), 'class' => 'mod_progcontest-prev-nav btn btn-secondary',
                    'id' => 'mod_progcontest-prev-nav'));
            $this->page->requires->js_call_amd('core_form/submit', 'init', ['mod_progcontest-prev-nav']);
        }
        if ($lastpage) {
            $nextlabel = get_string('endtest', 'progcontest');
        } else {
            $nextlabel = get_string('navigatenext', 'progcontest');
        }
        $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
                'value' => $nextlabel, 'class' => 'mod_progcontest-next-nav btn btn-primary', 'id' => 'mod_progcontest-next-nav'));
        $output .= html_writer::end_tag('div');
        $this->page->requires->js_call_amd('core_form/submit', 'init', ['mod_progcontest-next-nav']);
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
            'value' => get_string('redoquestion', 'progcontest'),
            'class' => 'mod_progcontest-redo_question_button btn btn-secondary');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_progcontest.timer.init', $options, false, progcontest_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'progcontest'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'progcontest'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_progcontest.secure_window.close',
                array($url, $delay), false, progcontest_get_js_module());
     ...
     *
     * @param progcontest_attempt $attemptobj
     * @param mod_progcontest_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_progcontest_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'progcontest'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable progcontestsummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'progcontest'), get_string('status', 'progcontest'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'progcontest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            $sections = $attemptobj->get_progcontestobj()->get_sections();
            if (!is_null($heading) && empty($heading) && count($sections) > 1) {
                $heading = get_string('sectionnoname', 'progcontest');
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'progcontestsummaryheading';
            }
            ...
            if ($attemptobj->is_question_flagged($slot)) {
                // Programmingcontest has custom JS manipulating these image tags - so we can't use the pix_icon method here.
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'progcontestsummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == progcontest_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'progcontest'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'progcontest'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == progcontest_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'progcontest'), null,
                    get_string('submitallandfinish', 'progcontest')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == progcontest_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'progcontest', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'progcontest', userdate($duedate));
        }
     ...
     * @param array $progcontest Array conting progcontest data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this progcontest
     * @param mod_progcontest_view_object $viewobj
     ...
     *      attempt this progcontest now, if appicable this progcontest
     */
    public function view_page($course, $progcontest, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($progcontest, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($progcontest, $context, $viewobj);
        $output .= $this->view_result_info($progcontest, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'progcontestattempt');
        return $output;
    }
     ...
     * @param mod_progcontest_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_progcontest_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'progcontest'), 'get',
     ...
     * @param mod_progcontest_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_progcontest_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_progcontest_preflight_check_form from ' .
                    'progcontest_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext, 'post', true);
        $button->class .= ' progconteststartbuttondiv';
        if ($popuprequired) {
            $button->class .= ' progcontestsecuremoderequired';
        }
        ...
        $this->page->requires->js_call_amd('mod_progcontest/preflightcheck', 'init',
                array('.progconteststartbuttondiv [type=submit]', get_string('startattempt', 'progcontest'),
                       '#mod_progcontest_preflight_form', $popupjsoptions));
    ...
    public function no_questions_message($canedit, $editurl) {
        ...
        $output .= $this->notification(get_string('noquestions', 'progcontest'), 'warning', false);
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editprogcontest', 'progcontest'), 'get');
        }
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
    ...
    /**
     * Outputs an error message for any guests accessing the progcontest
     *
     * @param int $course The course ID
     * @param array $progcontest Array contingin progcontest data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $progcontest, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($progcontest, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'progcontest'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $progcontest, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($progcontest, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'progcontest'));
     ...
     * @return string HTML to output.
     */
    public function view_information($progcontest, $cm, $context, $messages) {
        global $USER;

        $output = '';

        // Print progcontest name.
        $output .= $this->heading(format_string($progcontest->name));
        ...
        // Print progcontest description.
        $output .= $this->progcontest_intro($progcontest, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'progcontestinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/progcontest:viewreports', $context)) {
            if ($strattemptnum = $this->progcontest_attempt_summary_link_to_reports($progcontest, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'progcontestattemptcounts'));
            }
        }

        if (has_any_capability(['mod/progcontest:manageoverrides', 'mod/progcontest:viewoverrides'], $context)) {
            if ($overrideinfo = $this->progcontest_override_summary_links($progcontest, $cm)) {
                $output .= html_writer::tag('div', $overrideinfo, ['class' => 'progcontestattemptcounts']);
            }
        }
    ...
    /**
     * Output the progcontest intro.
     * @param object $progcontest the progcontest settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function progcontest_intro($progcontest, $cm) {
        if (html_is_blank($progcontest->intro)) {
            return '';
        }

        return $this->box(format_module_intro('progcontest', $progcontest, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'progcontest'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $progcontest Array contining progcontest data
     * @param int $context The page context ID
     * @param mod_progcontest_view_object $viewobj
     */
    public function view_table($progcontest, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable progcontestattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'progcontest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'progcontest');
        $table->align[] = 'left';
        $table->size[] = '';
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'progcontest') . ' / ' .
                    progcontest_format_grade($progcontest, $progcontest->sumgrades);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('gradenoun') . ' / ' .
                    progcontest_format_grade($progcontest, $progcontest->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'progcontest');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'progcontest');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'progcontest');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = progcontest_format_grade($progcontest, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = progcontest_rescale_grade($attemptobj->get_sum_marks(), $progcontest, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == progcontest_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $progcontest->grademethod == progcontest_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = progcontest_format_grade($progcontest, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = progcontest_feedback_for_grade($attemptgrade, $progcontest, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param progcontest_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case progcontest_attempt::OVERDUE:
                return get_string('stateoverdue', 'progcontest') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'progcontest',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case progcontest_attempt::FINISHED:
                return get_string('statefinished', 'progcontest') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'progcontest',
    ...
    /**
     * Generates data pertaining to progcontest results
     *
     * @param array $progcontest Array containing progcontest data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_progcontest_view_object $viewobj
     */
    public function view_result_info($progcontest, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = progcontest_get_grading_option_name($progcontest->grademethod);
                $a->mygrade = progcontest_format_grade($progcontest, $viewobj->mygrade);
                $a->progcontestgrade = progcontest_format_grade($progcontest, $progcontest->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'progcontest', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = progcontest_format_grade($progcontest, $viewobj->mygrade);
                $a->maxgrade = progcontest_format_grade($progcontest, $progcontest->grade);
                $a = get_string('outofshort', 'progcontest', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'progcontest', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'progcontest'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'progcontestteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'progcontest'), 3);
            $resultinfo .= html_writer::div(
                    progcontest_feedback_for_grade($viewobj->mygrade, $progcontest, $context),
                    'progcontestgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'progcontest'));
            $button->add_action(new popup_action('click', $url, 'progcontestpopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'progcontest'),
                    array('title' => get_string('reviewthisattempt', 'progcontest')));
        }
    }
     ...
     *
     * @param stdClass $progcontest the progcontest object. Only $progcontest->id is used at the moment.
     ...
     */
    public function progcontest_attempt_summary_link_to_reports($progcontest, $cm, $context,
                                                          $returnzero = false, $currentgroup = 0) {
        global $CFG;
        $summary = progcontest_num_attempt_summary($progcontest, $cm, $returnzero, $currentgroup);
        if (!$summary) {
            return '';
        }

        require_once($CFG->dirroot . '/mod/progcontest/report/reportlib.php');
        $url = new moodle_url('/mod/progcontest/report.php', array(
                'id' => $cm->id, 'mode' => progcontest_report_default_report($context)));
     ...
     * @return string HTML fragment for the link.
     */
    public function progcontest_override_summary_links(stdClass $progcontest, stdClass $cm, $currentgroup = 0): string {

        $baseurl = new moodle_url('/mod/progcontest/overrides.php', ['cmid' => $cm->id]);
        $counts = progcontest_override_summary($progcontest, $cm, $currentgroup);

        $links = [];
        if ($counts['group']) {
            $links[] = html_writer::link(new moodle_url($baseurl, ['mode' => 'group']),
                    get_string('overridessummarygroup', 'progcontest', $counts['group']));
        }
        if ($counts['user']) {
            $links[] = html_writer::link(new moodle_url($baseurl, ['mode' => 'user']),
                    get_string('overridessummaryuser', 'progcontest', $counts['user']));
        }
        ...
        switch ($counts['mode']) {
            case 'onegroup':
                return get_string('overridessummarythisgroup', 'progcontest', $links);

            case 'somegroups':
                return get_string('overridessummaryyourgroups', 'progcontest', $links);

            case 'allgroups':
                return get_string('overridessummary', 'progcontest', $links);
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'progcontest'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'progcontest'), FORMAT_MARKDOWN, $options);
...
class mod_progcontest_links_to_other_attempts implements renderable {
    /**
...
class mod_progcontest_view_object {
    /** @var array $infomessages of messages with information to display about the progcontest. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this progcontest. */
    public $attempts;
    /** @var array $attemptobjs progcontest_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var progcontest_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the progcontest. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this progcontest. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a progcontest. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $progcontesthasquestions whether the progcontest has any questions. */
    public $progcontesthasquestions;
