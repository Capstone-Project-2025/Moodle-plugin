...
/**
 * Unit tests restoring progcontest attempts
 *
...
class restore_attempt_test extends \advanced_testcase {
     ...
     * Test restore dates.
     *
     * @covers \restore_progcontest_activity_structure_step
     */
    public function test_restore_question_attempts_missing_users(): void {
        ...
        // Contains a progcontest with four attempts from different users. The users are as follows (user ID -> user):
        ...
        $restoredprogcontest = $DB->get_record('progcontest', []);
        ...
        $this->assertContains("Mapped user ID not found for user 4, progcontest {$restoredprogcontest->id}, attempt 1. Skipping progcontest attempt",
            $loginfomessages);
        $this->assertContains("Mapped user ID not found for user 5, progcontest {$restoredprogcontest->id}, attempt 1. Skipping progcontest attempt",
            $loginfomessages);

        // User 01 has supplied the wrong answer, assert dates match the backup file too.
        $user01attempt = $DB->get_record('progcontest_attempts', [
            'progcontest' => $restoredprogcontest->id,
        ...
        $user04attempt = $DB->get_record('progcontest_attempts', [
            'progcontest' => $restoredprogcontest->id,
