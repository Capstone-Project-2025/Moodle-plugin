 ...
 * @covers \mod_progcontest_generator
 */
class generator_test extends \advanced_testcase {
    public function test_generator() {
        ...
        $this->assertEquals(0, $DB->count_records('progcontest'));

        /** @var \mod_progcontest_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_progcontest');
        $this->assertInstanceOf('mod_progcontest_generator', $generator);
        $this->assertEquals('progcontest', $generator->get_modulename());

        $generator->create_instance(array('course'=>$SITE->id));
        $generator->create_instance(array('course'=>$SITE->id));
        $createtime = time();
        $progcontest = $generator->create_instance(array('course' => $SITE->id, 'timecreated' => 0));
        $this->assertEquals(3, $DB->count_records('progcontest'));

        $cm = get_coursemodule_from_instance('progcontest', $progcontest->id);
        $this->assertEquals($progcontest->id, $cm->instance);
        $this->assertEquals('progcontest', $cm->modname);
        $this->assertEquals($SITE->id, $cm->course);

        $context = \context_module::instance($cm->id);
        $this->assertEquals($progcontest->cmid, $context->instanceid);

        $this->assertEqualsWithDelta($createtime,
                $DB->get_field('progcontest', 'timecreated', ['id' => $cm->instance]), 2);
    }
    ...
    public function test_generating_a_user_override() {
        ...
        $progcontest = $generator->create_module('progcontest', ['course' => $course->id]);
        ...
        $progcontestgenerator->create_override([
        ...
        $this->assertEquals('progcontest', $event->modulename);
        $this->assertEquals($progcontest->id, $event->instance);
