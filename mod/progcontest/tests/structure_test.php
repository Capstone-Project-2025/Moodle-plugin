...
class structure_test extends \advanced_testcase {
    ...
    /**
     * Create a course with an empty progcontest.
     * @return array with three elements progcontest, cm and course.
     */
    protected function prepare_progcontest_data() {
        ...
        return array($progcontest, $cm, $course);
    }

    /**
     * Creat a test progcontest.
     *
     ...
     * @return progcontest the created progcontest.
     */
    protected function create_test_progcontest($layout) {
        list($progcontest, $cm, $course) = $this->prepare_progcontest_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                progcontest_add_progcontest_question($q->id, $progcontest, $page);
                $lastpage = $page;
            }
        }

        $progcontestobj = new progcontest($progcontest, $cm, $course);
        $structure = structure::create_for_progcontest($progcontestobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_progcontest()}.
     * @param \mod_progcontest\structure $structure the structure to test.
     */
    protected function assert_progcontest_layout($expectedlayout, structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first progcontest section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The progcontest contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the progcontest.');
            }
        }
    }
     ...
     * used by create_test_progcontest as assert_progcontest_layout.
    ...
    public function test_get_progcontest_slots() {
        $progcontestobj = $this->create_test_progcontest(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_progcontest_has_one_section_by_default() {
        $progcontestobj = $this->create_test_progcontest(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = structure::create_for_progcontest($progcontestobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = structure::create_for_progcontest($progcontestobj);
        $this->assert_progcontest_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_progcontest_remove_slot() {
        $progcontestobj = $this->create_test_progcontest(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_progcontest_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        progcontest_add_random_questions($progcontestobj->get_progcontest(), 1, $cat->id, 1, false);
        $structure = structure::create_for_progcontest($progcontestobj);
    ...
    public function test_can_remove_last_question_in_a_progcontest() {
        $progcontestobj = $this->create_test_progcontest(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        progcontest_add_progcontest_question($q->id, $progcontestobj->get_progcontest(), 0);
        $structure = structure::create_for_progcontest($progcontestobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        progcontest_add_progcontest_question($q->id, $progcontestobj->get_progcontest(), 1);
     ...
     * @param  array $layout Programmingcontest layout for create_test_progcontest function
    ...
    public function test_get_slot_tags_for_slot($layout, $tagnames, $slotnumber, $expected) {
        ...
        $progcontest = $this->create_test_progcontest($layout);
        $structure = structure::create_for_progcontest($progcontest);
        ...
        $DB->insert_records('progcontest_slot_tags', $slottagrecords);
    ...
    public function test_can_add_random_questions() {
        ...
        $progcontest = $this->create_test_progcontest([]);
        $course = $progcontest->get_course();
