...
/**
 * Class mod_progcontest_local_structure_slot_random_test
 * Class for tests related to the {@link \mod_progcontest\local\structure\slot_random} class.
 *
...
class local_structure_slot_random_test extends \advanced_testcase {
    ...
    public function test_constructor() {
        ...
        // Create a random question without adding it to a progcontest.
        // We don't want to use progcontest_add_random_questions because that itself, instantiates an object from the slot_random class.
        $form = new \stdClass();
        ...
        $randomslotdata->usingcontextid = \context_module::instance($progcontest->cmid)->id;
        ...
        $rc = new \ReflectionClass('\mod_progcontest\local\structure\slot_random');
        $rcp = $rc->getProperty('record');
        $rcp->setAccessible(true);
        $record = $rcp->getValue($randomslot);

        $this->assertEquals($progcontest->id, $record->progcontestid);
    ...
    public function test_get_progcontest_progcontest() {
        global $SITE, $DB;
        ...
        progcontest_add_random_questions($progcontest, 0, $category->id, 1, false);

        // Get the random question's id. It is at the first slot.
        $questionid = $DB->get_field('progcontest_slots', 'questionid', array('progcontestid' => $progcontest->id, 'slot' => 1));
        ...
        // The create_instance had injected an additional cmid propery to the progcontest. Let's remove that.
        unset($progcontest->cmid);

        $this->assertEquals($progcontest, $randomslot->get_progcontest());
    }

    public function test_set_progcontest() {
        global $SITE, $DB;
        ...
        $rc = new \ReflectionClass('\mod_progcontest\local\structure\slot_random');
        $rcp = $rc->getProperty('progcontest');
        $rcp->setAccessible(true);
        $progcontestpropery = $rcp->getValue($randomslot);

        $this->assertEquals($progcontest, $progcontestpropery);
    }
    ...
    public function test_insert() {
        ...
        // Insert the random question to the progcontest.
        $randomslot = new \mod_progcontest\local\structure\slot_random($randomslotdata);
        $randomslot->set_tags([$footag, $bartag]);
        $randomslot->insert(1); // Put the question on the first page of the progcontest.

        // Get the random question's progcontest_slot. It is at the first slot.
        $progcontestslot = $DB->get_record('progcontest_slots', array('progcontestid' => $progcontest->id, 'slot' => 1));
        // Get the random question's tags from progcontest_slot_tags. It is at the first slot.
        $progcontestslottags = $DB->get_records('progcontest_slot_tags', array('slotid' => $progcontestslot->id));

        $this->assertEquals($question->id, $progcontestslot->questionid);
        $this->assertEquals($category->id, $progcontestslot->questioncategoryid);
        $this->assertEquals(1, $progcontestslot->includingsubcategories);
        $this->assertEquals(1, $progcontestslot->maxmark);

        $this->assertCount(2, $progcontestslottags);
