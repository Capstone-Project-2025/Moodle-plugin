...
Feature: Settings form fields disabled if not required
  In to create progcontestzes as simply as possible
  As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | progcontest     | C1     | 1       | Test progcontest 1 |

  @javascript
  Scenario: Depending on the number of attempts, different form fields are disabled.
    When I am on the "Test progcontest 1" "progcontest activity editing" page logged in as teacher
  ...
  Scenario: Depending on whether there is a close date, some review options are disabled.
    ...
    And I add a "Programmingcontest" to section "1"
    And I expand all fieldsets
    And I set the field "Name" to "Test progcontest"
  ...
  @javascript
  Scenario: If there are progcontest attempts, there is not option to repaginate.
    ...
    And progcontest "Programmingcontest 1" contains the following questions:
      ...
      | TF1      | 1    |         |
    When I am on the "Programmingcontest 1" "progcontest activity editing" page logged in as teacher
    And I expand all fieldsets
    Then I should see "Repaginate now"
    And user "student1" has attempted "Programmingcontest 1" with responses:
      | slot | response |
      |   1  | True     |
    And I am on the "Programmingcontest 1" "progcontest activity editing" page
    And I expand all fieldsets
    And I should not see "Repaginate now"
