@mod @mod_progcontest
Feature: Allow students to redo questions in a practice progcontest, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a progcontest as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | progcontest     | Programmingcontest 1 | Programmingcontest 1 description | C1     | progcontest1    | immediatefeedback  | 1                |
  ...
  Scenario: After completing a question, there is a redo question button that restarts the question
    Given I am on the "Programmingcontest 1" "mod_progcontest > View" page logged in as "student"
    When I press "Attempt progcontest now"
  ...
  Scenario: The redo question button is visible but disabled for teachers
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I log out
    And I am on the "Programmingcontest 1" "mod_progcontest > View" page logged in as "teacher"
  ...
  Scenario: Teachers reviewing can see all the questions attempted in a slot
    ...
    And I follow "TF1"
    And "False" row "Frequency" column of "progcontestresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "progcontestresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "progcontestresponseanalysis" table should contain "100.00%"
  ...
  Scenario: Redoing questions should work with random questions as well
    ...
    And the following "activities" exist:
      | activity | name   | intro              | course | idnumber | preferredbehaviour | canredoquestions |
      | progcontest     | Programmingcontest 2 | Programmingcontest 2 description | C1     | progcontest2    | immediatefeedback  | 1                |
    And progcontest "Programmingcontest 2" contains the following questions:
      | question                | page |
      | Random (Test questions) | 1    |
    And user "student" has started an attempt at progcontest "Programmingcontest 2" randomised as follows:
      | slot | actualquestion |
      | 1    | TF1            |
    And I am on the "Programmingcontest 2" "mod_progcontest > View" page logged in as "student"
  ...
  Scenario: Teachers reviewing can see author of action in review attempt
    ...
    And the following "activities" exist:
      ...
      | progcontest     | Programmingcontest 2 | Programmingcontest 2 description | C1     | progcontest2    |
    And progcontest "Programmingcontest 2" contains the following questions:
      | question | page |
      | SA1      | 1    |
    And user "student" has attempted "Programmingcontest 2" with responses:
      | slot | response    |
      | 1    | J.K.Rowling |
    And I am on the "Programmingcontest 2" "mod_progcontest > View" page logged in as "teacher"
