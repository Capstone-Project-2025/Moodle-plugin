@mod @mod_progcontest
Feature: Programmingcontest user override
  In order to grant a student special access to a progcontest
  As a teacher
  ...
  Scenario: Add, modify then delete a user override
    Given the following "activities" exist:
      ...
      | progcontest       | Test progcontest | C1     | progcontest1    |
    And I am on the "Test progcontest" "mod_progcontest > View" page logged in as "teacher"
  ...
  @javascript
  Scenario: Can add a user override when the progcontest is not available to the student
    Given the following "activities" exist:
      | activity   | name      | course | idnumber | visible |
      | progcontest       | Test progcontest | C1     | progcontest1    | 0       |
    When I am on the "Test progcontest" "mod_progcontest > User overrides" page logged in as "teacher"
    And I press "Add user override"
    And I set the following fields to these values:
      | Override user    | Student One (student1@example.com) |
      | Attempts allowed | 1                                  |
  ...
  Scenario: A teacher without accessallgroups permission should only be able to add user override for users that he/she shares groups with,
    ...
    And the following "activities" exist:
      ...
      | progcontest     | Test progcontest | C1     | progcontest1    | 1         |
    When I am on the "Test progcontest" "mod_progcontest > User overrides" page logged in as "teacher"
    And I press "Add user override"
  ...
  Scenario: A non-editing teacher can see the overrides, but not change them
    Given the following "activities" exist:
      | activity   | name      | course | idnumber |
      | progcontest       | Test progcontest | C1     | progcontest1    |
    And the following "mod_progcontest > user overrides" exist:
      | progcontest      | user     | attempts |
      | Test progcontest | student1 | 2        |
      | Test progcontest | student2 | 2        |
    And I am on the "Test progcontest" "mod_progcontest > View" page logged in as "helper"
    ...
    And I am on the "Test progcontest" "mod_progcontest > View" page
