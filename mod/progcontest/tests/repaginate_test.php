...
/**
 * Unit tests for the {@link \mod_progcontest\repaginate} class.
...
require_once($CFG->dirroot . '/mod/progcontest/classes/repaginate.php');

/**
 * Testable subclass, giving access to the protected methods of {@link \mod_progcontest\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_progcontest_repaginate_testable extends repaginate {

    public function __construct($progcontestid = 0, $slots = null) {
        return parent::__construct($progcontestid, $slots);
    }
...
class repaginate_test extends \advanced_testcase {
    ...
    private $progcontestslots;
    /** @var mod_progcontest_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp(): void {
        $this->set_progcontest_slots($this->get_progcontest_object()->get_slots());
        $this->repaginate = new mod_progcontest_repaginate_testable(0, $this->progcontestslots);
    }
    ...
    /**
     * Create a progcontest, add five questions to the progcontest
     * which are all on one page and return the progcontest object.
     */
    private function get_progcontest_object() {
        global $SITE;
        ...
        $progcontest = $progcontestgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('progcontest', $progcontest->id, $SITE->id);
        ...
        progcontest_add_progcontest_question($shortanswer->id, $progcontest);
        progcontest_add_progcontest_question($numerical->id, $progcontest);
        progcontest_add_progcontest_question($essay->id, $progcontest);
        progcontest_add_progcontest_question($truefalse->id, $progcontest);
        progcontest_add_progcontest_question($match->id, $progcontest);

        // Return the progcontest object.
        $progcontestobj = new progcontest($progcontest, $cm, $SITE);
        return structure::create_for_progcontest($progcontestobj);
    }

    /**
     * Set the progcontest slots
     * @param string $slots
     */
    private function set_progcontest_slots($slots = null) {
        if (!$slots) {
            $this->progcontestslots = $this->get_progcontest_object()->get_slots();
        } else {
            $this->progcontestslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_progcontest_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->progcontestslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->progcontestslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->progcontestslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->progcontestslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->progcontestslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->progcontestslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->progcontestslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->progcontestslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->progcontestslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->progcontestslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_progcontest_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->progcontestslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->progcontestslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->progcontestslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
