...
/**
 * Programmingcontest attempt walk through.
 *
 ...
 * @covers \progcontest_attempt
 */
class attempt_walkthrough_test extends \advanced_testcase {

    /**
     * Create a progcontest with questions and walk through a progcontest attempt.
     */
    public function test_progcontest_attempt_walkthrough() {
        global $SITE;
        ...
        $progcontest = $progcontestgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $timenow = time();
        $attempt = progcontest_create_attempt($progcontestobj, 1, false, $timenow, false, $user1->id);
        ...
        $grades = progcontest_get_user_grades($progcontest, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'progcontest', $progcontest->id, $user1->id);
    ...
    /**
     * Create a progcontest containing one question and a close time.
     *
     * The question is the standard shortanswer test question.
     * The progcontest is set to close 1 hour from now.
     * The progcontest is set to use a grade period of 1 hour once time expires.
     *
     * @return \stdClass the progcontest that was created.
     */
    protected function create_progcontest_with_one_question(): \stdClass {
        global $SITE;
        $this->resetAfterTest();
        ...
        // Add them to the progcontest.
        progcontest_add_progcontest_question($saq->id, $progcontest, 0, 1);
        progcontest_update_sumgrades($progcontest);

        return $progcontest;
    }

    public function test_progcontest_attempt_walkthrough_submit_time_recorded_correctly_when_overdue() {

        $progcontest = $this->create_progcontest_with_one_question();
        ...
        // Start the attempt.
        $attempt = progcontest_prepare_and_start_new_attempt($progcontestobj, 1, null);

        // Process some responses from the student.
        $attemptobj = progcontest_attempt::create($attempt->id);
        $attemptobj->process_submitted_actions($progcontest->timeclose - 30 * MINSECS, false, [1 => ['answer' => 'frog']]);

        // Attempt goes overdue (e.g. if cron ran).
        $attemptobj = progcontest_attempt::create($attempt->id);
        $attemptobj->process_going_overdue($progcontest->timeclose + 2 * get_config('progcontest', 'graceperiodmin'), false);
        ...
        $attemptobj = progcontest_attempt::create($attempt->id);
        $attemptobj->process_attempt($progcontest->timeclose + 30 * MINSECS, true, false, 1);

        // Verify the attempt state.
        $attemptobj = progcontest_attempt::create($attempt->id);
        $this->assertEquals(1, $attemptobj->get_attempt_number());
        $this->assertEquals(true, $attemptobj->is_finished());
        $this->assertEquals($progcontest->timeclose + 30 * MINSECS, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
        $this->assertTrue($attemptobj->has_response_to_at_least_one_graded_question());
    }

    public function test_progcontest_attempt_walkthrough_close_time_extended_at_last_minute() {
        global $DB;

        $progcontest = $this->create_progcontest_with_one_question();
        $originaltimeclose = $progcontest->timeclose;
        ...
        // Teacher edits the progcontest to extend the time-limit by one minute.
        $DB->set_field('progcontest', 'timeclose', $originaltimeclose + MINSECS, ['id' => $progcontest->id]);
        \course_modinfo::clear_instance_cache($progcontest->course);

        // Timer expires in the student browser and thinks it is time to submit the progcontest.
        ...
        $this->assertEquals(progcontest_attempt::IN_PROGRESS, $attemptobj->get_state());
        $this->assertEquals(0, $attemptobj->get_submitted_date());
        $this->assertEquals($user->id, $attemptobj->get_userid());
    }

    /**
     * Create a progcontest with a random as well as other questions and walk through progcontest attempts.
     */
    public function test_progcontest_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $progcontest = $progcontestgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the progcontest.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            progcontest_start_new_attempt($progcontestobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $progcontestwithvariants = null;

    /**
     * Create a progcontest with a single question with variants and walk through progcontest attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_progcontest_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->progcontestwithvariants === null) {
            // Make a progcontest.
            $progcontestgenerator = $this->getDataGenerator()->get_plugin_generator('mod_progcontest');

            $this->progcontestwithvariants = $progcontestgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            progcontest_add_progcontest_question($calc->id, $this->progcontestwithvariants, 0);
        }


        // Make a new user to do the progcontest.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $progcontestobj = progcontest::create($this->progcontestwithvariants->id, $user1->id);
        ...
        // Select variant.
        progcontest_start_new_attempt($progcontestobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check progcontest grades.
        $grades = progcontest_get_user_grades($this->progcontestwithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'progcontest', $this->progcontestwithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
