...
/**
 * Programmingcontest module test data generator class
 *
 * @package mod_progcontest
 * @copyright 2012 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_progcontest_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        $defaultprogcontestsettings = array(
            'timeopen'               => 0,
            'timeclose'              => 0,
            ...
            'showuserpicture'        => 0,
            'showblocks'             => 0,
            'navmethod'              => progcontest_NAVMETHOD_FREE,
        );

        foreach ($defaultprogcontestsettings as $name => $value) {
    ...
    /**
     * Create a progcontest attempt for a particular user at a particular course.
     *
     * @param int $progcontestid the progcontest id (from the mdl_quit table, not cmid).
     ...
     */
    public function create_attempt($progcontestid, $userid, array $forcedrandomquestions = [],
            array $forcedvariants = []) {
        // Build progcontest object and load questions.
        $progcontestobj = progcontest::create($progcontestid, $userid);

        $attemptnumber = 1;
        $attempt = null;

        if ($attempts = progcontest_get_user_attempts($progcontestid, $userid, 'all', true)) {
        ...
        return progcontest_prepare_and_start_new_attempt($progcontestobj, $attemptnumber, $attempt, false,
                $forcedrandomquestions, $forcedvariants);
    }

    /**
     * Submit responses to a progcontest attempt.
     *
    ...
    /**
     * Create a progcontest override (either user or group).
     *
     * @param array $data must specify progcontestid, and one of userid or groupid.
     */
    public function create_override(array $data): void {
        global $DB;

        // Validate.
        if (!isset($data['progcontest'])) {
            throw new coding_exception('Must specify progcontest (id) when creating a progcontest override.');
        }

        if (!isset($data['userid']) && !isset($data['groupid'])) {
            throw new coding_exception('Must specify one of userid or groupid when creating a progcontest override.');
        }

        if (isset($data['userid']) && isset($data['groupid'])) {
            throw new coding_exception('Cannot specify both userid and groupid when creating a progcontest override.');
        }

        // Create the override.
        $DB->insert_record('progcontest_overrides', (object) $data);

        // Update any associated calendar events, if necessary.
        progcontest_update_events($DB->get_record('progcontest', ['id' => $data['progcontest']], '*', MUST_EXIST));
    }
}
