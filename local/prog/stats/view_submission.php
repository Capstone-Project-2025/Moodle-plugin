...
$PAGE->set_url(new moodle_url('/local/prog/stats/view_submission.php', ['id' => $courseid, 'user' => $userid]));
...
foreach ($allsubmissions as $sub) {
    ...
    $language = $DB->get_field('local_prog_language', 'name', ['id' => $languageid]) ?? '---';
...
$templatecontext = [
    'pagingbar' => $OUTPUT->paging_bar($total, $page, $perpage,
        new moodle_url('/local/prog/stats/view_submission.php', [
            'id' => $courseid,
            'user' => $userid,
            'status' => $statusfilter,
    ...
    'backurl' => (new moodle_url('/local/prog/stats/report.php', ['id' => $courseid, 'user' => $userid]))->out(false),
];
...
foreach ($submissions as $obj) {
    ...
    $templatecontext['submissions'][] = [
        ...
        'memory' => $obj->memory,
        'color' => $color,
        'viewlink' => (new moodle_url('/local/prog/stats/submission_viewer.php', ['sid' => $obj->submission_id]))->out(false)
    ];
}
...
echo $OUTPUT->render_from_template('local_prog/view_submission', $templatecontext);
