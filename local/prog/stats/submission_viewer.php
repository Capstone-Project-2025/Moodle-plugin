...
$PAGE->set_url(new moodle_url('/local/prog/stats/submission_viewer.php', ['sid' => $sid]));
...
$problem = $DB->get_record('local_prog_problem', ['id' => $question->problem_id], '*', MUST_EXIST);

// ✅ Nouveau : récupérer le nom du langage via language_id
$language = $DB->get_field('local_prog_language', 'name', ['id' => $submission->language_id]) ?? '---';
...
echo $OUTPUT->render_from_template('local_prog/submission_viewer', $templatecontext);
