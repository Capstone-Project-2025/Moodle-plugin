...
require_capability('local/prog:viewstats', $context);

$PAGE->set_url(new moodle_url('/local/prog/stats/report.php', ['id' => $courseid]));
...
$submissions = $DB->get_records('qtype_prog_submission', ['user_id' => $userid, 'course_id' => $courseid]);
$totalsub = count($submissions);
echo $OUTPUT->heading("Total number of submissions : $totalsub");

// 🔗 Lien vers la page de soumissions
$submissionviewurl = new moodle_url('/local/prog/stats/view_submission.php', ['id' => $courseid, 'user' => $userid]);
...
$sql = "
    ...
    FROM {qtype_prog_submission} s
    JOIN {qtype_prog_options} o ON s.question_id = o.id
    JOIN {local_prog_type} pt ON o.problem_id = pt.problem_id
    JOIN {local_prog_type} t ON pt.type_id = t.id
...
foreach ($lowestTypes as $typename) {
    $sql = "SELECT id FROM {local_prog_type} WHERE " . $DB->sql_compare_text('name') . " = " . $DB->sql_compare_text(':name');
    $typeid = $DB->get_field_sql($sql, ['name' => $typename]);

    if (!$typeid) continue;

    $sql = "SELECT DISTINCT pt.problem_id
            FROM {local_prog_type} pt
    ...
    $sql = "SELECT DISTINCT qo.problem_id
            FROM {qtype_prog_submission} qs
            JOIN {qtype_prog_options} qo ON qs.question_id = qo.questionid
    ...
    $problem = $DB->get_record('local_prog_problem', ['id' => $problemid]);
    if (!$problem) continue;

    $code = $problem->code ?? 'N/A';
    $name = $problem->name ?? 'Unnamed Problem';

    $typerecords = $DB->get_records('local_prog_type', ['problem_id' => $problemid]);
    $typenames = [];
    foreach ($typerecords as $trecord) {
        $tname = $DB->get_field('local_prog_type', 'name', ['id' => $trecord->type_id]);
