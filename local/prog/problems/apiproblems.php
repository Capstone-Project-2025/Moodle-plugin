...
$PAGE->set_url(new moodle_url('/local/prog/problems/apiproblems.php'));
...
$total = $DB->count_records_select('local_prog_problem', $sqlwhere, $params);
$offset = $page * $perpage;
...
foreach ($problems as $problem) {
    $problem_list[] = [
        ...
        'addtestcaseurl' => (new moodle_url('/local/prog/problems/addtestcase.php', ['code' => $problem->code]))->out()
    ];
}
...
$templatecontext = [
    ...
    'minpoints' => $minpoints,
    'pagination' => $OUTPUT->paging_bar($total, $page, $perpage, new moodle_url('/local/prog/problems/apiproblems.php', [
        'searchname' => $searchname,
        'searchdifficulty' => $searchdifficulty,
        'minpoints' => $minpoints
    ]), 'page'),
    'uploadurl' => (new moodle_url('/local/prog/problems/uploadproblem.php'))->out()
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_prog/apiproblems', $templatecontext);
echo $OUTPUT->footer();
