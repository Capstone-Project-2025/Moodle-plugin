...
/**
 * Upgrade script for local_prog plugin
 */
function xmldb_local_prog_upgrade($oldversion) {
    global $DB;

    // Update step to create all missing tables from install.xml (version bump to 2025061904)
    if ($oldversion < 2025061909) {
/*
        // Table: local_prog_language
        $table = new xmldb_table('local_prog_language');
        ...
        // Table: local_prog_problem
        $table = new xmldb_table('local_prog_problem');
        if (!$DB->get_manager()->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        // Table: local_prog_problem_language
        $table = new xmldb_table('local_prog_problem_language');
        if (!$DB->get_manager()->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            ...
            $table->add_key('fk_problem', XMLDB_KEY_FOREIGN, ['problem_id'], 'local_prog_problem', ['id']);
            $table->add_key('fk_language', XMLDB_KEY_FOREIGN, ['language_id'], 'local_prog_language', ['id']);

            $DB->get_manager()->create_table($table);
        }

        // Table: local_prog_type
        $table = new xmldb_table('local_prog_type');
        if (!$DB->get_manager()->table_exists($table)) {
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        if (!$DB->get_manager()->table_exists($table)) {
            ...
            $table->add_key('fk_problem', XMLDB_KEY_FOREIGN, ['problem_id'], 'local_prog_problem', ['id']);
            $table->add_key('fk_type', XMLDB_KEY_FOREIGN, ['type_id'], 'local_prog_type', ['id']);

            $DB->get_manager()->create_table($table);
        }

        $table = new xmldb_table('local_prog_testcase');
    ...
    foreach ($languages as $languageid => $name) {
        ...
        $record->name = $name;
        $DB->insert_record('local_prog_language', $record);
    }

    // Insert default prog problem types
    $types = [
        1 => 'Simple Math',
        2 => 'Array',
        3 => 'Vector',
    ...
    foreach ($types as $typeid => $fullname) {
        ...
        $DB->insert_record('local_prog_type', $record);
    }
*/
        // ✅ Mark the upgrade as successful
        upgrade_plugin_savepoint(true, 2025061909, 'local', 'prog');
    }
