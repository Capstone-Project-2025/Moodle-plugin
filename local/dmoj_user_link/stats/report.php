...
$PAGE->set_url(new moodle_url('/local/dmoj_user_link/stats/report.php', ['id' => $courseid]));
...
$submissionviewurl = new moodle_url('/local/dmoj_user_link/stats/view_submission.php', ['id' => $courseid, 'user' => $userid]);
...
$sql = "
    ...
    JOIN {local_dmoj_user_linktype} pt ON o.problem_id = pt.problem_id
    JOIN {local_dmoj_user_linktype} t ON pt.type_id = t.id
...
foreach ($lowestTypes as $typename) {
    $sql = "SELECT id FROM {local_dmoj_user_linktype} WHERE " . $DB->sql_compare_text('name') . " = " . $DB->sql_compare_text(':name');
    $typeid = $DB->get_field_sql($sql, ['name' => $typename]);

    if (!$typeid) continue;

    $sql = "SELECT DISTINCT pt.problem_id
            FROM {local_dmoj_user_linktype} pt
    ...
    $typerecords = $DB->get_records('local_dmoj_user_linktype', ['problem_id' => $problemid]);
    $typenames = [];
    foreach ($typerecords as $trecord) {
        $tname = $DB->get_field('local_dmoj_user_linktype', 'name', ['id' => $trecord->type_id]);
