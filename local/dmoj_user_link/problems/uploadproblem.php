...
$PAGE->set_url(new moodle_url('/local/dmoj_user_link/problems/uploadproblem.php'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($status === 201) {
            ...
            $problemid = $DB->insert_record('local_dmoj_user_linkproblem', $problem);

            foreach (array_unique($types) as $typeid) {
                $type = new stdClass();
                $type->problem_id = $problemid;
                $type->type_id = $typeid;
                $DB->insert_record('local_dmoj_user_linktype', $type);
            }
            ...
            foreach (array_unique($languages) as $langid) {
                ...
                $DB->insert_record('local_dmoj_user_linkproblem_language', $language);
            }
...
$typerecords = $DB->get_records('local_dmoj_user_linktype', null, 'id ASC');
$langrecords = $DB->get_records('local_dmoj_user_linklanguage', null, 'id ASC');
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_dmoj_user_link/uploadproblem', $templatecontext);
echo $OUTPUT->footer();
