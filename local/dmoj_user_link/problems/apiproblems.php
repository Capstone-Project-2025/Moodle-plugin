...
$PAGE->set_url(new moodle_url('/local/dmoj_user_link/problems/apiproblems.php'));
...
$total = $DB->count_records_select('local_dmoj_user_linkproblem', $sqlwhere, $params);
$offset = $page * $perpage;

$problems = $DB->get_records_select(
    'local_dmoj_user_linkproblem',
    $sqlwhere,
    $params,
    'id ASC',
    '*',
...
foreach ($problems as $problem) {
    $problem_list[] = [
        ...
        'addtestcaseurl' => (new moodle_url('/local/dmoj_user_link/problems/addtestcase.php', ['code' => $problem->code]))->out()
    ];
}
...
$templatecontext = [
    ...
    'minpoints' => $minpoints,
    'pagination' => $OUTPUT->paging_bar($total, $page, $perpage, new moodle_url('/local/dmoj_user_link/problems/apiproblems.php', [
        'searchname' => $searchname,
        'searchdifficulty' => $searchdifficulty,
        'minpoints' => $minpoints
    ]), 'page'),
    'uploadurl' => (new moodle_url('/local/dmoj_user_link/problems/uploadproblem.php'))->out()
];

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_dmoj_user_link/apiproblems', $templatecontext);
echo $OUTPUT->footer();
